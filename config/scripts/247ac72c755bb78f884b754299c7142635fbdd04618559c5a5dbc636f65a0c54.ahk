...
;     refreshes live - for VEHICLE lookups only. Nothing about a part number
;     is kept on disk (v4.2): not the stock, not the news bulletin. Stock is
;     what the counter acts on, and a saved "1 in stock" from half an hour
;     ago is a sale that may have happened, so every part is read live.
;   - WAKE-UP CHECK (v4.2): the PC coming back from sleep is the one time the
;     keepalive cannot have run, so the session is checked a few seconds after
;     the wake and signed in again if it died - before anyone types a number.
;   - BACKGROUND CHECK (v4.2): a Windows scheduled task runs this script with
;     --health at logon and every 10 minutes, so the session is repaired while
;     the window is closed too, and the next launch never waits on a sign-in.
;   - START WITH WINDOWS (v4.3): a shortcut in the Startup folder launches this
;     script, minimised, at every logon - so the window is up, signed in and
;     keeping itself alive before anyone sits down. A startup that cannot reach
;     Isuzu (Wi-Fi still coming up) is retried every 30 s instead of given up.
;   - NO LOOKUP DROPPED (v4.4): a number that arrives while the app is busy
;     (the keepalive poke, a session check, the last lookup) used to be
;     answered with a status line and forgotten. Now it is queued and runs the
;     moment the busy stretch ends - the same way a number typed during
;     sign-in always has been.
...
;==============================================================================
#NoEnv
; NOT #SingleInstance Force. Force replaces the running copy of this script,
; and the background --health run IS this script - so with Force the task
; would have killed the open window every ten minutes. The GUI does its own
; replacing (see the start of GUI mode below); the headless runs share the
; session through the mutex and never touch the window.
#SingleInstance Off
; This app sits on the TASKBAR, not in the notification area. Every way back to
; it - the taskbar button, Ctrl+PageDown, Session > Show - goes through a window
; that is always minimised rather than hidden, so a tray icon would be a second
...
;------------------------- CONFIG (edit here) --------------------------------
global IV_VER  := "4.4"                       ; bump by hand on every change
...
global CFG_STALE_MS := 3000                   ; IDS quiet this long = check the session, re-sign-in if dead
global CFG_TASK := "IsuzuVIN keepalive"       ; the scheduled task that runs --health while the window is closed
global CFG_LNK := A_Startup . "\IsuzuVIN.lnk"  ; the Startup-folder shortcut that launches this, minimised, at logon
...
global gPending := false   ; a lookup that arrived while the app was busy - runs when it is not
global gPendDoor := "part"   ; which window's lookup is queued: part or vin
...
global gTaskOn := true     ; install the scheduled --health task (Session menu turns it off)
global gAutoStart := true  ; keep the Startup-folder shortcut (Session menu turns it off)
...
if FileExist(gIni) {       ; IniRead throws on a missing file in this AHK build
    ...
    IniRead, tmpTaskOn, %gIni%, Session, BackgroundCheck, 1
    gTaskOn := (tmpTaskOn != 0)
    IniRead, tmpAuto, %gIni%, Session, StartWithWindows, 1
    gAutoStart := (tmpAuto != 0)
...
; Repl# on the record we get back - so the only evidence that the number
; changed is that the record on screen is not the number in the box. This is
; that evidence.
global gTypedPart := ""

; IDS keeps a chain: an old number carries Repl#, the new one carries Old#.
; The app shows the number that was typed, exactly as IDS holds it. An old
; number is tagged OLD SUPERSESSION and the line under the box links to the
; number that replaced it - one hop, as IDS reports it, and clicking the link
; is the walk. v3.9 stopped following Repl# on its own: a picker with the old
; number on the paperwork wants to see THAT record, not one three hops away.
global gSupTyped := ""     ; the number the user typed, when the screen shows another
global gSupOlder := []     ; numbers walked past on the way to the one on screen
...
global gStale    := false  ; the last lookup timed out waiting on IDS to answer
global gWakeTries := 0     ; checks still owed after the PC woke up and the first one failed
...
global VehHwnd := 0          ; the vehicle window (v4.0: its own window, not a tab)
global VLookHwnd := 0        ; ...and its Look up button
global gBrandPng := ""       ; the ISUZU wordmark, decoded to %TEMP% at startup
global gBusyHwnd := 0        ; the sweep bar under the part box, shown while a lookup runs
global gBusyDots := 0        ; the button reads Working, Working ., Working .. while busy
global gBusyOn := false
global gBrandHwnd := 0
...
; Nothing about a part number is kept on disk. Earlier versions saved the
; news bulletin beside the part (news_<part>.txt); those files are swept out
; here so an older install cleans itself up on its first run of this version.
Loop, Files, %CFG_CACHE%\news_*.txt
    SafeDelete(A_LoopFileFullPath)
Loop, Files, %CFG_CACHE%\part_*.txt
    SafeDelete(A_LoopFileFullPath)
...
; What #SingleInstance Force used to do, for the GUI only: a second copy of
; the window replaces the first. The headless runs above never get here.
if GuiRunning(oldPid)
    Process, Close, %oldPid%
BuildGui()
return

; Is a copy of this app's window up, in this or any other process? The
; window is always minimised rather than hidden, but DetectHiddenWindows is
; on for the check anyway - a miss here would mean two windows or a killed one.
GuiRunning(ByRef pid = "") {
    prev := A_DetectHiddenWindows
    DetectHiddenWindows, On
    hw := WinExist("Isuzu Parts & VIN Lookup ahk_class AutoHotkeyGUI")
    if (hw)
        WinGet, pid, PID, ahk_id %hw%
    DetectHiddenWindows, %prev%
    return hw ? true : false
}
...
RunHealth() {
    ...
    SafeDelete(gLog)
    BLog("== health check ==")
    ; The open window keeps its own session (KeepAlive every 4 min, wake-up
    ; check) and may be mid-lookup. Nothing for the background run to do.
    if GuiRunning() {
        BLog("HEALTH: SKIPPED (the app window is open and keeps its own session)")
        return
    }
    if !MxAcquire(60000) {
...
BuildGui() {
    ...
    Menu, SessMenu, Add
    Menu, SessMenu, Add, Keep signed in while the window is closed, MenuToggleTask
    if (gTaskOn)
        Menu, SessMenu, Check, Keep signed in while the window is closed
    Menu, SessMenu, Add, Start with Windows (minimised and signed in), MenuToggleAutoStart
    if (gAutoStart)
        Menu, SessMenu, Check, Start with Windows (minimised and signed in)
    ...
    ; The vehicle screen is a window of its own, opened from here. It was the
    ; second tab until v4.0; the tab strip went so the parts screen could be
    ; the whole window, the way BydStock is - one box, one button, one answer.
    Menu, MainMenuBar, Add, &Vehicle, MenuVehicle
    Gui, Main:New, , % "Isuzu Parts & VIN Lookup  v" IV_VER
    Gui, Main:Menu, MainMenuBar
    Gui, Main:Font, s10, Segoe UI
    ; WS_CLIPCHILDREN: the coloured panels are child windows, and the ground
    ; must not paint over them
    Gui, Main:+0x02000000
    Gui, Main:+HwndMainHwnd

    ; ---------------------------- the parts screen ---------------------------
    ; Every y here is 34 less than it was with the tab strip above it.
    Gui, Main:Add, Text, x20 y12, Part number:
    Gui, Main:Add, Edit, x20 y32 w330 vPartInput
    ...
    ; y65 h13, not y94 h16: the busy strip below wants BydStock's 7px of air
    ; under the box, and the part header at 112 does not move
    Gui, Main:Add, Link, x20 y65 w450 h13 vSupLine gSupClick,
    ; The busy strip: a 3px marquee 7px under the part box, the gap BydStock
    ; gives it, swept while a lookup runs and hidden the rest of the time.
    ; The colour option takes the theme off it, which is what lets the bar be
    ; steel and not the system green. Parts tab only: BusySet() never shows
    ; it for a VIN.
    Gui, Main:Add, Progress, x20 y61 w450 h3 c2C4A66 BackgroundD7DDE3 0x8 vBusyBar hwndBusyTmp Hidden
    gBusyHwnd := BusyTmp
    ...
    Gui, Main:-Theme
    Gui, Main:Add, Progress, x20 y78 w3 h36 Background2C4A66 vPartRule Hidden
    Gui, Main:+Theme
    ...
    Gui, Main:Add, Text, x31 y78 w300 vPartNum c6E7B87,
    Gui, Main:Font, s8 Bold, Segoe UI
    ; w134: OLD SUPERSESSION is the widest tag and needs the room
    Gui, Main:Add, Text, x336 y78 w134 Right vPartTag,
    Gui, Main:Font, s12 Bold, Segoe UI
    Gui, Main:Add, Text, x31 y91 w439 vPartTitle,
    Gui, Main:Font, s8 Norm, Segoe UI
    ; w300, not w450: the right of this row belongs to the verdict pill
    Gui, Main:Add, Text, x20 y116 w300 vTradeLine c6E7B87,
    ...
    Gui, Main:Add, ListView, x20 y146 w450 h26 Grid -Multi vLVStock hwndLVStockHwnd, Whs|Branch|Available|On order|Min sell|Pack
    ...
    Gui, Main:Font, s8 Bold, Segoe UI
    Gui, Main:Add, Text, x20 y210 w450 vNewsHdr c7A5600 Hidden, GENERAL TECHNICAL INFORMATION
    Gui, Main:Font, s10 Norm, Consolas
    ...
    Gui, Main:Add, Edit, x20 y226 w450 h58 ReadOnly -WantReturn Hidden vNewsBox c4A3200 BackgroundFFF7E8,
    ...
    Gui, Main:Add, Text, x20 y288 w450 h2 0x10 vPriceRule Hidden
    Gui, Main:Font, s7 Norm, Segoe UI
    ; columns sit 106 apart, not 110: the last figure used to run under .EXC
    Gui, Main:Add, Text, x20 y296 w34 vPriceLbl1 cA2A2A2 Hidden, LIST
    Gui, Main:Add, Text, x126 y296 w38 vPriceLbl2 cA2A2A2 Hidden, TRADE
    Gui, Main:Add, Text, x232 y296 w38 vPriceLbl3 cA2A2A2 Hidden, DAILY
    Gui, Main:Add, Text, x338 y296 w38 vPriceLbl4 cA2A2A2 Hidden, STOCK
    ; The currency sits where .EXC used to. Ex-GST is said once, in words, on
    ; the note under the figures - the same line BydStock carries.
    Gui, Main:Add, Text, x426 y296 w40 Right vPriceExc cB4B4B4 Hidden, AUD
    Gui, Main:Add, Text, x20 y312 w200 vPriceNote cB4B4B4 Hidden, All prices exclude GST
    Gui, Main:Font, s8 Norm, Consolas
    Gui, Main:Add, Text, x56 y294 w60 vPrice1 c3E3E3E Hidden,
    Gui, Main:Add, Text, x162 y294 w60 vPrice2 c3E3E3E Hidden,
    Gui, Main:Add, Text, x268 y294 w60 vPrice3 c3E3E3E Hidden,
    Gui, Main:Add, Text, x374 y294 w56 vPrice4 c3E3E3E Hidden,

    ; The Look up button, on the box's row and owner-drawn steel. Its parent
    ; is Main, which is what makes WM_DRAWITEM reach OnDrawItem.
    Gui, Main:Font, s10, Segoe UI
    Gui, Main:Add, Button, x360 y31 w110 h27 gDoLookup Default vLookBtn hwndLookHwnd, &Look up
    OwnerDrawButton(LookHwnd)
    ; The wordmark, directly above the Look up button on the "Part number:"
    ; row - the same spot BydStock's sits in (its 64x13 at x406 y8; this one
    ; is 68x15, so x402 y7 keeps the same right edge and centre). Baked on
    ; F0F0F0, the window's own ground, so there is no alpha to get wrong.
    if (gBrandPng != "") {
        Gui, Main:Add, Picture, x402 y7 w68 h15 vBrand hwndBrandTmp, %gBrandPng%
        gBrandHwnd := BrandTmp
    }
    OnMessage(0x2B, "OnDrawItem")      ; WM_DRAWITEM
    OnMessage(0x4E, "OnNotify")        ; WM_NOTIFY - warehouse row colours
    OnMessage(0x218, "OnPower")        ; WM_POWERBROADCAST - the PC woke up
    ...
    Gui, Main:Font, s8, Segoe UI
    Gui, Main:Add, Text, x12 y212 w470 vSess c808080, Starting...
    ...
    ; Minimize still creates and SIZES the window, so FitWindow's measurements
    ; work exactly as they do when it is shown. 250 tall is BydStock's idle
    ; height; FitWindow grows it to the result and shrinks it back.
    Gui, Main:Show, Minimize w490 h250

    ; --------------------------- the vehicle window --------------------------
    ; Same shape as the parts screen, one row lower for the kind dropdown,
    ; and its own Look up so Enter works in it. Hidden until Vehicle on the
    ; menu bar (or a vehicle self-test) opens it; Escape and the X hide it
    ; again. Built before BuildPanels() because the warranty band is a child
    ; of THIS window and needs it to exist.
    Gui, Veh:New, +HwndVehHwnd, % "Isuzu Vehicle Lookup  v" IV_VER
    Gui, Veh:+0x02000000
    Gui, Veh:Font, s10, Segoe UI
    Gui, Veh:Add, Text, x20 y12, VIN / Rego / Serial / Engine:
    Gui, Veh:Add, Edit, x20 y32 w236 vVinInput
    Gui, Veh:Add, DropDownList, x262 y32 w88 vKindInput Choose1, VIN|Rego|Serial|Engine
    Gui, Veh:Add, Button, x360 y31 w110 h27 gDoVinLookup Default vVLookBtn hwndVLookHwnd, &Look up
    OwnerDrawButton(VLookHwnd)
    if (gBrandPng != "")
        Gui, Veh:Add, Picture, x402 y7 w68 h15 vVBrand, %gBrandPng%
    ; y66..96 is the warranty band - a coloured child window, see BuildPanels
    Gui, Veh:Font, s11 Bold, Segoe UI
    Gui, Veh:Add, Text, x20 y104 w450 vVinTitle,
    Gui, Veh:Font, s9 Norm, Segoe UI
    ; its own row, and the full width, because it carries three facts now:
    ; the model description, the rego, and high ride / low ride
    Gui, Veh:Add, Text, x20 y126 w450 vVinSub c47535F,
    Gui, Veh:Font, s10, Consolas
    Gui, Veh:Add, ListView, x20 y148 w450 h26 Grid -Multi vLV, Field|Value
    LV_ModifyCol(1, 140)
    LV_ModifyCol(2, 280)
    Gui, Veh:Font, s8, Segoe UI
    Gui, Veh:Add, Text, x12 y212 w470 vVSess c808080, Starting...
    Gui, Veh:Show, Hide w490 h250
    Gui, Main:Default

    BuildPanels()      ; child windows need their parents to exist first
    FitWindow()        ; the idle height, with nothing on screen yet
    VFitWindow()
...
; Vehicle on the menu bar: bring the vehicle window up and put the caret in
; its box. Escape or the X on that window hides it; the parts window is never
; touched by either.
MenuVehicle:
    Gui, Veh:Show, w490
    WinActivate, ahk_id %VehHwnd%
    GuiControl, Veh:Focus, VinInput
    if (gReady && !gBusy)
        SetStatus("Type a VIN, rego, serial or engine number and press Enter.")
return
...
BuildPanels() {
    ...
    ; --- warranty band, the vehicle window's answer to the verdict band ---
    ; a child of the vehicle window, not of Main
    Gui, VBand:New, -Caption +Border +ParentVeh +HwndVBandHwnd
...
ApplyPanels() {
    ...
    if (!gPanelsBuilt)
        return
    if (gVPanelsOn)
        Gui, VBand:Show, x20 y66 w450 h30 NoActivate
    else
        Gui, VBand:Hide
...
OnDrawItem(wParam, lParam, msg, hwnd) {
    global LookHwnd, VLookHwnd
    ; DRAWITEMSTRUCT - the handle members are what shift between 32 and 64 bit
    if (A_PtrSize = 8)
        oHwnd := 24, oDC := 32, oRc := 40
    else
        oHwnd := 20, oDC := 24, oRc := 28
    ; both Look up buttons - the parts window's and the vehicle window's -
    ; paint here; the caption is read off whichever one is asking
    hBtn := NumGet(lParam + 0, oHwnd, "Ptr")
    if (hBtn != LookHwnd && hBtn != VLookHwnd)
        return
    ...
    DllCall("GetWindowText", "Ptr", hBtn, "Str", txt, "Int", 256)
...
; ------------------------------------------------------- the busy signals
; Two of them, the same two BydStock shows: the button reads Working with
; dots that tick over every 400ms, and a steel marquee sweeps under the part
; box. The lookup sleeps between CDP polls, so the timer gets its turns while
; the drive is on. The strip lives on the Parts tab, so it is only shown for a
; part - GuiControl Show on a control of the tab behind would drag it over
; the Vehicle page.
BusySet(on, strip := false) {
    global gBusyHwnd, gBusyDots, gBusyOn, LookHwnd, VLookHwnd
    if (on) {
        gBusyOn := true
        gBusyDots := 0
        GuiControl, Main:, LookBtn, Working
        GuiControl, Veh:, VLookBtn, Working
        if (strip && gBusyHwnd) {
            GuiControl, Main:Show, BusyBar
            SendMessage, 0x40A, 1, 40, , ahk_id %gBusyHwnd%     ; PBM_SETMARQUEE on, 40ms step
        }
        ...
        if (gBusyHwnd) {
            SendMessage, 0x40A, 0, 0, , ahk_id %gBusyHwnd%
            GuiControl, Main:Hide, BusyBar
        }
        GuiControl, Main:, LookBtn, &Look up
        GuiControl, Veh:, VLookBtn, &Look up
    }
    DllCall("InvalidateRect", "Ptr", LookHwnd, "Ptr", 0, "Int", 1)
    DllCall("InvalidateRect", "Ptr", VLookHwnd, "Ptr", 0, "Int", 1)
}

BusyTick:
    gBusyDots := Mod(gBusyDots + 1, 4)
    d := ""
    Loop, %gBusyDots%
        d .= "."
    GuiControl, Main:, LookBtn, % "Working" . (d != "" ? " " . d : "")
    GuiControl, Veh:, VLookBtn, % "Working" . (d != "" ? " " . d : "")
    DllCall("InvalidateRect", "Ptr", LookHwnd, "Ptr", 0, "Int", 1)
    DllCall("InvalidateRect", "Ptr", VLookHwnd, "Ptr", 0, "Int", 1)
return

; ---------------------------------------------------------- the wordmark
; Decoded from the base64 below into %TEMP%\isuzu-parts at startup, the way
; BydStock carries its own. A file on disk because Gui,Add,Picture wants a
; path; %TEMP% because the script folder may be a share nobody can write to.
BrandFiles() {
    global gBrandPng
    dir := A_Temp . "\isuzu-parts"
    FileCreateDir, %dir%
    png := dir . "\isuzu-logo.png"
    if (B64ToFile(IsuzuPngB64(), png))
        gBrandPng := png
}
...
; isuzu-logo.png: 68x15, ISUZU in Arial Bold, black on the window grey,
; stretched a touch wider than the face's own ratio (59 wide at that height)
IsuzuPngB64() {
    return ""
    . "iVBORw0KGgoAAAANSUhEUgAAAEQAAAAPCAYAAABdjGkdAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsMA"
    . "AA7DAcdvqGQAAARsSURBVFhHvVdZKK1fFD8uMg8Zbkkyk5BIMqQIGTIUGSIZnryRPImkSLhEIQ+SB0NIqZtMiRBKKFMSMmSW+6CE"
    . "aN1++9+3+/Z3zrk8HP9frYez1299++zft/Za61MVFBRQfHw8s/HxcVLi+vqa6urqKCYmhjw9PcnBwYGcnJwoMDCQCgsLaWpqShnC"
    . "sLe3RwkJCcwSExPp7OxMSWFYWlpie4OXnp5Oj4+P3Dc9Pc19nxl45eXlPHZxcZHHZmRk0J8/f7hPDpxZOj/Oo7KxsSGVSsWsqalJ"
    . "IM/MzNDPnz+5X5sVFRXR+/u7ELuwsCBwtre3Bb+EwcFBgXd5ecl9bW1tanv9y7y9vXnswMCA4Lu5ueE+OXBmiWNra0sqFxcXvtDe"
    . "3s6Jt7e3JBfLzc2NSktL6devX1RTU0OhoaHChvX19cJGy8vL3Kenp8cyRhNGR0c5z9LSkmWkhNnZWUpOTqbU1FTB0tLSKDc3V/h/"
    . "MGSYhJGREb5uZWXFzqMJOLPEc3V11S5IX18fX4cYHx8fwoOAqKgo4TAvLy/cpwtB/gUc2NTUlMfiCt/d3Ql+nQrS09PD1+3s7Ghl"
    . "ZUV4ELCzs0O1tbXU0tLC+P+XIPLDwry8vNTidC7I6ekpGRkZCRtHRERQZWUl/f79my4uLoQHK/FdgkxOTpK+vj6PQYE/OTlR0nQv"
    . "CNDf3y9sLjeka1hYGDU0NAipKuE7BEFHkl8Te3t72t3dVdIYvkUQYG1tjbKzs/+rwBqEgTk6OrKuIoeuBdna2iJra2uBu76+rqRx"
    . "DA8Pc65OBZFwf3/P0hUdJikpSa0d48/KWyZqjq4EOTw8ZPOPxDM2Nqb5+XklTcDY2Bjnm5ub09XVlZLCIBeEaaFJEHQU1AgcAm9e"
    . "01CDAQo93NDQkMd3d3dz/8bGBl+HaUvtoaEhzkEbfXh4EPzn5+esy0mcHz9+sBr2GSYmJniMiYmJRqEBjBESz8PDQ7Mgb29v5OPj"
    . "w9erq6uVz+FAhZd4VVVVfP34+JgMDAy4T9tE29rayjm4evJOhaz09fXlfhgE/ApWV1d5DDJ0c3NTSWHAdCvxgoKCNAsCYKSX1nEH"
    . "lTUCwJ+TdyJMhxJeX1/J2dmZ+4KDg9nblgO/MV1KHMw1ElCoQ0JCuA9vWf78z4BMMzMz4/FxcXFq2YcbIL+KeXl52gU5Ojpid0/y"
    . "IVUxNVZUVDBV8W0j+WD4znl6ehI2bG5uFji4EpgmMfFi0kSXkPtRTyTI5yCYhYUFpaSkUGxsLNtbm2FckCB/+zAcPicnh8rKyigz"
    . "M5PVLLkf3z8qeeVubGzkDwOQFXLBtJmfnx/t7+8LsRKwuZKvNIiNAU+Ozs5ONd5XDN1IArI0Pz9fjaM0ZF9XVxeLUZWUlLC3BkMh"
    . "UgLFs6Ojg30/+Pv7k7u7OytyAQEBlJWVRb29vfT8/KwME4D5AfuEh4ezK4L2htoTGRnJ3qKm+z03N8f+E75Uv2ISV9PAiI/U4uJi"
    . "dgWxL/bH/4iOjmaD5sHBAef+BQ1DVycNW43mAAAAAElFTkSuQmCC"
}
...
ListFit(ctl, rows, maxH, gui := "Main") {
    global
    local hLV, hHdr, rc, rowH, hdrH, h, r0, r1, ok0, ok1
    GuiControlGet, hLV, %gui%:Hwnd, %ctl%
...
; The window is only as tall as what is on it: the list is only as tall as
; its rows, the verdict line, news and price rail stack under the list, and
; the session line rides the bottom. Idle it is BydStock's 250; a result grows
; it and the next clear shrinks it back.
FitWindow() {
    global
    local top, bottom, ch, base
    ; the part header starts at 78 (it was 112 with the tab strip). The
    ; header controls never move, so their y is set once in BuildGui; only
    ; the pill's row is recorded here for ApplyPanels.
    base := 78
    ; the band is 30 tall whatever the pill is, so the table does not move
    ; under a part that happens to have an ETA on it
    gPillY := base + 34
    top := base + 68
    ...
    if (gNewsOn) {
        GuiControl, Main:Move, NewsHdr, % "y" (bottom + 12)
        GuiControl, Main:Move, NewsBox, % "y" (bottom + 28)
        GuiControlGet, nb, Main:Pos, NewsBox
        bottom := bottom + 28 + nbH
    }
    ; then the rail, on whatever that bottom turned out to be
    if (gPriceOn) {
        GuiControl, Main:Move, PriceRule, % "y" (bottom + 12)
        Loop, 4 {
            GuiControl, Main:Move, PriceLbl%A_Index%, % "y" (bottom + 20)
            GuiControl, Main:Move, Price%A_Index%, % "y" (bottom + 18)
        }
        GuiControl, Main:Move, PriceExc, % "y" (bottom + 20)
        ; the GST note, one line under the figures; the rail is 52 with it
        GuiControl, Main:Move, PriceNote, % "y" (bottom + 36)
        bottom := bottom + 52
    }
    ; 212 is the floor that makes an empty window 250 tall - BydStock's size
    if (bottom < 212)
        bottom := 212
    GuiControl, Main:Move, Sess, % "y" (bottom + 14)
    ch := bottom + 38
    ; FitWindow runs on startup and on every result change, including while
    ...
    ApplyPanels()
    ; Everything under the table just moved, and a control that moves up
    ; can leave its old pixels on the ground behind it - a second news
    ; header sitting on the price rail was seen once and could not be made
    ; to happen again. Repaint the ground from the table down, children
    ; included, so nothing stale survives whatever order the moves ran in.
    RepaintBelow(MainHwnd, top)
}

; Invalidate a window from y down to its bottom edge - erase, and every
; child in that band - and let the normal paint pass redraw it. The rows
; above y (header, part line, band) are left alone so they do not flicker.
RepaintBelow(hwnd, y) {
    if (!hwnd)
        return
    VarSetCapacity(rc, 16, 0)
    DllCall("GetClientRect", "Ptr", hwnd, "Ptr", &rc)
    NumPut(y, rc, 4, "Int")                          ; top := y
    DllCall("RedrawWindow", "Ptr", hwnd, "Ptr", &rc, "Ptr", 0, "UInt", 0x85)  ; RDW_INVALIDATE|RDW_ERASE|RDW_ALLCHILDREN
}

; The vehicle window, the same way: the field list starts under the warranty
; band at 148 and the window ends 38 under the list. A hidden window is sized
; hidden (Gui,Show with Hide) so opening it later lands at the right height
; instead of flashing 250 tall and then jumping.
VFitWindow() {
    global
    local top, bottom, ch, vis
    top := 148
    GuiControlGet, lv, Veh:Pos, LV
    bottom := top + lvH
    if (bottom < 212)
        bottom := 212
    GuiControl, Veh:Move, VSess, % "y" (bottom + 14)
    ch := bottom + 38
    vis := VehHwnd && DllCall("IsWindowVisible", "Ptr", VehHwnd)
    Gui, Veh:Show, % (vis ? "NoActivate" : "Hide") " w490 h" ch
    ApplyPanels()
    RepaintBelow(VehHwnd, top)
}
...
LadderSet(ordered, news := "", part := "") {
    ...
    if (pickState = "stock")
        ; the branch is named, as BydStock names it: the picker reads the
        ; pill before the table, and "Melbourne" is the whole answer
...
; The news lands AFTER the result is drawn - it is walked live once the
; figures are up - so NewsShow() calls this again every time the text
; changes, and gLadNone says whether the line is ours to rewrite.
LadFootNone(news) {
    global gLadPart
...
PriceRailShow(on) {
    ...
    if (on) {
        ...
        GuiControl, Main:Show, PriceNote
        Loop, 4 {
        ...
        GuiControl, Main:Hide, PriceNote
        Loop, 4 {
...
NewsShow(text) {
    ...
    ; reason - and a vehicle result is drawn twice, saved copy then live.
    if (text = gNewsText)
        return
    gNewsText := text
...
; The news is not cached (v4.2). It used to be saved beside the part so a
; repeat lookup showed it at once; now nothing about a part is kept on disk
; and the bulletin is walked live on every lookup, a second after the stock.
...
InitSession:
    ...
    try {
        ...
        if !OpenPage() {
            ...
            StartupRetry("cannot reach Isuzu")
            return
        }
        if (gAttached && IsLoggedIn() && SessionAlive()) {
            gReady := true
        ...
        if (gSelfTest != "") {
            stv := gSelfTest
            ; --selftest part:8982488221 tests the parts screen
            if (SubStr(stv, 1, 5) = "part:") {
                ...
                Gui, Veh:Show, w490
                GuiControl, Veh:, VinInput, %stv%
                SetTimer, DoVinLookup, -500
            }
        }
        ; a number typed or handed over while we were still signing in runs
        ; the moment gBusy clears below
    } catch e {
        ...
        gBusy := false
        PendingRun()
        ; off the startup path: the task check spawns PowerShell, and nobody
        ; waiting on a part number should pay for that. In finally, so the
        ; Startup shortcut and the task are kept even when the sign-in failed.
        SetTimer, TaskEnsure, -2000
    }
return

; Startup could not reach Isuzu, or the sign-in did not go through. At logon
; that is nearly always Wi-Fi still coming up, not Isuzu being down - so
; instead of sitting on "down" until someone types a number, the check is
; tried again every 30 s, six times, the way a wake-up is. KeepAlive arms its
; own periodic timer on the first run, so after that the 4-minute cycle
; carries on as if startup had gone through first time.
StartupRetry(why) {
    global gWakeTries
    SLog("startup: " why " - checking again every 30 s")
    gWakeTries := 6
    SetTimer, KeepAlive, -30000
}

; Two doors, one room. The parts window's button (and Enter in its box) comes
; in through DoLookup; the vehicle window's through DoVinLookup. Neither does
; the lookup itself: a button's thread must be over in an instant, because
; AutoHotkey drops a second press of the same button while its first thread
; is still running - so a press that arrived mid-lookup used to vanish. Each
; door only notes which window asked (gPending, gPendDoor) and LookupGo does
; the work on a timer thread of its own, now, or when the busy stretch ends
; (PendingRun). LookupGo reads the window when it fires, so the newest number
; in the box is the one looked up.
DoLookup:
    LookupAsk("part")
return
...
LookupAsk(door) {
    global gPending, gPendDoor, gBusy
    gPending := true
    gPendDoor := door
    if (!gBusy)
        SetTimer, LookupGo, 10
}

; The timer is periodic and turns itself off here: a one-shot could be skipped
; if it fell due while a LookupGo thread was still up, and the request would
; then wait for the next busy stretch to end. Periodic, it simply tries again.
LookupGo:
    if (!gPending) {
        SetTimer, LookupGo, Off
        return
    }
    if (gBusy)
        return
...
LookupRun:
    ...
    ; Busy - signing in, poking the session, or still on the last lookup?
    ; Remember which window asked and run it the moment the busy stretch ends
    ; (PendingRun, called wherever gBusy clears), instead of making the user
    ; notice nothing happened and press again. LookupGo already checks this,
    ; so this is the belt to its braces.
    if (gBusy) {
        gPending := true
        gPendDoor := (kind = "part") ? "part" : "vin"
        if (!gReady)
            SetStatus("Signing in - your lookup will run as soon as that is done...")
        else
            SetStatus("One moment - finishing the last thing, then " v " runs...")
        return
    }
    gBusy := true
    lkT0 := A_TickCount
    ; the button says Working and the strip under the box sweeps until the
    ; finally below puts them back - whatever route the lookup leaves by
    BusySet(true, kind = "part")
    ...
    ; Vehicles only. A part's stock count must be live every time: the
    ; saved copy used to go up first and, when Isuzu could not be reached,
    ; stayed up with a status line - and a stale "1 in stock" reads as one
    ; in stock. So for a part there is no saved copy to show and none to
    ; fall back on; the screen clears and waits for Isuzu.
    cf := (kind = "part") ? "" : CacheFile(kind, v)
    cached := ""
    if (cf != "" && FileExist(cf)) {
        FileRead, cached, %cf%
    ...
    try {
        ...
        if (res = "") {
            ...
            ; A stale session is the one fault worth naming on screen: IDS
            ; went quiet for CFG_STALE_MS (3 s), so the session is signed in
            ; again and the same number is sent again - the line says so.
            if (gStale)
                SetStatus("Isuzu went quiet - signing back in, then searching " v " again...")
            else
        ...
        if (res = "NOTFOUND") {
            ...
            ; cached under the number that was typed, and shown as fetched -
            ; no walk forward. An old number is shown as the old number, with
            ; its replacement one click away (see ShowSupersession).
...
; Anything queued while the app was busy runs now. Called wherever gBusy is
; cleared, so a number that arrived during a keepalive poke, a session check
; or the last lookup is never lost. A timer rather than a Gosub, so the busy
; stretch's own thread finishes first and the lookup gets a clean one.
PendingRun() {
    global gPending
    if (!gPending)
        return
    SetTimer, LookupGo, 50
}

; periodic session health check + keepalive poke. Also the wake-up check:
; OnPower fires this once, a few seconds after the PC comes back from sleep,
; and a failed wake-up check (network still coming up) is tried again every
; 30 s a few times before the 4-minute rhythm takes over.
KeepAlive:
    ; a one-shot run (wake-up, retry) must not cost the periodic timer
    SetTimer, KeepAlive, %CFG_KEEPALIVE%
    if (gBusy)
        return
    gBusy := true
    ...
    try {
        if SessionCheck() {
            gWakeTries := 0
        } else if (gWakeTries > 0) {
            gWakeTries--
            SetSessState("working", "Isuzu not reachable yet - checking again in 30 s...")
            SetTimer, KeepAlive, -30000
        }
    } finally {
        MxRelease()
...
; WM_POWERBROADCAST. PBT_APMRESUMEAUTOMATIC (0x12) is the one message every
; wake sends, whether a person or a timer woke the PC. The session was not
; poked while the PC slept, so it is checked - and signed in again if it
; died - 5 s after the wake, which is before anyone has a number to type.
; 5 s is for the network: a check the instant the screen lights up would find
; no Wi-Fi and call the session dead when it is only unreachable.
OnPower(wParam, lParam) {
    global gWakeTries
    if (wParam != 0x12)
        return
    SLog("wake: PC resumed - checking the session in 5 s")
    gWakeTries := 6
    SetTimer, KeepAlive, -5000
}
...
; A Windows scheduled task runs this script with --health at logon and every
; 10 minutes for as long as the user is signed in to Windows. With the window
; open it does nothing (RunHealth sees the window and leaves). With the window
; closed it is the keepalive: it starts Chrome after a reboot, signs in after
; a sleep, and repairs a dead session - so the next launch attaches to a live
; session instead of signing in while someone waits on a part number.
; Off by Session > Keep signed in while the window is closed; the choice is
; kept in settings.ini so it holds across launches.
TaskEnsure:
    AutoStartEnsure()
    if (!gTaskOn)
        return
    if TaskExists()
        return
    SLog("task: installing the background check")
    if TaskInstall()
        SLog("task: installed - --health at logon and every 10 min")
    else
        SLog("task: install FAILED - see task_install.log")
return

TaskExists() {
    global CFG_TASK
    RunWait, %ComSpec% /c schtasks /Query /TN "%CFG_TASK%" >nul 2>&1, , Hide
    return (ErrorLevel = 0)
}

; Register-ScheduledTask rather than schtasks /Create: schtasks defaults to
; "only on AC power" and one trigger per task, and this needs to run on a
; laptop on battery and to fire both at logon and on the clock.
TaskInstall() {
    global CFG_TASK
    q := Chr(39)   ; PowerShell single quote; a quote inside a path is doubled
    ps := "$exe = " q StrReplace(A_AhkPath, q, q q) q "`n"
        . "$scr = " q StrReplace(A_ScriptFullPath, q, q q) q "`n"
        . "$dir = " q StrReplace(A_ScriptDir, q, q q) q "`n"
        . "$me = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name`n"
        . "$a = New-ScheduledTaskAction -Execute $exe -Argument ('""' + $scr + '"" --health') -WorkingDirectory $dir`n"
        . "$t1 = New-ScheduledTaskTrigger -AtLogOn -User $me`n"
        . "$t1.Delay = 'PT1M'`n"
        . "$t2 = New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(1) -RepetitionInterval (New-TimeSpan -Minutes 10)`n"
        . "$s = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -ExecutionTimeLimit (New-TimeSpan -Minutes 5) -MultipleInstances IgnoreNew`n"
        . "$p = New-ScheduledTaskPrincipal -UserId $me -LogonType Interactive -RunLevel Limited`n"
        . "Register-ScheduledTask -TaskName " q CFG_TASK q " -Action $a -Trigger @($t1, $t2) -Settings $s -Principal $p -Force | Out-Null`n"
        . "'installed'`n"
    f := A_Temp "\IsuzuVIN_task.ps1"
    log := A_ScriptDir "\task_install.log"
    SafeDelete(f)
    FileAppend, %ps%, %f%
    SafeDelete(log)
    RunWait, %ComSpec% /c powershell -NoProfile -ExecutionPolicy Bypass -File "%f%" > "%log%" 2>&1, , Hide
    SafeDelete(f)
    return TaskExists()
}
...
TaskRemove() {
    global CFG_TASK
    RunWait, %ComSpec% /c schtasks /Delete /TN "%CFG_TASK%" /F >nul 2>&1, , Hide
    return !TaskExists()
}

;=============================== start with Windows ============================
; A shortcut in the user's Startup folder launches this script at every logon.
; It opens minimised (the window always does) and signs in from there, so by
; the time anyone sits down the session is up and the 4-minute keepalive is
; running - the window is never closed, only put away, so nothing is left to
; the 10-minute task. Checked on every GUI start rather than only when the
; menu item is ticked, because the script moves (new PC, new folder) and a
; shortcut pointing at the old path would launch nothing.
AutoStartEnsure() {
    global CFG_LNK, gAutoStart
    if (!gAutoStart)
        return
    if AutoStartOk()
        return
    if AutoStartInstall()
        SLog("startup: shortcut written - " CFG_LNK)
    else
        SLog("startup: could not write the shortcut " CFG_LNK)
}

; true when the shortcut is there AND points at this exe, this script and this folder
AutoStartOk() {
    global CFG_LNK
    if !FileExist(CFG_LNK)
        return false
    FileGetShortcut, %CFG_LNK%, tgt, dir, args
    if (ErrorLevel)
        return false
    return (tgt = A_AhkPath && dir = A_ScriptDir && args = AutoStartArgs())
}
...
AutoStartInstall() {
    global CFG_LNK
    args := AutoStartArgs()
    ; RunState 7 = SW_SHOWMINNOACTIVE: even the console flash stays off the desk
    FileCreateShortcut, %A_AhkPath%, %CFG_LNK%, %A_ScriptDir%, %args%, Isuzu Parts & VIN Lookup - opens minimised and signed in, %A_AhkPath%, , , 7
    return (!ErrorLevel && AutoStartOk())
}

AutoStartRemove() {
    global CFG_LNK
    SafeDelete(CFG_LNK)
    return !FileExist(CFG_LNK)
}

MenuToggleAutoStart:
    gAutoStart := !gAutoStart
    IniWrite, % (gAutoStart ? 1 : 0), %gIni%, Session, StartWithWindows
    if (gAutoStart) {
        Menu, SessMenu, Check, Start with Windows (minimised and signed in)
        if AutoStartInstall()
            SetStatus("On. This opens minimised and signed in every time you sign in to Windows.")
        else
            SetStatus("Could not write the Startup shortcut - see session.log.")
        SLog("startup: shortcut turned on by the Session menu")
    } else {
        Menu, SessMenu, Uncheck, Start with Windows (minimised and signed in)
        if AutoStartRemove()
            SetStatus("Off. Open it yourself after signing in to Windows.")
        else
            SetStatus("Could not remove the Startup shortcut - see session.log.")
        SLog("startup: shortcut removed by the Session menu")
    }
return

MenuToggleTask:
    gTaskOn := !gTaskOn
    IniWrite, % (gTaskOn ? 1 : 0), %gIni%, Session, BackgroundCheck
    if (gTaskOn) {
        Menu, SessMenu, Check, Keep signed in while the window is closed
        SetStatus("Turning the background check on...")
        if (TaskExists() || TaskInstall())
            SetStatus("On. The session is checked at logon and every 10 minutes, even with this window closed.")
        else
            SetStatus("Could not install the background check - see task_install.log.")
    } else {
        Menu, SessMenu, Uncheck, Keep signed in while the window is closed
        if TaskRemove()
            SetStatus("Off. The session is only kept alive while this window is open.")
        else
            SetStatus("Could not remove the background check - see session.log.")
        SLog("task: removed by the Session menu")
    }
return
...
ClearResults(kind) {
    if (kind = "part") {
        ...
        Gui, Veh:Default           ; see ShowResultPart - LV_ needs this
        Gui, Veh:ListView, LV
        LV_Delete()
        ListFit("LV", 0, 296, "Veh")
        GuiControl, Veh:, VinTitle,
        GuiControl, Veh:, VinSub,
        VPanelsShow(false)
        Gui, Main:Default
    }
}
...
ShowResult(res, kind = "") {
    ...
    GuiControl, Veh:, VinTitle, % map["vin"]
    GuiControl, Veh:, VinSub, % map["model_desc"]
...
; one live DC210 read of a part number - never saved
PartFetch(num) {
...
ShowResultPart(map) {
    global gShowTrade, gRowState, gLadNone
    Gui, Main:ListView, LVStock
    LV_Delete()
    if (map["part"] = "") {
        GuiControl, Main:, PartNum,
    ...
    ; The part number goes in with it: on a part nobody holds, direct ship
    ; is the difference between 2 to 3 weeks and 6 to 8, and the accessory
    ; list answers that at once. The bulletin is not in hand yet - it is
    ; walked live after the figures are up - so LadFootNone runs again when
    ; it lands (NewsShow).
    LadderSet(ordered, "", map["part"])
    ; An old number: the line under the table points at the replacement, and
    ; a BACKORDER pill with an ETA comes off - nobody orders a dead number.
    ; Stock an old number still holds is real, so that pill stays.
    repl := Trim(map["repl"])
    if (repl != "" && repl != Trim(map["part"])) {
        if (gLadNone)
            PillSet("", "", "")
        LadFootSet("none", "Old number - stock is under " repl)
    }
    ...
    ; no news yet: the live walk runs after this and fills the box in
    NewsShow("")
    SafeDelete(A_ScriptDir "\last_result.txt")
    FileAppend, %dump%, %A_ScriptDir%\last_result.txt
}
...
ShowSupersession(map) {
    ...
    gSupNums := []
    if (repl != "" && repl != part) {
        ; an old record, shown as typed - this warning must not fade. The
        ; link is the next hop as IDS reports it; a longer chain is walked
        ; one click at a time.
        GuiControl, Main:+cA35B00, PartTag
        GuiControl, Main:, PartTag, OLD SUPERSESSION
        gSupNums.Push(repl)
        GuiControl, Main:, SupLine, % "Old supersession - replaced by <a>" repl "</a>.  Click it to look that one up."
        return
    }
...
SetStatus(s) {
    ...
    if (!gBatch) {
        ...
        ; the vehicle window carries the same line, so whichever one is in
        ; front says what is happening
        GuiControl, Veh:+c808080, VSess
        GuiControl, Veh:, VSess, %s%
    }
}
...
SetSessState(state, detail = "") {
    ...
    GuiControl, Veh:+%colour%, VSess
    dot := A_IsUnicode ? Chr(0x25CF) " " : ""
    GuiControl, Main:, Sess, % dot word (detail != "" ? "  -  " detail : "")
    GuiControl, Veh:, VSess, % dot word (detail != "" ? "  -  " detail : "")
}
...
; Part results are never written: stock is looked up live every time (see
; the note at the top of LookupRun), and since v4.2 the bulletin is not
; written either - nothing about a part is on disk. Vehicle results are.
CacheWrite(kind, value, res) {
    if (kind = "part" || kind = "news")
        return
...
; than CFG_STALE_MS to answer and the session did not prove alive after it.
LkStale(why) {
    global gStale
    gStale := true
    return LkFail(why)
...
Lookup(field, value) {
    global gBatch, gLog, gDump, gFailWhy, gCdpDead, gStale
    gFailWhy := "", gCdpDead := false, gStale := false
    if (field = "part")
...
PartLookup(value) {
    global gBatch, gLog, gDump, CFG_STALE_MS
    jsMenu := "(function(){var e=Array.from(document.querySelectorAll('input')).filter(function(x){var r=x.getBoundingClientRect();return !x.readOnly&&r.width>0&&r.top<140&&r.left<400;})[0];if(!e)return'';var r=e.getBoundingClientRect();return Math.round(r.left+r.width/2)+','+Math.round(r.top+r.height/2);})()"
    if !Coord(jsMenu, mx, my)
    ...
    ; The first answer the server has to give. CFG_STALE_MS without it and
    ; the session is asked, with a real RAP send, whether it is still there:
    ; a dead one comes back as stale, and DoLookup signs in again and resends
    ; the number. A live one that was only slow gets the old fallback below.
    if !WaitCoord(jsItem, fx, fy, CFG_STALE_MS, 200) {
        if (!SessionAlive())
            return LkStale("DC210: no answer in " CFG_STALE_MS "ms and the session is dead")
        ; fallback: retry the menu open with slow per-key typing
        if !Coord(jsMenu, mx, my)
    ...
    ; on it and gets its own 4.5s rule above; anything else past CFG_STALE_MS
    ...
    Loop, 40 {
        ...
        if (st = "empty") {
            ...
            empties := 0
            if (!asked && st = "wait" && A_TickCount - pt0 > CFG_STALE_MS) {
                asked := true
                if (!SessionAlive())
                    return LkStale("DC210: no record in " CFG_STALE_MS "ms and the session is dead")
            }
        }
        if (gBatch)
