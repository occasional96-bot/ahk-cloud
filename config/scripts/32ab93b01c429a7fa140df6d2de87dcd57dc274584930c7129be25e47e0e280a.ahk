...
global gWinHidden := true    ; window starts parked in the tray; FitWindow
                             ; must resize WITHOUT unhiding it
...
BuildGui() {
    global
    ; A hidden window with no taskbar button needs a way back that does not
    ; depend on remembering a hotkey: the tray icon, double-clicked.
    Menu, Tray, Add
    Menu, Tray, Add, Show window (Ctrl+PageDown), MenuShowWindow
    Menu, Tray, Default, Show window (Ctrl+PageDown)
    Menu, Tray, Tip, Isuzu Parts & VIN Lookup
    Menu, SessMenu, Add, Hide window (Ctrl+PageDown brings it back), MenuHideWindow
    ...
    ; Launching this does not put a window in your face. It parks in the tray
    ; and signs in there, so by the time you actually want it - tray icon or
    ; Ctrl+PageDown - the session is already up and it opens instantly.
    ; Hide still creates and SIZES the window, so every measurement below
    ; (gTabDY, control positions) works exactly as it did when it was shown.
    Gui, Main:Show, Hide w490 h530
...
FitWindow() {
    ...
    ch := bottom + 38
    ; FitWindow runs on startup and on every tab/result change, including
    ; while the window is parked. Gui,Show is the only way to set the height,
    ; so re-assert Hide or the first fit would pop the window open by itself.
    Gui, Main:Show, % (gWinHidden ? "Hide " : "NoActivate ") "w490 h" ch
...
; The X does not close this app, it puts it away. Ctrl+PageDown brings it
; back, and it comes back INSTANTLY because nothing was ever torn down: the
; Chrome session, the signed-in IDS session and the keep-alive timer all keep
; running behind the hidden window. Signing in again costs seconds; hiding
; costs nothing. Escape does the same thing, because it is the same reflex.
;
...
; The minimize button means the same thing as the X here: put it away. Sending
; it to the taskbar instead would leave two ways back that behave differently.
MainGuiSize:
    if (A_EventInfo = 1) {
        gWinHidden := true
        Gui, Main:Hide
...
MenuShowWindow:
    gWinHidden := false
    ; a window hidden WHILE minimized comes back still minimized unless the
    ; minimized state is undone as well
