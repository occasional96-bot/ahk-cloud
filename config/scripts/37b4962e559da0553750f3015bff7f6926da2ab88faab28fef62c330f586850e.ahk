...
;   edit box in each - so Edit1 and Button1 cannot drift. The text is set
;   straight on the box and the button is clicked with a posted message, so
;   it works even if the window is behind something.
;=============================================================================
AUDOS_LookupByShape() {
    part := Trim(Clipboard, " `t`r`n")
...
AUDOS_BydLookup(part) {
    ...
    ControlGet, edit, Hwnd, , Edit1,   ahk_id %hwnd%
    ControlGet, btn,  Hwnd, , Button1, ahk_id %hwnd%
    if (!edit || !btn) {
        MsgBox, 48, AUDOS Switch
            , % "Found the " AUDOS_BYD_TITLE " window but not its part box "
              . "and Look up button."
        return false
    }
...
AUDOS_IsuzuLookup(part) {
    ...
    if (!edit || !btn) {
        MsgBox, 48, AUDOS Switch
            , % "Found the " AUDOS_ISUZU_TITLE " window but not its part box "
              . "and Look up button."
        return false
    }

    ; It parks itself minimised on the taskbar, so Restore is what actually
    ; brings it up - Activate alone would leave it down there.
    WinGet, state, MinMax, ahk_id %hwnd%
    if (state = -1)
        WinRestore, ahk_id %hwnd%
    WinActivate, ahk_id %hwnd%
