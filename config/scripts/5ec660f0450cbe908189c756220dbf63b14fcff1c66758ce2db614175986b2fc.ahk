...
SidebarBuild(visible := true) {
    ...
    Gui, SB:Add, Edit, % "x8 y7 w" (W - 154) " h26 vSbFilter",
    ...
    ; 2.22.0: double height (h18 -> h36). Still the THEMED flat bar - colouring it strips
    ; the visual style permanently - so the percentage stays in the Text below.
    Gui, SB:Add, Progress, % "x8 y+8 w" (W - 16) " h36 vSbProg Range0-100", 0
...
SbLayout(w, h) {
    ...
    ; 2.22.0: no Receipt button, and the bar is 36 tall instead of 18. The 40px the button
    ; and its gap gave back do NOT become a hole - the ListView absorbs every one of them:
    ;   below the LV = 8 + 36 (bar) + 4 + 18 (text) + 8 + 22 (footer) + 8 (margin) = 104
    logH := 110
    logExtra := CfgShowLogPane ? (6 + logH) : 0
    lvH := h - 66 - 104 - logExtra
    if (lvH < 100)
        lvH := 100
    GuiControl, SB:Move, SbLv, % "x" M " y66 w" innerW " h" lvH
    tickW := 34
    progY := 66 + lvH + 8
    GuiControl, SB:Move, SbProg, % "x" M " y" progY " w" (innerW - tickW) " h36"
    GuiControl, SB:Move, SbTick, % "x" (M + innerW - tickW + 4) " y" (progY + 1) " w" (tickW - 4) " h34"
    txtY := progY + 36 + 4
