...
; without this a tooltip is placed relative
...
;
;   Cell width is canvas width divided by columns, cell height is canvas
;   height divided by rows. On this machine right now that is 800 / 80 = 10
;   and 360 / 24 = 15, both exact.
;
;   "Both exact" is the check, not a coincidence. A terminal lays its grid out
;   on whole pixels, so the true column count divides the canvas evenly and a
;   wrong guess does not. That is why the code tries 80 and 132 columns, and
;   24 and 25 rows, and keeps whichever divides cleanly - it is measuring,
;   not assuming.
;
;-----------------------------------------------------------------------------
...
EW_PickWord(mx, my) {
    ...
    WinGetPos, tx, ty, tw, th, ahk_id %term%
    ...
    x1 := tx + (span.c1 * grid.cw) + 2
    x2 := tx + ((span.c2 + 1) * grid.cw) - 2
    y  := ty + (row * grid.ch) + (grid.ch // 2)
...
;   THE GRID
;
;   Work out how big one character cell is, by measuring rather than assuming.
;   Returns { cols, rows, cw, ch } or "" if nothing sensible came out.
;=============================================================================
EW_Grid(term, lines) {
    ; How wide is the canvas, in pixels?
    VarSetCapacity(rc, 16, 0)
    if (!DllCall("GetClientRect", "Ptr", term, "Ptr", &rc))
        return ""
    cw := NumGet(rc, 8, "Int")           ; client width
    chh := NumGet(rc, 12, "Int")         ; client height
    if (cw < 100 || chh < 100)
        return ""
    ...
    return { cols: cols, rows: rows, cw: cw // cols, ch: chh // rows }
}
...
;   does both wins; if none divides evenly we fall back to the first one that
;   is merely big enough, rather than giving up.
;-----------------------------------------------------------------------------
...
EW_EraWindow() {
    ...
    WinGet, list, List, % EW_ERA_TITLE " ahk_exe " EW_ERA_EXE
