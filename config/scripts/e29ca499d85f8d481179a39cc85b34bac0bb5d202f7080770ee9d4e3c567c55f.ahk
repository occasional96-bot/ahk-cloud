...
RC_Fill(text, plate, vin := "", ByRef make := "", st := "") {
    ...
    ; Whenever a rego is in hand, Autobarn's shop gets asked for the exact
    ; build - it is the only free source that knows the trim, so it is worth
    ; the call even when the model and engine are already settled.
    abTrim := ""
    if (plate != "" && st != "") {
    ...
    if (abTrim != "" && !InStr(" " . vinModel . " ", " " . abTrim . " "))
        tag := RC_NiceName(vinModel . " " . abTrim)
    else
