...
global EW_STRIP_SCREEN := "Cross Reference"   ; the heading that says which
                                             ; screen. Every cross reference
                                             ; screen carries the make code -
                                             ; Description, Part Number and
                                             ; BIN alike - so the words they
                                             ; share are what we look for.
...
; The screen usually says which make it is on, in as many words: a "Make"
; field with the code beside it. When it does, that is the make - not a
; guess from position, and not a list of codes anybody has to keep up to
; date. Two letters come off only when they are THOSE two letters.
global EW_STRIP_FIELD_RE := "i)\bMake\b\s*:?\s*([A-Za-z]{2})(?![A-Za-z0-9])"
...
EW_StripMake(lines, row, span) {
    ...
    ; THE MAKE THE SCREEN SAYS IT IS ON
    ;
    ; Counter Sales and its like print the make in a field of its own. When
    ; that field is there, it settles the question outright: the word starts
    ; with those exact two letters or it keeps them. No screen has to be
    ; recognised by name and no column has to be measured, so this works on
    ; any screen and in any field - the New Part Number box included.
    ;
    ; = is a case blind comparison in AutoHotkey, which is what we want here.
    make := EW_ScreenMake(lines)
    if (make != "" && StrLen(span.text) > StrLen(make)
        && SubStr(span.text, 1, StrLen(make)) = make)
        return SubStr(span.text, StrLen(make) + 1)
    ...
    ; A make code is letters. Without this the chop is positional and nothing
    ; else - a part number that is all digits, sitting in the right column on
    ; the right screen, quietly loses two real characters and you would never
    ; see it happen. Letters on the front are the evidence that there is a
    ; make there to remove.
    if (!RegExMatch(SubStr(span.text, 1, EW_STRIP_LEN), "^[A-Za-z]+$"))
        return span.text
...
;-----------------------------------------------------------------------------
;   The make code the screen is showing, or "" if it is not showing one.
;
;   Read off the screen's own Make field rather than matched against a list of
;   codes kept somewhere in here. A list would be one more thing to maintain
;   and would be wrong the day a new franchise arrives; the field is right by
;   definition, because it is what ERA itself is telling you.
;-----------------------------------------------------------------------------
EW_ScreenMake(lines) {
    global EW_STRIP_FIELD_RE

    for i, ln in lines
        if (RegExMatch(ln, EW_STRIP_FIELD_RE, m))
            return m1
