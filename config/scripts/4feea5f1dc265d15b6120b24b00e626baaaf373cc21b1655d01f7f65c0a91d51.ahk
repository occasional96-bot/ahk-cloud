...
RC_EpcHot(on) {
    global RC_EPCHOT, RC_EPC_LBL, RC_EPC_LBL_HOT, RC_EPC_BG, RC_EPC_BG_HOT
    ...
    GuiControl, RC:, RC_EpcBtn, % (on ? RC_EPC_LBL_HOT : RC_EPC_LBL)
    RC_BtnPaint(RC_hEpcBtn)
}
...
RC_TagShow() {
    ...
    if (c.mkX != "")
        tag .= (tag != "" ? ", " : "") . c.mkX
    ; The state's own body word is deliberately NOT in here. A cab chassis is
    ; registered as a UTILITY the same as a tub is, so handing that word to
    ; the tail had every Isuzu ute called a tub on the strength of it. Only
    ; what a shop or the dealer system said gets a say - and when none of them
    ; said, the line stops at the driveline rather than guessing.
