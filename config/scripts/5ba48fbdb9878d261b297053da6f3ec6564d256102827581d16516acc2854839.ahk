...
SidebarFill() {
    ...
    global g_wlDates, g_wlExGst, g_wlPoLines, g_wlInvLines, GLYPH_OK, GLYPH_WARN, CfgHideWarnRows
...
SbCustomDraw(wParam, lParam, msg, hwnd) {
    ...
    ; the deleted Receipted column used to say in text. clrText lives at +80 in
    ; NMLVCUSTOMDRAW (64-bit) and is written on EVERY subitem, hit or miss, for the same
    ; reason the font is: the struct is reused, so an unset colour is a stale colour.
    NumPut((col = COL_ST && idx && SbIdxReceipted(idx)) ? GREEN : SbLvTextColour()
         , lParam + 0, 80, "UInt")
...
ReadConfig() {
    ...
    global CfgHideOrder, CfgHideMake, CfgHideDate, CfgHideExGst, CfgHidePoInv, CfgHideWarnRows
