...
;
;  Ctrl+Alt+J (1.29.0): the same three cells as a floating strip AT THE MOUSE.
;  Closes on: Ctrl+Alt+J again, Esc, any cell click, or clicking anywhere else
;  (focus loss). Right-click on it = DockMenu (Settings etc). See CurStrip*.
;  1.30.0: the TITLEBAR strip is retired - Ctrl+Alt+J is the only strip. The Dock
;  gui is still built and TrackWindow still ticks (trackedHwnd feeds toasts), but
;  an early return in TrackWindow keeps it hidden forever.
; ============================================================================
#NoEnv
...
global g_csMenuOn := false      ; StMenu is up - watcher must not hide the strip under it
global g_csGrace := 0           ; Show->activation lag counter, see CurStripWatch
global csH := 28
Gui, CurStrip:New, +ToolWindow -Caption +AlwaysOnTop +HwndCurStripHwnd
Gui, CurStrip:Color, 0F766E
Gui, CurStrip:Add, Progress, x%w1% y0 w1 h%csH% -Theme c0B5850 Background0B5850, 100
Gui, CurStrip:Add, Progress, x%dx2% y0 w1 h%csH% -Theme c0B5850 Background0B5850, 100
Gui, CurStrip:Font, s9 Bold cWhite, Segoe UI
Gui, CurStrip:Add, Text, x0 y0 w%w1% h%csH% Center 0x200 BackgroundTrans gCurStripWl, Receipting
Gui, CurStrip:Add, Text, x%x2% y0 w%w2% h%csH% Center 0x200 0x80 BackgroundTrans gCurStripSnd, BYD & Isuzu Despatch
Gui, CurStrip:Add, Text, x%x6% y0 w%w6% h%csH% Center 0x200 BackgroundTrans gCurStripDot vCsDot c9CA3AF, % Chr(0x25CF)
...
; ---- Ctrl+Alt+J: the cursor strip (1.29.0) ---------------------------------
WheelRight::CurStripToggle()

CurStripToggle() {
    global CurStripHwnd, guiW, csH, g_csGrace
    ...
    MouseGetPos, mx, my
    x := mx - 10, y := my - csH // 2        ; cursor lands just inside the first cell
    CurStripClamp(x, y)
    g_csGrace := 0
    ; Shown ACTIVATED on purpose - focus loss is the close signal. (The dock shows NA
    ; and never takes focus; this strip NEEDS focus or "click elsewhere to close"
    ; has nothing to watch.)
    Gui, CurStrip:Show, x%x% y%y% w%guiW% h%csH%
...
CurStripClamp(ByRef x, ByRef y) {
    ...
    if (hMon && DllCall("GetMonitorInfo", "Ptr", hMon, "Ptr", &mi)) {
        ...
        x := (x + guiW > wr) ? wr - guiW : x,  x := (x < wl) ? wl : x
