...
global EW_POST_DOWN_MS := 15  ; pause after the button-down that anchors the
                              ; drag, before the walk across the word starts.
...
EW_PickWord(mx, my) {
    ...
    if (pad < 1)
        pad := 1
    x1 := tx + (span.c1 * tw) // grid.cols + pad
    ...
    if (EW_DRAG_POST)
    {
        ...
        ; State the anchor again, as a move, and give it a beat before the
        ; walk sets off. If ERA is still digesting your own click when the
        ; button-down lands, the down can be swallowed and the anchor taken
        ; from wherever ERA thought the pointer was - the middle of the cell,
        ; which snaps forward, and the first character misses out on the
        ; highlight. A move at the same spot pins the anchor to this cell,
        ; and the pause lets it be read before anything else arrives.
