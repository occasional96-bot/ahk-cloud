...
; ---- dock globals ----------------------------------------------------------
global DOCK_VERSION := "1.21.3"  ; 1.21.3: receipting no longer resizes the ERA window (only grows a sub-640x400 one) | 1.21.2: same-day re-exports upload when the local file grew (row-count tie-break) | 1.21.1: no more "? not checked yet" - check runs before the boot import and on first dot click | 1.21.0: Full refresh runs the invoice CSVs in a child process, two-lane progress panel | 1.20.0: Full refresh + grouped menu, no passwords in the one-click run | 1.19.1: HY defaults to recv-aLL under auto | 1.19.0: "Export + upload PO data" - ERA drives the export, then the normal PO upload + import
...
; Where the KI + HY invoice feeds live on THIS machine (1.20.0).
;
; C:\Temp is the norm and stays first, but the dock runs on several shared machines and the
; feed does not always land there - so each file falls back to the OneDrive copy under
; whoever is logged in. A_UserName, never a hard-coded profile: kainet on the work box,
; kaine on the dev box, joshuar on Josh's, and his is not a OneDrive setup at all, which is
; exactly why the plain C:\Temp entry has to stay first rather than be replaced.
;
; Returns one path per feed - the first candidate that exists. If none exists the C:\Temp
...
global g_reconSent  := []  ; 1.21.3: {path, brand} per fan-out post so the batch end can
                           ; verify each landed on recon and retry any that vanished
...
global GLYPH_WARN := Chr(0x26A0)   ; warning triangle (sidebar: parts missing / order not in export)
global HINT_IDLE  := ""
global HINT_MORE  := ""
...
SendOne(path, brand) {
    global GLYPH_OK, GLYPH_BAD, g_lastId, g_lastLines, g_sndPath
    SplitPath, path, fname
    if (!FileExist(path)) {
    ...
    g_sndPath := path        ; 1.21.3: lets ReconFire record which file this post carries
    ReconFire(b64, brand)
...
ReconBatchBegin() {
    global ReconUrl, g_batchId, g_reconCount, g_reconReqs, g_reconSent
    g_reconSent := []
...
; 1.21.3: did every fan-out post actually land? Recon keys invoices by the id the PARSER
; assigned, which the client does not know per-post - but the filename is the invoice number
; for every PDF this sender handles (I0973284.pdf), so the check keys on that. A miss is
; re-posted once, synchronously, from bytes re-read off disk.
ReconVerifyBatch() {
    global ReconUrl, g_reconSent
    if (g_reconSent.Length() = 0)
        return
    chk := HttpGetPumped(ReconUrl "/invoices?limit=100", 15)
    if (chk.status != 200) {
        SndLog("recon: verify skipped - /invoices unreachable (http=" chk.status ")")
        g_reconSent := []
        return
    }
    for i, ent in g_reconSent {
        entPath := ent.path
        SplitPath, entPath, , , , invId         ; I0973284.pdf -> I0973284 (SplitPath takes a
                                                ; VAR name - a forced expression is a v1 trap)
        if (invId = "" || InStr(chk.text, """" invId """"))
            continue
        SndLog("recon: " invId " missing after the batch - re-posting synchronously")
        b64 := Base64FromFile(ent.path)
        if (b64 = "") {
            SndLog("recon: " invId " retry ABANDONED - could not re-read " ent.path)
            continue
        }
        rr := HttpPostJson(ReconUrl "/parse-invoice", "{""dataBase64"":""" b64 """}", false, "application/json", 15000, 2)
        if (rr.status = 200)
            SndLog("recon: " invId " retry OK")
        else
            SndLog("recon: " invId " retry FAILED http=" rr.status " - invoice is NOT on the worklist | " SubStr(rr.text, 1, 150))
    }
    g_reconSent := []
}
...
ReconFire(b64, brand) {
    global ReconUrl, g_batchId, g_reconCount, g_reconReqs, CfgFanOut, g_reconSent, g_sndPath
    ...
    g_reconCount += 1
    ; 1.21.3: remember WHAT went up, not the bytes - a retry re-reads the PDF from disk
    ; rather than holding every base64 in memory across a bulk send.
    g_reconSent.Push({path: g_sndPath, brand: brand})
}
...
ReconBatchEnd() {
    ...
    ; 1.21.3 (Kaine, 2026-07-22: I0973284 said "Sent" but never reached the worklist): the
    ; fan-out posts are fire-and-forget, so one can die without a trace - e.g. /flags's own
    ; single 3s attempt failing used to return HERE, releasing an in-flight post with it.
    ; Verify-and-retry closes that hole client-side, and it runs on the FAILURE path too:
    ; a dead /flags is precisely when the batch most needs re-checking. Each miss gets one
    ; synchronous re-post (bytes re-read from disk); still missing after that -> logged
    ; loudly, because that is a server-side loss the client cannot fix.
    ReconVerifyBatch()
    if (res.status != 200) {
...
; 2.18.0: toast dwell is a Settings > Service field (ToastSeconds, default 2.5).
...
global CfgSubtractBy     := "x"   ; "x" = line/x/Enter (video-proven stream, never run live)
                                  ; "zb" = line/0/B/Enter (ordinary qty path, UNPROVEN on ERA)
...
global CfgToastSecs      := 2.5   ; how long the corner toast stays up (2.18.0, Settings >
...
global CfgHideReceipted  := 0
global CfgHideWarnRows   := 0     ; worklist footer toggle: hide the warn (check-parts) rows
        ...
        if (g_sbVisible) {
            ...
            if (SidebarFetch(false)) {
                if (!g_sbBuilt)
                    SidebarBuild(false)
                SidebarFill()
                GuiControl, SB:, SbFilter, % "|" SidebarComboList()
            }
        }
    }
}
...
; Polls the lookup box while the sidebar is up. A ComboBox g-label only fires reliably on
; SELECTION; catching typing needs CBN_EDITCHANGE plumbing - a 250ms poll is simpler and
; costs nothing. Refills only when the text actually changed.
SbFilterTick:
...
SidebarShow() {
    ...
    if (g_sbBuilt) {
        ...
        if (g_wlFetchedAt) {
            ...
            if (ok) {
                SidebarFill()
                GuiControl, SB:, SbFilter, % "|" SidebarComboList()   ; combo was built before data existed
            }
        }
    }
...
; geometry SidebarBuild computes, minus the build. The w/h on Show fires SBGuiSize, which
; runs SbLayout, so every control re-fits without calling it here.
SbReposition() {
    global VERSION
    W := 680, h := 640, x := "", y := ""
    SetTitleMatchMode, 2
...
SidebarFetch(quick) {
    global ReconUrl, g_wlIds, g_wlOrders, g_wlGlyphs, g_wlTips, GLYPH_OK, GLYPH_WARN
    ...
    Loop, Parse, block, `n
    {
        ...
        g_wlIds.Push(f[1])
        g_wlOrders.Push(f[2])
        g_wlGlyphs.Push(f[3] = "ready" ? GLYPH_OK : GLYPH_WARN)
...
SidebarBuild(visible := true) {
    ...
    global SbFilter, SbLv, SbFooter   ; Gui control output vars in a FUNCTION must be global
    global SbProg, SbProgTxt, SbBtnReceipt, SbBtnSettings, SbLogPane
    global SbBtnRefresh, SbBtnSetup, SbHideWarn, SbTick
    global SbBrAll, SbBr1, SbBr2, SbBr3, SbBr4   ; brand strip radios
    Gui, SB:Destroy

    W := 680                    ; wide enough that all 8 columns show FULLY (no truncation, no
                                ; h-scroll). 616 + 62 for Make + 2 - hide Make in Settings to
                                ; get the old width back.
    h := 640, x := "", y := ""
    SetTitleMatchMode, 2
    if WinExist("ERA Port") {
        WinGetPos, ex, ey, ew, eh
        h := eh - 46            ; window -> client height, roughly (caption + borders)
        if (h < 300)
            h := 300
        x := ex - (W + 8)
        if (x < 0)
    ...
    lvH := h - 186 - logH       ; filter row + brand strip + progress + % text + Receipt button + footer + margins
    if (lvH < 120)
        lvH := 120

    Gui, SB:+Resize +MinSize440x320 +LabelSBGui +Hwndg_sbWinHwnd
    Gui, SB:Font, s10, Segoe UI
    Gui, SB:Add, ComboBox, % "x8 y8 w" (W - 130) " vSbFilter", % SidebarComboList()
    ...
    Gui, SB:Add, ListView, % "x8 y66 w" (W - 16) " h" lvH " -Multi NoSortHdr NoSort Hwndg_sbLvHwnd vSbLv gSbLvEvent AltSubmit", % "St|Invoice|Make|Order|Date|Ex GST|Po/Inv|Receipted"
    Gui, SB:Default
    SbApplyCols(W - 16)
    Gui, SB:Font, s9, Segoe UI
    ...
    Gui, SB:Add, Text, % "x8 y+4 w" (W - 16) " vSbProgTxt", Idle
    Gui, SB:Font, s10, Segoe UI
    ; Receipt names the selected order so it can't fire the wrong row; double-click still works.
    ; Qty mode + Dry run now live in Settings; the sidebar is just the worklist + the run.
    Gui, SB:Add, Button, % "x8 y+8 w" (W - 16) " h32 vSbBtnReceipt gSbReceiptBtn", Receipt (select a row)
    Gui, SB:Font, s9, Segoe UI
    Gui, SB:Add, Text, % "x8 y+8 w" (W - 16) " vSbFooter", % GLYPH_OK " 0 ready"
    if (CfgShowLogPane) {
        Gui, SB:Font, s8, Consolas
        Gui, SB:Add, Edit, % "x8 y+6 w" (W - 16) " h" logH " vSbLogPane ReadOnly -Wrap +HScroll", (activity log)
        Gui, SB:Font, s9, Segoe UI
    }
    ; Bottom-right footer toggle: hide the warn (check-parts) rows. Added AFTER the flow
    ; controls (its own position is set by SbLayout, so where it lands here doesn't matter).
    Gui, SB:Font, s9, Segoe UI
    Gui, SB:Add, CheckBox, % "x8 y8 w150 vSbHideWarn gSbHideWarnToggle Checked" CfgHideWarnRows, % "Hide " GLYPH_WARN " rows"
    ...
    SbLayout(W, h)   ; normalise every control's geometry (also the single source of layout truth on resize)
    g_sbBuilt := 1
}
...
SbApplyCols(lvClientW) {
    global CfgHideOrder, CfgHideMake, CfgHideDate, CfgHideExGst, CfgHidePoInv, CfgHideReceipted
    global g_sbLvHwnd
    Gui, SB:Default
    Gui, SB:ListView, SbLv
    hide := [0, 0, CfgHideMake, CfgHideOrder, CfgHideDate, CfgHideExGst, CfgHidePoInv, CfgHideReceipted]
    ...
    ; Too narrow for the content: scale every visible column down so the grid fits the
    ; list exactly. 25 = vertical scrollbar (17) + client insets.
    avail := lvClientW - 25
...
;   bottom:   progress bar (+ big done-tick capping its right end), % text, Receipt button,
;             then the footer counts on the left with the Hide-warn toggle in the far corner,
;             then the optional log pane
SbLayout(w, h) {
    ...
    if (innerW < 220)
        innerW := 220
    ; --- top: filter + Refresh + Setup (buttons hug the right edge) ---
    setX := w - M - 48
    refX := setX - 4 - 56
    GuiControl, SB:Move, SbBtnSetup,   % "x" setX " y7 w48 h26"
    GuiControl, SB:Move, SbBtnRefresh, % "x" refX " y7 w56 h26"
    GuiControl, SB:Move, SbFilter,     % "x" M " y8 w" (refX - M - 6)
    ...
    GuiControl, SB:Move, SbTick, % "x" (M + innerW - tickW + 4) " y" (progY - 8) " w" (tickW - 4) " h34"
    txtY := progY + 18 + 4
    GuiControl, SB:Move, SbProgTxt, % "x" M " y" txtY " w" innerW " h18"
    btnY := txtY + 18 + 8
    GuiControl, SB:Move, SbBtnReceipt, % "x" M " y" btnY " w" innerW " h32"
    footY := btnY + 32 + 8
    togW := 128
    GuiControl, SB:Move, SbFooter,   % "x" M " y" (footY + 2) " w" (innerW - togW - 6) " h20"
    GuiControl, SB:Move, SbHideWarn, % "x" (M + innerW - togW) " y" footY " w" togW " h22"
    if (CfgShowLogPane) {
        logY := footY + 22 + 6
        GuiControl, SB:Move, SbLogPane, % "x" M " y" logY " w" innerW " h" logH
    }
    SbApplyCols(innerW)           ; re-flex the columns to the new ListView width
    GuiControl, SB:Move, SbFilter, % "x" M " y8 w" (refX - M - 6)   ; combo redraw after moves
...
; Snap the window's WIDTH to what the columns actually need (2.20.0, Kaine: hiding a
; column should make "the whole GUI resize to be more form fitting"). Called from the
; Settings save path only - a live drag stays the user's own business. Height is kept;
; the Show fires SBGuiSize, which re-runs SbLayout against the new width.
SbFitWidth() {
    global g_sbLvHwnd, g_sbWinHwnd, g_sbVisible
    if (!g_sbVisible || !g_sbLvHwnd)
        return
    total := 0
    Loop, 8
    {
    ...
    if (total < 100)
        return
    W := total + 25 + 16          ; column total + LV scrollbar/insets + window margins
    if (W < 440)                  ; the Gui's own MinSize floor
        W := 440
    VarSetCapacity(rcFit, 16, 0)
    DllCall("GetClientRect", "Ptr", g_sbWinHwnd, "Ptr", &rcFit)
    ch := NumGet(rcFit, 12, "Int")
    if (ch < 320)
        ch := 320
    Gui, SB:Show, % "w" W " h" ch " NoActivate"
}
...
SidebarFill() {
    ...
    global g_wlDates, g_wlExGst, g_wlPoLines, g_wlInvLines, g_rcpDone, GLYPH_OK, GLYPH_WARN, CfgHideWarnRows
    ...
    ; Counts tally EVERY filtered row (ready + warn + receipted), whether or not the warn
    ; rows are hidden - so the footer never lies about what the toggle is hiding.
    ready := 0
    warn := 0
    recq := 0
    for k, idx in vis {
        isWarn := (g_wlGlyphs[idx] != GLYPH_OK)
        done := g_rcpDone.HasKey(g_wlIds[idx]) ? g_rcpDone[g_wlIds[idx]] : ""
        if (isWarn)
            warn++
        else
            ready++
        if (done != "")
            recq++
        if (CfgHideWarnRows && isWarn)   ; counted above, just not drawn
            continue
        ...
        row := LV_Add("", g_wlGlyphs[idx], g_wlIds[idx], SbMakeLabel(g_wlMakes[idx]), g_wlOrders[idx]
                    , g_wlDates[idx], g_wlExGst[idx], poInv, done)
    ...
    SbApplyCols(sbLvPW)
    ; The counts below describe what the brand filter LET THROUGH, so say which brand that
    ; was - a footer reading "3 ready" with no context is the kind of half-truth that gets
    ; a run fired on the wrong list.
    hid := (CfgHideWarnRows && warn) ? "   (" warn " hidden)" : ""
    if (g_sbBrand != "")
        hid .= "   [" SbMakeLabel(g_sbBrand) " only]"
    GuiControl, SB:, SbFooter, % GLYPH_OK " " ready " ready      " GLYPH_WARN " " warn " check parts      " recq " receipted" hid
    SbSelChanged()   ; no selection survives a refill - reset the Receipt button caption
}

; Does worklist index a sort strictly before index b?  ready > warn; newer date first;
; invoice number desc breaks the tie.
SbOrderBefore(a, b) {
    global g_wlGlyphs, g_wlDates, g_wlIds, GLYPH_OK
    ra := (g_wlGlyphs[a] = GLYPH_OK ? 0 : 1)
    rb := (g_wlGlyphs[b] = GLYPH_OK ? 0 : 1)
    if (ra != rb)
        return ra < rb
...
; Selection -> the Receipt button names the order it would fire (a warn row gets "Why?").
SbSelChanged() {
    global g_wlRowIdx, g_wlOrders, g_wlGlyphs, GLYPH_OK
    Gui, SB:Default
    row := LV_GetNext()
    idx := row ? g_wlRowIdx[row] : 0
    if (!idx) {
        GuiControl, SB:, SbBtnReceipt, Receipt (select a row)
        return
    }
    if (g_wlGlyphs[idx] = GLYPH_OK)
        GuiControl, SB:, SbBtnReceipt, % "Receipt " g_wlOrders[idx]
    else
        GuiControl, SB:, SbBtnReceipt, % "Why not ready?"
}

; Double-click a worklist row -> click-to-receipt. Selection -> re-caption the Receipt
; button (AltSubmit: "I" = item changed). Both paths still work - the button is an
; ADDITION, not a replacement for the proven double-click.
SbLvEvent:
...
; ---- bold the total of the invoice you last ran -----------------------------------
...
SbCustomDraw(wParam, lParam, msg, hwnd) {
    global g_sbLvHwnd, g_wlRowIdx, g_sbBoldIdx
    ...
    static COL_EXGST := 5                 ; 0-based: St|Invoice|Make|Order|Date|Ex GST|...
    if (!g_sbLvHwnd || !g_sbBoldIdx)
        return                            ; rule 1
    ...
    hit := (NumGet(lParam + 0, 88, "Int") = COL_EXGST) && (g_wlRowIdx[row] = g_sbBoldIdx)
...
; The Receipt button: same entry as double-click, on the selected row. A warn row goes
; through RcpStart too - it shows the WHY dialog and refuses, which is the button's answer.
SbReceiptBtn:
    Gui, SB:Default
    sbRow := LV_GetNext()
    if (!sbRow) {
        ShowToast("bad", "Select a row first", "Click an invoice, then Receipt")
        return
    }
    RcpStart(sbRow)
return
...
SettingsShow:
    ...
    Gui, Set:Font, s8 cGray
    Gui, Set:Add, Text, xs w430, How recv-aLL un-receives the lines the invoice doesn't ship. X is the video-proven stream; 0+B reuses the ordinary qty path but leaves a DIFFERENT footprint (0 receipt + booked backorder). NEITHER has run on real ERA yet - the checkpoint text follows this choice.
    Gui, Set:Font, s9 cDefault
    ...
    Gui, Set:Font, s8 cGray
    Gui, Set:Add, Text, xs w430, How long the corner pop-up stays before fading. Decimals fine (default 2.5). "Working..." toasts ignore this - they sit until their result replaces them.
    Gui, Set:Font, s9 cDefault
    ...
    setShExGst := !CfgHideExGst, setShPoInv := !CfgHidePoInv, setShRecd := !CfgHideReceipted
    Gui, Set:Tab, 5
    ...
    Gui, Set:Add, CheckBox, xs vSetCbColRecd Checked%setShRecd%, Receipted
    Gui, Set:Font, s8 cGray
    Gui, Set:Add, Text, xs w430, The last column still showing stretches to fill the list, so hiding one never leaves a gap on the right. Turning Make off does NOT turn the brand strip off - the strip is a filter, the column is a label.
    Gui, Set:Font, s9 cDefault
...
SetSave:
    ...
    if (g_sbVisible) {
        Gui, SB:-Disabled
        SidebarBuild()            ; column hides / log pane may have changed - re-fit the window
        SidebarFill()
        SbFitWidth()              ; 2.20.0: snap the window width to the surviving columns
    }
return
...
RcpStart(row) {
    global g_wlRowIdx, g_wlIds, g_wlGlyphs, g_wlTips, GLYPH_OK, g_wlExGst
    ...
    if (g_wlGlyphs[idx] != GLYPH_OK) {
        ; A warning row can never be receipted - show WHY instead (same text as the tooltip).
        Gui, SB:+OwnDialogs
...
RcpLaunch(id, exRaw, attempt) {
    ...
    if (note)
        msg .= "`n" note
    if (plan.mode = "recvall" && plan.uncovered.Length() > 0) {
        subDesc := (CfgSubtractBy = "zb" ? "receive 0 + backorder (line/0/B - UNPROVEN)" : "X out (line/x)")
        msg .= "`n`nrecv-aLL will receipt ALL " plan.poLines " line(s), then work IN LINE ORDER, subtracting by " subDesc ":`n" RcpUncoveredNote(plan)
    }
    if (overN)
...
;   per-line: every plan line (line# / qty / [B] / price).
...
; E-BYDAU26061461 (1 part on DO35874's 8 open lines): per-line 1 vs recv-aLL 1 + 7 + 1 = 9
; -> per-line. That invoice is the regression guard: it must still resolve per-line, but now
; because it is CHEAPER, not because recv-aLL is forbidden. If it ever picks recv-aLL the
; cost model is inverted.
RcpCostPerLine(plan) {
...
RcpRun(plan) {
    ...
    steps := 1 + (plan.mode = "recvall" ? 1 + plan.uncovered.Length() + RcpActionableRecvAll(plan) : plan.lines.Length())
    RcpProgBegin(steps)
    ok := false
    if (RcpHeader(plan)) {
        RcpProgStep()
    ...
    if (ok) {
        ...
        if (g_sbVisible) {
            if (clean) {
                ...
                GuiControl, SB:, SbProgTxt, % GLYPH_OK " " (g_rcpDry ? "Dry run complete - nothing was sent to ERA" : "Done - " plan.order " typed. Review + finalize on ERA.") "   " RcpElapsed() " / " g_rcpStepsDone " steps"
            } else {
...
RcpHomePage() {
    ...
    ; 1.21.3 (Kaine, 2026-07-22: "stop it from resizing my era screen"): the hard 820x502
    ; came over with the Speed_Receipting port, but EraScreen() reads TEXT off the clipboard
    ; (F11 -> EditSelectWindow -> EditCopy) - the terminal buffer, not pixels - so the
    ; capture never needed a fixed window size. Only grow, and only if the window is
    ; genuinely tiny (below the ~80x24 terminal at a readable font); a maximized or
    ; hand-sized window is left exactly where Kaine put it.
    WinGetPos, , , rhW, rhH, ahk_id %g_rcpHwnd%
    if (rhW < 640 || rhH < 400)
        WinMove, ahk_id %g_rcpHwnd%, , , , % (rhW < 640 ? 820 : rhW), % (rhH < 400 ? 502 : rhH)
    Loop, 13
...
RcpWorkItems(plan) {
    ...
    if (plan.mode != "recvall") {
        ; Per-line mode types the qty on EVERY line - that typing IS the receipt, so no line
        ; can be skipped here (priceOnly stays 0; the accurate-price skip is a recv-aLL saving).
        for i, l in plan.lines
    ...
    for i, l in plan.lines {
        ...
        if (RcpIsUncovered(plan, l.line))
            continue
        ; priceOnly = the ONLY reason to visit is the (stale-CSV) chg flag. The work loop may
        ; drop such a visit when a paid-for capture already showed the Exc equal to the
...
ReadConfig() {
    ...
    global CfgHideOrder, CfgHideMake, CfgHideDate, CfgHideExGst, CfgHidePoInv, CfgHideReceipted, CfgHideWarnRows
    ...
    IniRead, CfgSubtractBy,      %ini%, settings, SubtractBy, x
    if (CfgSubtractBy != "zb")
        CfgSubtractBy := "x"
    ...
    IniRead, CfgToastSecs,       %ini%, settings, ToastSeconds, 2.5
    ...
    IniRead, CfgTimingPreset,    %ini%, settings, TimingPreset, normal
    ...
    IniRead, CfgHideReceipted,   %ini%, settings, HideColReceipted, 0
    IniRead, CfgHideWarnRows,    %ini%, settings, HideWarnRows, 0
    ; Timing (ms). The six REAL waits the run uses - stored so a slow ERA day is tunable
    ; without editing source. Defaults are today's proven values.
    IniRead, RCP_AFTER_MENU,     %ini%, settings, WaitAfterMenu, 250
    IniRead, RCP_AFTER_PO,       %ini%, settings, WaitAfterPo, 400
    IniRead, RCP_AFTER_INV,      %ini%, settings, WaitAfterInv, 0
    IniRead, RCP_BETWEEN_LINES,  %ini%, settings, WaitBetweenLines, 120
    IniRead, RCP_TEXT_SETTLE,    %ini%, settings, WaitTextSettle, 200
    IniRead, RCP_VAR_SETTLE,     %ini%, settings, WaitVarSettle, 150
...
WriteConfig() {
    ...
    IniWrite, %CfgHideReceipted%,   %ini%, settings, HideColReceipted
