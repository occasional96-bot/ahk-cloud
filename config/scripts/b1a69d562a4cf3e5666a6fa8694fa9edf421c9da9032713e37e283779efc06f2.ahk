...
Menu, Tray, Tip, % "AUDOS Switch"
                 . "`nCtrl+Left click = part lookup, BYD / Isuzu / Kia by number"
...
;       2. puts DS007 in the screen box on the bottom bar, reads it back to
;          make sure all five characters are there, then presses Enter
;          (or skips this when the window is already on Part Master)
;       3. waits for Part Master to open
;       4. puts the clipboard into the part field, checks that too, Enter
;
;-----------------------------------------------------------------------------
...
AUDOS_PartLookup(brand) {
    ...
    steps := ""
    err   := AUDOS_PartLookupDo(hwnd, part, steps)

    DetectHiddenWindows, %previous%

    ; One line per lookup in AUDOS_Switch.log, the same as the BYD and Isuzu
    ; hand-offs, so a "nothing happened" can be read back afterwards.
    AUDOS_HandLog(brand, (part = "" ? "(no part)" : part), hwnd
                , (err = "" ? "ok, " : "FAILED, ") steps (err = "" ? "" : " - " err))

    if (err != "")
        MsgBox, 48, AUDOS Switch, %err%
...
;-----------------------------------------------------------------------------
;   The lookup itself. Returns "" when it all landed, or the words for the
;   error box. `steps` comes back with what happened at each stage, for the
;   log.
;
;   The screen box gets DS007 by ControlSetText (WM_SETTEXT), never by typing
;   it. Typed keystrokes went in one at a time, and every so often AUDOS was
;   busy for one of them, so the box read "DS07" or "DS00" when Enter arrived
;   and the wrong screen opened, or none. WM_SETTEXT puts the whole word in
;   at once, and the box is read straight back to prove it holds exactly
;   DS007 before Enter is pressed. If it does not, it is set again; if the
;   box itself has gone (AUDOS rebuilds the bar on some screen changes) it is
;   found again; and if the box will not take text that way at all, the old
;   keystroke path is the last resort, with the same read-back before Enter.
;
;   Enter is not taken on trust either. If Part Master is not up three
;   seconds after the first Enter, a second Enter goes in (the box still
;   holds DS007, so the worst case is the same screen opening again); after
;   that the keystroke path has a go; and only then is it given up on.
;
;   And when the window is already sitting on Part Master, none of that
;   runs - straight to the part field.
;-----------------------------------------------------------------------------
AUDOS_PartLookupDo(hwnd, part, ByRef steps) {
    global AUDOS_PART_SCREEN, AUDOS_PART_TITLE, AUDOS_PART_WAIT_MS
    t0 := A_TickCount

    ;--- 1. get to Part Master ---------------------------------------------
    WinGetTitle, title, ahk_id %hwnd%
    if InStr(title, AUDOS_PART_TITLE) {
        steps .= AUDOS_PART_TITLE " already up"
    } else {
        box := AUDOS_BottomBox(hwnd)
        if (!box) {
            steps .= "no screen box"
            return "Could not find the screen box on the bottom bar."
        }
        ControlFocus, , ahk_id %box%

        ; The screen code, and proof it landed. A miss after three tries
        ; means the handle may be stale - find the box again and try once
        ; more before falling back to keystrokes.
        landed := AUDOS_SetBoxText(box, AUDOS_PART_SCREEN, 3)
        if (!landed) {
            box2 := AUDOS_BottomBox(hwnd)
            if (box2 && box2 != box) {
                box    := box2
                landed := AUDOS_SetBoxText(box, AUDOS_PART_SCREEN, 3)
                if (landed)
                    landed := "refound box, try " landed
            }
        }
        if (landed) {
            steps .= AUDOS_PART_SCREEN " set, landed try " landed
            ; Enter, and a second Enter if the first did not take.
            ControlSend, , {Enter}, ahk_id %box%
            opened := AUDOS_WaitTitle(hwnd, AUDOS_PART_TITLE, AUDOS_PART_WAIT_MS * 0.3)
            if (!opened) {
                ControlSend, , {Enter}, ahk_id %box%
                opened := AUDOS_WaitTitle(hwnd, AUDOS_PART_TITLE, AUDOS_PART_WAIT_MS * 0.3)
                steps .= (opened ? ", second Enter" : ", two Enters ignored")
            }
        } else {
            ControlGetText, got, , ahk_id %box%
            steps .= AUDOS_PART_SCREEN " would not set, box holds [" got "]"
            opened := false
        }

        ; Last resort: type it the way a person would, still checking the
        ; box before Enter.
        if (!opened) {
            if (AUDOS_TypeBoxText(hwnd, box, AUDOS_PART_SCREEN)) {
                SendInput, {Enter}
                opened := AUDOS_WaitTitle(hwnd, AUDOS_PART_TITLE, AUDOS_PART_WAIT_MS * 0.4)
                steps .= ", typed" (opened ? " and opened" : ", still not open")
            } else {
                steps .= ", typing did not land either"
            }
        }
        if (!opened)
            return "The " AUDOS_PART_TITLE " screen did not open."
        steps .= ", opened at " (A_TickCount - t0) " ms"
    }
    ...
    if (!fld) {
        steps .= ", no part field"
        return "Found the " AUDOS_PART_TITLE " screen but not the part field."
    }
    ...
    if (part = "") {
        ...
        steps .= ", no part on clipboard, field focused"
        return ""
    }

    ; The part number, checked the same way as the screen code.
    landed := AUDOS_SetBoxText(fld, part, 3)
    if (!landed) {
        fld2 := AUDOS_PartField(hwnd)
        if (fld2 && fld2 != fld) {
            fld    := fld2
            landed := AUDOS_SetBoxText(fld, part, 3)
            if (landed)
                landed := "refound field, try " landed
        }
    }
    if (!landed && AUDOS_TypeBoxText(hwnd, fld, part))
        landed := "typed"
    if (!landed) {
        ControlGetText, got, , ahk_id %fld%
        steps .= ", part would not set, field holds [" got "]"
        return "Could not put " part " into the part field."
    }
    ControlSend, , {Enter}, ahk_id %fld%
    steps .= ", part landed try " landed ", Enter at " (A_TickCount - t0) " ms"
    return ""
}

;-----------------------------------------------------------------------------
;   AUDOS_SetBoxText(box, "DS007")  ->  the try it landed on, or 0
;
;   Sets the text, reads it straight back, and only believes the read-back.
;   Up to `tries` goes, 50 ms apart.
;-----------------------------------------------------------------------------
AUDOS_SetBoxText(box, text, tries := 3) {
    Loop, %tries%
    {
...
;-----------------------------------------------------------------------------
;   AUDOS_TypeBoxText(hwnd, box, "DS007")  ->  true if the box then holds it
;
;   The keystroke way: focus the box, make sure it really has the keyboard,
;   clear it and type. Then read it back like everything else. Typing goes
;   through AUDOS exactly as your own does, which is why it stays as the
;   fallback - but it is the path that dropped letters, so it comes last.
;-----------------------------------------------------------------------------
AUDOS_TypeBoxText(hwnd, box, text) {
    ControlFocus, , ahk_id %box%
    Sleep, 60
    if (AUDOS_FocusedHwnd(hwnd) != box)
        return false
    SendInput, {End}{BS 30}%text%
    Loop, 10                        ; up to a second for the keys to arrive
    {
        Sleep, 100
        ControlGetText, got, , ahk_id %box%
        if (got = text)
