...
; ---------- element lookups (AutomationId first, Name fallback) ----------
FindReset(root) {
    btn := ""
    try btn := root.FindFirstBy("AutomationId=button-1107")
    if !btn
...
FindQuery(root) {
    btn := ""
    try btn := root.FindFirstBy("AutomationId=button-1106")
    if !btn
...
DoResetPasteQuery(clickReset, fast) {
    ...
    if needReset {
        ...
        ; guard against the late wipe: field must STAY empty 300ms
        Loop, 3 {
            Sleep, 100
            if (GetFieldValue(root) != "") {
                ; wipe landed late - wait for it to finish
                Loop, 20 {
                    Sleep, 100
                    if (GetFieldValue(root) = "")
                        break
...
; Mouse-free Query: Enter in the field (ExtJS submits on Enter), verified by
; results loading; then focused-button Enter; Invoke and real click as backups
FireQuery(root) {
    ; 1) Enter while the field has focus - standard ExtJS form submit
    fld := FindField(root)
    if fld {
        try {
            fld.SetFocus()
    ...
    ; 2) focus the button itself, press Enter (real keyboard event)
    qbtn := FindQuery(root)
    if qbtn {
        try {
            qbtn.SetFocus()
        ...
        ; 4) last resort: physical click, cursor restored
        try {
