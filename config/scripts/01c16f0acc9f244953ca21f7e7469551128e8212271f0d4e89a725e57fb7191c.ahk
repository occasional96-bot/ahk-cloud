...
RC_BuildGui() {
    ...
    Gui, Add, Text, x230 y74 w138 h22 Right vRC_Make, %A_Space%
    Gui, Font, s9 w400, Segoe UI
    Gui, Add, Text, x80 y100 w288 h16 Right Hidden vRC_MakeX, %A_Space%
    ...
    Gui, Show, Hide w380 h426, Rego check
}
...
RC_Fill(text, plate, vin := "", ByRef make := "", st := "") {
    ...
    if (vin != "" && InStr(make, "KIA")) {
        ...
        ; character.
        GuiControl, RC:, RC_Status, Asking Hyundai about this VIN ...
        hX := ""
        hTrim := ""
    ...
    ; talking: JAX Tyres first, Mobile Tyre Shop when JAX has never met the
    ; plate. Both hand back the model, the trim and the year; JAX names the
    ; engine too. When both are silent the body type still names the model -
    ...
    if (InStr(make, "ISUZU")) {
        ...
        if (rego != "" && st != "") {
            GuiControl, RC:, RC_Status, Asking JAX what build this is ...
            jX := ""
            jTrim := ""
            jYear := ""
            jCab := ""
            ...
            if (vinModel = "") {
                GuiControl, RC:, RC_Status, Asking Mobile Tyre Shop what build this is ...
                vinModel := RC_MtsRego(rego, st, jX, jTrim, jYear, jCab)
            }
            if (vinModel != "") {
                vinYear := jYear
                mkX := jX
            ...
            ; The cab, the doors and the ride height get their own row, slid
            ; in just above the wiper rows so the red expiry row above keeps
            ; its place.
            if (jCab != "") {
    ...
    ; Whatever the maker's own sources left blank - model, engine or trim -
    ; Autobarn's shop gets one chance to fill from the rego. Not for Isuzu:
    ; there the tyre shops above are the whole story, with the VIN as the
    ; last word.
    if ((vinModel = "" || mkX = "" || trm = "") && plate != "" && st != "" && !InStr(make, "ISUZU")) {
        GuiControl, RC:, RC_Status, Asking AB what build this is ...
        abX := ""
        abTrim := ""
        abModel := RC_AbVin(plate, st, abX, abTrim)
        if (vinModel = "" && abModel != "")
            vinModel := abModel
        if (abX != "" && mkX = "")
            mkX := abX
...
RC_AbVin(plate, state, ByRef extra, ByRef trim := "") {
...
RC_JaxRego(plate, state, ByRef extra, ByRef trim, ByRef year, ByRef cab := "") {
    ...
    if RegExMatch(page, """VehicleName"":""([^""]+)""", n) {
        ...
        t := RegExReplace(t, "i)\b(4x4|4x2|AWD|4WD|2WD|FWD|RWD|Crew|Space|Dual|Single|Extended|King|Cab|Chassis|Utility|Ute|Pick.?Up|Tray|Wagon|Van|Sedan|Hatch|Coupe|SUV|Hi.?Rider?|Low.?Rider?|MY\d+(\.\d+)?|\d+dr|\d+D)\b", " ")
        trim := Trim(RegExReplace(t, "\s+", " "))
    }
...
RC_MtsRego(plate, state, ByRef extra, ByRef trim, ByRef year, ByRef cab := "") {
    ...
    ; An unknown plate answers with the vehicle set to null.
    ...
    ; The trim sits in the build line after the model, with the driveline and
    ; body words around it - same peeling as JAX, and the same keeping of the
    ; cab, door and ride words in their own line.
    if RegExMatch(car, """name"":""([^""]+)""", n) {
        cut := InStr(n1, model)
        if (cut) {
            t := SubStr(n1, cut + StrLen(model))
            cab := RC_CabWords(t)
            t := RegExReplace(t, "i)\b(4x4|4x2|AWD|4WD|2WD|FWD|RWD|Crew|Space|Dual|Single|Extended|King|Cab|Chassis|Utility|Ute|Pick.?Up|Tray|Wagon|Van|Sedan|Hatch|Coupe|SUV|Hi.?Rider?|Low.?Rider?|MY\d+(\.\d+)?|\d+dr|\d+D)\b", " ")
            trim := Trim(RegExReplace(t, "\s+", " "))
        }
...
RC_CabWords(text) {
    out := ""
    pos := 1
    while (pos := RegExMatch(text, "i)\b(Crew|Space|Dual|Single|Extended|King|Cab|Chassis|Pick.?Up|Tray|Utility|Ute|Wagon|Van|Hi.?Rider?|Low.?Rider?|\d+D|\d+dr)\b", c, pos)) {
        pos += StrLen(c)
        out .= (out = "" ? "" : " ") . c1
    }
    return out
...
RC_SetMake(bold, bracket) {
    Gui, RC:Default
    ; A long build line drops a point of size so it still fits the window.
    Gui, Font, % "s" . (StrLen(bracket) > 44 ? 8 : 9) . " w400", Segoe UI
    GuiControl, RC:Font, RC_MakeX
