...
;     refreshes live - for VEHICLE lookups only. A part number is never
;     cached: its stock figure is what the counter acts on, and a saved
;     "1 in stock" from half an hour ago is a sale that may have happened.
...
;------------------------- CONFIG (edit here) --------------------------------
global IV_VER  := "4.1"                       ; bump by hand on every change
...
global CFG_STALE_MS := 3500                   ; IDS quiet this long = check the session, re-sign-in if dead
...
; in through DoLookup; the vehicle window's through DoVinLookup. Each reads
; its own window and hands kind and value to LookupRun, which is the lookup.
DoLookup:
...
LookupRun:
    ...
    try {
        ...
        if (res = "") {
            ...
            ; went quiet for 3.5s, so the session is signed in again and the
            ; same number is sent again, and the line says so while it runs.
            if (gStale)
...
; the note at the top of LookupRun). Vehicle results are.
CacheWrite(kind, value, res) {
    if (kind = "part")
        return
