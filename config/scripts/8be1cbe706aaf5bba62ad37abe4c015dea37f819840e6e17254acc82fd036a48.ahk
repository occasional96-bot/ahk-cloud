...
; three trims on one build line - "MU-X LS-M, LS-T, LS-U" - and only a
; fitment database says which one the plate wears. That line never goes under
; the make either; the VIN names the model (RC_IsuSeed). For everything else
; the shops fill blanks and no more.
;
...
RC_IsuSeed(c) {
    ...
    if (model = "")
        return false
    c.isu := true
    ; The VIN's word for the model, whatever came before it. EzyParts writes
    ; every trim the build was sold in on its model line - "MU-X LS-M, LS-T,
    ; LS-U" - and that is not a model, so it never goes under the make.
    c.vinModel := model
    if (yr != "")
        c.vinYear := yr
    if (gen != "" && c.genTag = "")
