...
global csHoverRow := 0
Gui, CurStrip:Font, s8 Norm c8A90A0, Segoe UI
; version rides the header (Kaine 2026-07-24: "add the version up at the very top,
; at the end of Quick Actions")
Gui, CurStrip:Add, Text, x12 y8 w224 h16 BackgroundTrans, ERA - QUICK ACTIONS   v%DOCK_VERSION%
...
Gui, CurStrip:Add, Text, x175 y8 w16 h16 0x200 Center BackgroundTrans gCurStripDot vCsDot c9CA3AF, % Chr(0x25CF)
; gear in the header (1.38.0): the status drawer's only front door now. Teal while
; the drawer is open (CurStripDot recolors it), gray otherwise.
Gui, CurStrip:Font, s10 Norm c8A90A0, Segoe UI
Gui, CurStrip:Add, Text, x214 y6 w24 h20 0x200 Center BackgroundTrans gCurStripDot vCsGear, % Chr(0x2699)
Gui, CurStrip:Font, s8 Norm c8A90A0, Segoe UI
; X in the header's right corner - one more way out (Kaine 2026-07-24)
Gui, CurStrip:Add, Text, x244 y6 w28 h20 0x200 Center BackgroundTrans gCurStripX, % Chr(0x2715)
...
StatusDot(state) {
    ...
    GuiControl, CurStrip:+c%c%, CsDot
    GuiControl, CurStrip:, CsDot, % Chr(0x25CF)
}
...
CurStripToggle() {
    ...
    if (DllCall("IsWindowVisible", "Ptr", CurStripHwnd)) {
        ...
        WinGetPos, bx, by, bw, bh, ahk_id %CurStripHwnd%
        ...
        if (hAt && hAt != CurStripHwnd) {
            WinSet, Top, , ahk_id %CurStripHwnd%
            if (DllCall("IsWindowVisible", "Ptr", CsStHwnd))
                WinSet, Top, , ahk_id %CsStHwnd%
            return
        }
        CurStripHide()
        return
...
; Gear color tracks the drawer: teal open, gray shut (1.38.0).
CsGearPaint(open) {
    c := open ? "2FBFB5" : "8A90A0"
    GuiControl, CurStrip:+c%c%, CsGear
    GuiControl, CurStrip:, CsGear, % Chr(0x2699)
}
...
CurStripDot:
    ...
    WinSet, Top, , ahk_id %CsStHwnd%    ; 1.37.0: not topmost - raise on show
    g_csStOpen := 1
    CsGearPaint(true)
return
