...
;  Right-click = DockMenu. 1.35.2: backtick means the palette EVERYWHERE - the
...
; ---- dock globals ----------------------------------------------------------
global DOCK_VERSION := "1.35.2"  ; 1.35.2: backtick ALWAYS opens the palette - the scoped SC029 rebuild (sender window/Outlook) hijacked it and popped the Invoice Sender; that hotkey is deleted, rebuild moved to the tray ("Rebuild window") | 1.35.1: RcpHomePage no longer resizes the ERA window (WinMove 820x502 removed - Kaine 2026-07-27) | 1.35.0: Status dropdown is a custom panel, EXACT palette width, flush underneath (native menus size to content and cannot be widened); Status row click toggles it - panel hides, palette stays; item click fires + closes both | 1.34.2: hover glow rebuilt as teal TEXT + 3px gutter accent strip (overlaps nothing) - the full-width bar always lost either paint or input to the rows; rows opaque now so the recolor repaints clean | 1.34.1: hover bars moved to LAST in creation order = bottom of z-order - text readable while hovered and real clicks reach the rows (probe5: real clicks died with bars on top even with WS_EX_TRANSPARENT) | 1.34.0: THE dead-rows fix - hover bars get WS_EX_TRANSPARENT so clicks pass through to the row statics (probe-verified; 1.33.1's header-only drag was necessary but not sufficient); version now shown in the palette header | 1.33.1: dead-rows fix - the drag hook was eating EVERY palette click (transparent statics deliver clicks via the parent); drag is header-only now, rows/X/dot click again | 1.33.0: palette snaps OUTSIDE ERA's left edge, tops aligned (inside top-left when no room; mouse position when no ERA); draggable by header/background (HTCAPTION), stays where dropped, re-snaps next open; hover glow bar tracks the row under the mouse (60ms timer - worklist owns WM_MOUSEMOVE) | 1.32.1: WS_EX_NOACTIVATE on the palette - clicking X/rows never activates it, focus stays put, X hides only (backtick re-shows) | 1.32.0: palette no longer steals focus - shown NA, persists while unfocused (AlwaysOnTop), closes only on X / backtick / row click; focus-loss watcher deleted | 1.31.0: cursor strip -> command palette (mockup 1) - dark 280x132 panel, header + X, three rows with hotkey hints, dot on the Status row only; trigger moved Ctrl+Alt+J -> backtick (SC029), sender's rebuild backtick now really scoped to its window + Outlook | 1.30.0: titlebar strip retired - Ctrl+Alt+J cursor strip is the only strip now (TrackWindow early-returns after trackedHwnd upkeep; right-click on the cursor strip = the old DockMenu, Settings' other home besides the tray) | 1.29.0: Ctrl+Alt+J cursor strip - the dock's three cells popped as a floating AlwaysOnTop strip at the mouse; closes on re-press, Esc, cell click, or focus loss; dot painted by the same StatusDot | 1.28.0: flicker fix - Gui Show only on the hidden->visible edge (it raises to TOP every call; the 250ms tick was a 4Hz z-order fight over Chrome), moves are SetWindowPos NOZORDER; strip parks 8px left of the REAL caption buttons (DWMWA_CAPTION_BUTTON_BOUNDS) so min/max/X never covered at 125/150% scale | 1.27.0: worklist 2.26.0 - red dots draw only when Po/Inv has both sides (Settings toggle, default ON) | 1.26.0: stdparts search is newest-wins across all 3 folders (invoice date, then rows, then file time - a stale C:\Temp copy can no longer shadow a fresh OneDrive/PSdata one), every candidate logged, the winner re-seeds C:\Temp | 1.25.0: Receipted column deleted, receipted rows draw a GREEN tick, the seven columns stretch to fill | 1.24.1: worklist 535 -> 575 so no column ellipsizes | 1.24.0: counts footer deleted, Hide-warn on the brand strip, h18 bar, unthemed search box, Ex GST floor 86 | 1.23.1: -MaximizeBox (v1) not -MaxBox (v2) - the worklist would not open | 1.23.0: worklist window fixed at 535x456, search Edit + X (no combo, no Receipt button), h36 bar | 1.22.2: default columns = all but Receipted; toast dwell 1.2s | 1.22.1: Subtract-by defaults to 0+B (25 live runs - it is the proven one now) | 1.22.0: uncovered lines are subtracted in BOTH qty modes (the missing 0+B), Fast + Hide-warn are the defaults | 1.21.2: same-day re-exports upload when the local file grew (row-count tie-break) | 1.21.1: no more "? not checked yet" - check runs before the boot import and on first dot click | 1.21.0: Full refresh runs the invoice CSVs in a child process, two-lane progress panel | 1.20.0: Full refresh + grouped menu, no passwords in the one-click run | 1.19.1: HY defaults to recv-aLL under auto | 1.19.0: "Export + upload PO data" - ERA drives the export, then the normal PO upload + import
...
; 1.32.0: shown NA and NEVER takes focus - it floats (AlwaysOnTop) while you keep
; working, and closes only deliberately: header X, backtick again, or a row click.
...
global csW := 280, csH := 132   ; header 8+16, three h28 rows at y32/64/96, 8px bottom pad
; 1.31.0: command-palette look (Kaine picked mockup 1) - dark vertical panel, header
; line, three rows with the hotkey hints right-aligned. No dot in the header (Kaine:
; "remove the green dot at the top") - the dot lives on the Status row only.
...
Gui, CurStrip:New, +ToolWindow -Caption +AlwaysOnTop +Border +E0x08000000 +HwndCurStripHwnd
...
Gui, CurStrip:Add, Text, x12 y64 w188 h28 0x200 0x80 gCurStripSnd vCsRow2, BYD & Isuzu Despatch
Gui, CurStrip:Add, Text, x12 y96 w168 h28 0x200 gCurStripDot vCsRow3, Status
Gui, CurStrip:Font, s8 Norm c8A90A0, Segoe UI
Gui, CurStrip:Add, Text, x180 y32 w88 h28 0x200 Right BackgroundTrans gCurStripWl, Ctrl+Space
Gui, CurStrip:Add, Text, x200 y64 w68 h28 0x200 Right BackgroundTrans gCurStripSnd, Ctrl+Alt+I
Gui, CurStrip:Font, s10 Norm c9CA3AF, Segoe UI
Gui, CurStrip:Add, Text, x236 y96 w32 h28 0x200 Center BackgroundTrans gCurStripDot vCsDot, % Chr(0x25CF)
...
global CsStHwnd := 0
global csStH := 330
Gui, CsSt:New, +ToolWindow -Caption +AlwaysOnTop +Border +E0x08000000 +HwndCsStHwnd
Gui, CsSt:Color, 14161C
...
Gui, CsSt:Add, Progress, x0 y78 w280 h1 -Theme c2A2E38 Background2A2E38, 100
...
; Global backtick pops the palette - everywhere, since 1.35.2. The scoped SC029
...
; Snap point: right edge of the palette touching ERA's LEFT edge, tops aligned
; (mockup 2026-07-24). DWM extended frame bounds, same reason as ToastAnchorRect -
; GetWindowRect includes an invisible resize border and the edges would not LOOK
; flush. Fallback when ERA is maximized / flush to the screen edge (no room on the
; left): park just INSIDE ERA's top-left instead. False = no usable ERA window.
...
; parks beside it. Bands mirror the control layout: y32/64/96, h28. Only repaints
...
CsRowPaint(i, on) {
    static cap1 := "Receipting", cap2 := "BYD & Isuzu Despatch", cap3 := "Status"
    c := on ? "2FBFB5" : "FFFFFF"
...
; backtick hotkey until 1.35.2 - backtick now always means the command palette).
RebuildWindow:
...
; 1.35.2: the scoped SC029 rebuild hotkey is GONE. With it, backtick over the
