...
; ---- dock globals ----------------------------------------------------------
global DOCK_VERSION := "1.21.2"  ; 1.21.2: same-day re-exports upload when the local file grew (row-count tie-break) | 1.21.1: no more "? not checked yet" - check runs before the boot import and on first dot click | 1.21.0: Full refresh runs the invoice CSVs in a child process, two-lane progress panel | 1.20.0: Full refresh + grouped menu, no passwords in the one-click run | 1.19.1: HY defaults to recv-aLL under auto | 1.19.0: "Export + upload PO data" - ERA drives the export, then the normal PO upload + import
