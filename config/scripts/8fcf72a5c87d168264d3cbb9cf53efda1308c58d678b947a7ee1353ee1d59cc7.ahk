...
global gSW_Pass     := 1          ; 1, or 2 while the unknowns are asked again
...
SW_StepDo() {
    ...
    local i, item, key, got, again
    if (gSW_Next > n) {
        ; The second pass: every part that came back unknown is asked for
        ; once more, from the top, and one that is still unknown is marked
        ; ?* - checked twice. (SW_BatchPollDo does the same for the Isuzu
        ; and BYD batches.)
        if (gSW_Pass = 1) {
            again := 0
            for i, item in gSW_Rows
                if (item.state = "unk") {
                    gSW_Done.Delete(SW_Key(item.part))
                    again++
                }
            if (again) {
                gSW_Pass := 2
                gSW_Next := 1
                SW_Log("second pass: " again " unknown")
                SW_SetStatus(gSW_Brand " - checking " again " again")
                SetTimer, SW_Step, -10
                return
            }
        }
    ...
    i := gSW_Next
    item := gSW_Rows[i]
    if (gSW_Pass = 2 && item.state != "unk") {
        gSW_Next := i + 1
        SetTimer, SW_Step, -10
        return
    }
    key := SW_Key(item.part)
    ...
    gSW_Next := i + 1
    if (gSW_Next <= n)
        SW_SetStatus(gSW_Brand " - checking " gSW_Next " of " n (gSW_Pass = 2 ? " again" : ""))
    SetTimer, SW_Step, -10
}
...
SW_FinishAll(state) {
    ...
    for i, item in gSW_Rows
        if (item.state = "wait")
            SW_Paint(i, state, SW_UnkText())
}

; The unknown mark: "?" on the first pass, "?*" once the part has been asked
; for twice (the second pass in SW_StepDo and SW_BatchPollDo).
SW_UnkText() {
    global gSW_Pass
    return (gSW_Pass >= 2) ? "?*" : "?"
}
...
; The "k" in front keeps the key a string: a bare number used as an object
; key is stored as an integer, so 0811230250 and 811230250 would be one key,
; and the variable it came from reads back without its zero.
SW_Key(s) {
...
SW_BatchPollDo() {
    ...
    ;--- the second pass: what came back unknown, or never came back, goes
    ;    once more; what is still unknown after that is marked ?*
    if (gSW_Pass = 1 && !quiet) {
        again := []
        for i, code in gSW_TBCodes {
            k := SW_Key(code)
            if (!gSW_Done.HasKey(k) || !gSW_Done[k].ok)
                again.Push(code)
        }
        if (again.MaxIndex()) {
            gSW_Pass := 2
            SW_Log("second pass: " again.MaxIndex() " unknown")
            SW_SetStatus(gSW_Brand " (Melbourne) - checking " again.MaxIndex() " again")
            SW_BatchStart(again)
            return
        }
    }
...
IZ_RunBatchList(listFile, outFile) {
    ...
    {
        c := Trim(A_LoopField)
        ; "k" in front of the key: a bare number used as a key is stored as
        ; an integer, and the variable it came from then reads back as one
        ; too - 0811230250 went into IDS as 811230250, which it has not got.
        if (c = "" || seen.HasKey("k" c))
            continue
        seen["k" c] := true
        codes.Push(c)
    }
...
; Exact map first. "澳洲分公司" is literally "Australia branch office"; it is
...
BY_WarehouseName(cn) {
    ...
    if (!init) {
        init := true
        map := {}
        map["澳洲分公司"]       := "Melbourne warehouse"
        map["澳洲悉尼仓库"]     := "Sydney warehouse"
        map["澳洲珀斯仓库"]     := "Perth warehouse"
        map["澳洲布里斯班仓库"] := "Brisbane warehouse"
        ; fallback vocabulary, longest tokens first so 中心库 wins over 仓库
        toks := "布里斯班=Brisbane |阿德莱德=Adelaide |墨尔本=Melbourne |悉尼=Sydney |珀斯=Perth |澳洲=Australia |分公司=branch |中心库=central store |仓库=warehouse |库=store "
    }
    cn := Trim(cn)
    if (cn = "")
        return ""
...
BY_CdpBatch(listFile, outFile, para := 6) {
    ...
    {
        c := Trim(A_LoopField)
        ; "k" in front of the key, as in the Isuzu leg: a bare number used
        ; as a key comes back without its leading zero.
        if (c = "" || seen.HasKey("k" c))
            continue
        seen["k" c] := true
        codes.Push(c)
...
BY_BatchEmit(outFile, code, summary) {
    ...
    BY_Log("RESULT " . code . "  " . RegExReplace(summary, "s)\R.*"))
}
