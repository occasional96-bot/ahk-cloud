...
;  Right-click = DockMenu. Over the sender window or Outlook, backtick still means
;  REBUILD (scoped #If wins there).
...
; ---- dock globals ----------------------------------------------------------
global DOCK_VERSION := "1.39.0"  ; 1.39.0: row 2 renamed "BYD & Isuzu Invoice Sender"; Ctrl+Space and Ctrl+Alt+I hotkeys RETIRED (hints + menu accelerators scrubbed) - palette rows are the way in
...
; Global backtick pops the palette. Over the sender window or Outlook the OTHER
; SC029 (the scoped rebuild, see the sender section) wins - #If hotkeys outrank
; global ones. Backtick was ALREADY swallowed globally by that rebuild hotkey, so
; typing ` in Notepad/ERA never inserted a character anyway - no new loss.
...
; 1.31.0: NOW ACTUALLY SCOPED to the tool window + Outlook. The comment above always
; claimed that, but the hotkey was global - it worked out because nothing else wanted
; backtick. The command palette does now (see the CurStrip section), and #If hotkeys
; outrank global ones, so: sender/Outlook = rebuild, everywhere else = palette.
