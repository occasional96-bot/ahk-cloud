...
;
; Which words those are is worked out from the word itself and the column it
; sits in - not from the name of the screen, which is what this used to do
; and which only ever worked on the one screen it was written for.
global EW_STRIP_MAKE  := true
global EW_STRIP_MAKES := "HY,KI,IA,BY,NG"  ; the only makes that come
                                           ; off. Case matters. A word
                                           ; starting with two letters
                                           ; that are not on this list is
                                           ; never touched, which is what
                                           ; keeps CMF70L, DIN66MF,
                                           ; NS40LMF and EMBLEM-I30
                                           ; intact - they read like a
                                           ; make and a part but are not.
global EW_STRIP_LEN   := 2              ; how long the make code is
global EW_STRIP_REST  := 5              ; and the shortest part number that
                                        ; can be left behind. Under this it
                                        ; was not a part number to begin with.
global EW_STRIP_MATES := 0              ; how many other rows have to carry
                                        ; the same make, in the same column,
                                        ; before any of it comes off. Off,
                                        ; because it was written for cross
                                        ; reference lists, where the make is
                                        ; on every row - and a backorder
                                        ; screen can show one part with
                                        ; nothing else to vote, where it
                                        ; refused a strip that was right.
                                        ; The list above is the real guard.
...
;   THE MAKE CODE
;
;   On a cross reference the Part# column reads make code first and part
;   number after, with nothing between them: KIAK0C0WS005 is make KI, part
;   AK0C0WS005. Two characters come off the front.
;
;   Which words those are used to be decided by the name of the screen and
;   the heading of the column. That only ever worked on the one screen it was
;   written for, and it read the make off anything sitting in that column.
;   The word decides now:
;
;     the first two characters are a make we were asked to strip, and
;     what is left is long enough to be a part number and has a digit in it.
;
;   Either of those failing leaves the word exactly as it reads. That is the
;   safe way round: a make left on the front is wrong on the screen in front
;   of you, a make taken off a part number that never had one is a different
;   part, ordered and paid for.
;
;   THE FRONT COMES OFF ONCE. Never in a loop. A real part number can begin
;   with letters that are themselves a make - HYBYA20APH00 is make HY and
;   part BYA20APH00, NGKI094705L is make NG and part KI094705L - and a second
;   pass would eat the front of a genuine number. Twenty seven of those in a
;   seventeen thousand row stocktake, so not a corner case.
;
;   Note also what is NOT tested: that the part number starts with a digit.
;   Three thousand of those seventeen thousand start with letters after the
;   make comes off - KIAAA20APH01, HYAB3Z6K683Q - so that test would have
;   left the make on one row in five.
;
;   The column can be made to corroborate as well, by setting
;   EW_STRIP_MATES above zero: the same make then has to start a word in
;   the same column on that many other rows. It is off, because a
;   backorder screen showing a single received part has no other rows to
;   vote, and a correct strip was refused for want of company.
;
;   Only the clipboard is affected. The highlight still covers the whole
;   word, because the whole word is what is on the screen.
;=============================================================================
EW_StripMake(lines, row, span) {
    global EW_STRIP_MAKE, EW_STRIP_LEN, EW_STRIP_REST, EW_STRIP_MATES
    global EW_DEBUG
    ...
    ; Is the front of it a make we were asked to take off?
    make := SubStr(text, 1, EW_STRIP_LEN)
    if (!EW_IsMakeCode(make))
        return text

    ; And is what is left a part number? Long enough, and with a digit in it.
    ; A description word that happens to start with two of those letters -
    ; HYDRAULIC, KINGPIN - has no digit and does not get this far.
    rest := SubStr(text, EW_STRIP_LEN + 1)
    if (StrLen(rest) < EW_STRIP_REST)
        return text
    if (!RegExMatch(rest, "[0-9]"))
        return text

    ; Does the column agree? On a cross reference every row in that column
    ; carries the same make, so this costs nothing there, and it is the one
    ; test a lone accessory code sitting among part numbers cannot pass.
    if (EW_STRIP_MATES > 0) {
        mates := EW_ColumnMates(lines, row, span.c1, make)
        if (mates < EW_STRIP_MATES) {
            if (EW_DEBUG)
                FileAppend, % "make [" make "] left on [" text "]"
                           . " - only " mates " mates in column " span.c1 "`n"
                           , %A_ScriptDir%\eraword_debug.txt
            return text
        }
    }
...
;   IS THIS ONE OF OURS
;
;   Case matters, which is why this compares with == and not with = or !=.
;   Both of those are blind to case in AutoHotkey, and a description reading
;   "hybrid" would match the make HY if they were used here.
;=============================================================================
EW_IsMakeCode(make) {
    global EW_STRIP_MAKES

    if (StrLen(make) < 1)
        return false

    for i, code in StrSplit(EW_STRIP_MAKES, ",", " `t") {
        if (code != "" && make == code)
            return true
    }
    return false
...
;
;   How many other rows start a word with this make, in this exact column.
;   The screen is a fixed grid, so a column really is the same character
;   position on every row - this is not an approximation.
;
;   A row only counts if the make begins a word there: nothing joined to it
;   on the left, and something joined to it on the right. Otherwise a heading
;   or a lone two letter code would vote, and the whole point of the count is
;   that only part numbers vote.
;
;   The clicked row does not count itself.
;=============================================================================
EW_ColumnMates(lines, row, col, make) {
    global EW_STRIP_LEN
    ...
    for i, line in lines {
        ...
        ; == not != on purpose - see EW_IsMakeCode.
        if (!(SubStr(line, col + 1, EW_STRIP_LEN) == make))
            continue

        ; The make has to be the front of the word, not the middle of one.
        if (col > 0 && EW_IsWordChar(SubStr(line, col, 1)))
            continue

        ; And there has to be a part number after it.
        if (!EW_IsWordChar(SubStr(line, col + EW_STRIP_LEN + 1, 1)))
            continue
