...
DoResetPasteQuery(clickReset, fast) {
    ...
    if needReset {
        ...
        ; guard: if the late wipe lands after the first empty read, wait it out
        Loop, 3 {
            Sleep, 100
            if (GetFieldValue(root) != "") {
                Loop, 20 {
...
; Query: DEFAULT = focus field -> Tab -> Enter (tab order lands on Query btn,
; Enter fires it - real keystrokes ExtJS always honours).
; Backup: ControlClick (background mouse messages, no cursor move).
; Last resort: physical click with cursor restore.
FireQuery(root) {
    fld := FindField(root)
    if fld {
        try {
