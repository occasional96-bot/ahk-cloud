...
global CP_HOST := { kia: "microcat", hyundai: "microcat" }
...
; GDI+'s token, the EPC button's two drawn faces, and the loading bar's
; measurements - it is redrawn as it moves and needs them every time.
...
; How the EPC button reads and how it is painted, resting and armed. The ring
; says where the next Enter goes; the return arrow on the label says it is the
; Enter key that goes there, which the ring alone cannot. The deeper green
; carries the same news for anyone who does not see a two pixel band.
...
RC_BuildGui() {
    ...
    ; through RC_OnEnter rather than the search itself, because after a search
    ; Enter has a second job: opening the catalog the ring is pointing at.
    Gui, Add, Button, x-200 y-200 w1 h1 Default gRC_OnEnter, S
    ; A drawn picture rather than a Progress with its corners clipped off - see
    ; the shape helpers for why. Nothing needs trimming a pixel in any more
    ; either: a Progress drew a sunken 3D edge of its own that had to be cut
    ; away, and a picture draws only what it was told to.
    RC_GdipOn()
    bg := RC_BgArgb()
    Gui, Add, Picture, % RC_P(324, 12, 84, 30) . " vRC_SearchBar HwndRC_hSearchBtn Disabled"
       , % "HBITMAP:*" . RC_PillBmp(RC_S(84), RC_S(30), RC_S(8), RC_Argb("0067C0"), bg)
    Gui, Font, s10 w600 cFFFFFF, Segoe UI
    Gui, Add, Text, % RC_P(324, 12, 84, 30) . " Center 0x200 BackgroundTrans gRC_OnSearch vRC_SearchTxt", Search
    ...
    br := RC_S(8)                                ; corner ellipse, the button's own
    ...
    ; Both faces of the button are drawn once here and swapped when the ring
    ; goes up or down. A Progress could be recoloured on the spot with one
    ; GuiControl; a picture cannot be repainted, only replaced, and drawing the
    ; replacement at the moment of the swap would be work done over and over
    ; for two pictures that never change.
    RC_hEpcRest := RC_PillBmp(bw, bh, br, RC_Argb(RC_EPC_BG), bg)
    RC_hEpcHot  := RC_PillBmp(bw, bh, br, RC_Argb(RC_EPC_BG_HOT), bg)
    Gui, Add, Picture, % RC_P(324, 50, 84, 30) . " vRC_EpcBar HwndRC_hEpcBtn Disabled"
       , % "HBITMAP:*" . RC_hEpcRest
    Gui, Font, s10 w600 cFFFFFF, Segoe UI
    Gui, Add, Text, % RC_P(324, 50, 84, 30) . " Center 0x200 BackgroundTrans gRC_OnEpc vRC_EpcTxt", % RC_EPC_LBL
    Gui, Font, s10 w400 cDefault, Segoe UI
...
;
; Every request this tool makes is a blocking WinHTTP call on the window's own
; thread. While one is in flight the window is frozen: timers do not fire and
; nothing repaints. Measured rather than assumed - a 297 ms request let one
; 50 ms timer tick through out of the six it was owed. So there is no animating
; THROUGH a request, and a bar that appeared to crawl along during one would be
; painting a lie about work that is not happening in any observable way.
;
; What it does instead is move on the real events and slide rather than jump
; between them, in the moments the thread is its own - which is exactly either
; side of a request. Every position the bar takes is something that actually
; happened.
;
...
RC_BarPos(pct) {
    global RC_hLoadBar, RC_BAR_W, RC_BAR_H, RC_BAR_R, RC_BAR_BG, RC_BAR_PX, RC_QUIET
    ; The bar is the one thing Ultra must never do. A search that has already
    ; answered and is quietly filling itself in behind the scenes is finished
    ; as far as anyone watching is concerned, and a bar creeping along the
    ; bottom says the opposite.
    if (RC_QUIET)
        return
    if (pct < 0)
        pct := 0
...
RC_BarUp() {
    global RC_BAR_ON, RC_SB1, RC_SB_LAST, RC_QUIET
    if (RC_QUIET)
        return
    RC_Use()
    if (!RC_BAR_ON) {
...
RC_BarDown() {
    global RC_BAR, RC_BAR_ON, RC_SB1, RC_SB_LAST, RC_QUIET
    if (RC_QUIET)
        return
    RC_Use()
...
RC_BarEnd(park := 0) {
    global RC_BAR_ON, RC_hLoadBar, RC_QUIET
    if (RC_QUIET)
        return
...
RC_Done(ok, word, detail := "") {
    ...
    RC_BarEnd((hit && detail != "kept" && RC_SPEED != 0) ? 42 : 0)
...
RC_EpcHot(on) {
    global RC_EPCHOT, RC_EPC_LBL, RC_EPC_LBL_HOT, RC_hEpcBtn, RC_hEpcRest, RC_hEpcHot
    if (RC_EPCHOT = on)
        return
    RC_EPCHOT := on
    RC_Use()
    GuiControl, % "RC:" . (on ? "Show" : "Hide"), RC_EpcRing
    ; Both faces were drawn at build time; neither is freed on the swap, because
    ; the other one is wanted back next time.
    RC_SetPic(RC_hEpcBtn, on ? RC_hEpcHot : RC_hEpcRest, false)
    GuiControl, RC:, RC_EpcTxt, % (on ? RC_EPC_LBL_HOT : RC_EPC_LBL)
}
...
RC_OnSearch() {
    ...
    ; down with it: it belongs to the car being replaced, and RC_Done puts it
    ; back up if the new one is a catalog make too.
    RC_GEN += 1
    RC_CTX := ""
    RC_EpcHot(false)
...
RC_Walk(order, plate, mode, ByRef tried, ByRef errors) {
    ...
    RC_Use()
    for i, st in order {
        ; Move the bar in a little, then let the window repaint, before we block
        ; on the network for a second or two and it can do neither.
        RC_BarBusy()
        Sleep, 20
...
RC_Fill(text, plate, vin := "", ByRef make := "", st := "") {
    ...
    RC_CTX := { gen: RC_GEN, key: RC_LastKey, rego: rego, st: st, src: "state"
        ...
        , yearWeak: false, note: "", stage: 1, full: (RC_SPEED != 1)
        , cabDone: false }
...
RC_Rest() {
    global RC_SPEED, RC_QUIET, RC_QOn
    ...
    if (InStr(make, "ISUZU")) {
        ...
        if (vin != "") {
            ...
            if (IsObject(ids)) {
                ...
                if (vinYear = "" && ids.year != "")
                    vinYear := ids.year
                ; The shops are let off only when the record settled BOTH the
                ; cab and the back of the truck. IDS writes C/C when it is a
                ; cab chassis and writes nothing at all when it is a tub, so a
                ; description naming only the cab leaves the tub unsaid - and
                ; the tub is half of what anyone asks for. One shop is cheap
                ; and being wrong about the back of a D-MAX is not.
...
RC_IsuzuCab() {
    ...
    ; shop's own words become the row and the last pass tidies them.
...
RC_TagShow() {
    ...
    if (c.mkX != "")
        tag .= (tag != "" ? ", " : "") . c.mkX
    tag := RC_TagTail(tag, c.badge, c.bodyHint . " " . c.mkX . " " . c.trm . " " . c.body)
...
RC_FillEzyPlate(plate) {
    ...
    RC_CTX := { gen: RC_GEN, key: RC_LastKey, rego: plate, st: hitSt, src: "ezy"
        ...
        , note: "", stage: 1, full: (RC_SPEED != 1), cabDone: false }
...
RC_IdsRows(ids, vin) {
    RC_SetRow("Model code", ids.model)
    RC_SetRow("Model", ids.model_desc)
    RC_SetRow("Ride height", RC_IdsRide(vin, ids.model, ids.model_desc))
    ...
    ; says and the VIN does not carry.
...
RC_IdsWords(desc) {
    return Trim(RegExReplace(desc, "i)\bC/C\b", " Cab Chassis "))
}
...
RC_IdsPaint(colour, code) {
    ...
    Loop, % LV_GetCount()
    {
        ...
        if (f == "Colour") {
            ...
            if InStr(v, code)
                return
            LV_Modify(A_Index, "Col2", Trim(Trim(v) . " (" . code . ")"))
            return
        }
    }
    ; No colour row at all - the state kept it to itself - so IDS writes one.
    RC_SetRow("Colour", (colour != "") ? colour . " (" . code . ")" : code)
}
...
RC_IdsLock(timeoutMs) {
    ...
    if (!RC_IdsMx)
        return false
    r := DllCall("WaitForSingleObject", "Ptr", RC_IdsMx, "UInt", timeoutMs, "UInt")
...
CP_BrandOf(title) {
    global CP_BRANDS, CP_WORDS, CP_WMI, CP_VETO, CP_ANTI, CP_MUSTCAT
    if (Trim(title) = "")
        return ""
    flat := CP_Norm(title)
