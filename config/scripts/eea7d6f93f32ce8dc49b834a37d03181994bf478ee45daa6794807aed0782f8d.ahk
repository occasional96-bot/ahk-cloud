...
Gui, CurStrip:Add, Text, x12 y8 w224 h16 BackgroundTrans, ERA - QUICK ACTIONS
...
Gui, CurStrip:Add, Text, x12 y32 w168 h28 0x200 BackgroundTrans gCurStripWl, Receipting
; 0x80 = SS_NOPREFIX or the "&" renders as an underline mnemonic
Gui, CurStrip:Add, Text, x12 y64 w188 h28 0x200 0x80 BackgroundTrans gCurStripSnd, BYD & Isuzu Despatch
Gui, CurStrip:Add, Text, x12 y96 w168 h28 0x200 BackgroundTrans gCurStripDot, Status
Gui, CurStrip:Font, s8 Norm c8A90A0, Segoe UI
...
CurStripToggle() {
    ...
    MouseGetPos, mx, my
    x := mx - 10, y := my - 10              ; cursor lands just inside the header
    CurStripClamp(x, y)
    ; 1.32.0 (Kaine: "make it not always steal the focus... it can stay out of focus"):
    ; shown NA - whatever you were typing in KEEPS focus, and the palette PERSISTS
    ; until deliberately closed (X, backtick, or a row click). AlwaysOnTop keeps it
    ; visible while unfocused. The old focus-loss watcher is gone with the focus.
...
CurStripDot:
    ; Same 3-pull menu as the dock dot: same first-click check, same StBuildMenu,
    ; anchored under the strip's left edge.
    if (g_stCheckedAt = 0)
        StatusRunCheck(true)
    Gosub, StBuildMenu
    WinGetPos, csMx, csMy,, csMh, ahk_id %CurStripHwnd%
    csMy := csMy + csMh
    Menu, StMenu, Show, %csMx%, %csMy%
    CurStripHide()
return
