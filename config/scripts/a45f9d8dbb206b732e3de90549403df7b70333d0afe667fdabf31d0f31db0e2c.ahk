...
global VERSION  := "1.2"            ; shown at the end of the window title; bump on each release
global PENDING  := false            ; a number that arrived while BUSY - runs when the busy stretch ends
...
; A number that arrived while BUSY runs now. A timer rather than a Gosub, so
; the busy thread finishes first and the query gets a clean one.
PendingRun() {
    global PENDING
    if (!PENDING)
        return
...
; The button, Enter in the box, and a number on the command line all come in
; here, and this does none of the work: a button's thread must be over in an
; instant, because AutoHotkey drops a second press of the same button while
; its first thread is still running - so a press that arrived mid-query used
; to vanish. This only notes that a query is wanted (PENDING) and QueryGo does
; it on a timer thread of its own, now, or when the busy stretch ends
; (PendingRun). QueryGo reads the box when it fires, so the newest number in
; it is the one looked up.
DoQuery:
    PENDING := true
    if (BUSY)
        SetStatus("One moment - finishing the last lookup, then yours runs...")
    else
        SetTimer, QueryGo, 10
return

; The timer is periodic and turns itself off here: a one-shot could be skipped
; if it fell due while a QueryGo thread was still up, and the request would
; then wait for the next busy stretch to end. Periodic, it simply tries again.
QueryGo:
    if (!PENDING) {
        SetTimer, QueryGo, Off
        return
    }
    if (BUSY)
        return
    ...
    ; BUSY stays up until the answer is drawn (QueryEnd), so a queued query
    ; cannot start redrawing the table under this one.
    Gui, ListView, LV
    if (!res.ok) {
...
; Every way out of QueryGo: the busy flag comes down and anything that
; arrived while it was up runs next.
QueryEnd:
    BUSY := false
    PendingRun()
return
