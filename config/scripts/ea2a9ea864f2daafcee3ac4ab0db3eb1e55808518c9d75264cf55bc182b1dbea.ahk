...
#Include, C:\Users\kainet\OneDrive - Hopper Motor Group\Desktop\ \AHK\UIA_Interface.ahk
SetBatchLines, -1
...
WatchField:
    ...
    ; Identify field by ControlType + Name (stable across sessions/machines)
    if (!focused || focused.CurrentControlType != 50004 || !InStr(focused.CurrentName, "Material code")) {
        lastVal := ""
        return
    }

    val := focused.CurrentValue
    if (val != lastVal && val != "") {
        clip := Trim(Clipboard)
...
UIA_10018Pattern(p := "") {
    static IID := "{828055ad-355b-4435-86d5-3b51c14a9b1b}" ; LegacyIAccessiblePattern
    return UIA._Pattern(p, IID, "IUIAutomationLegacyIAccessiblePattern")
}

UIA_10000Pattern(p := "") {
    static IID := "{b114a1db-887f-4d2f-b994-46e12f4f2fb0}" ; InvokePattern
    return UIA._Pattern(p, IID, "IUIAutomationInvokePattern")
}
