...
; Parse-verify guard: `AutoHotkeyU64.exe /ErrorStdOut <copy-of-this-file> selfcheck` makes
; the interpreter read the ENTIRE file (any syntax error exits non-zero before this line
; runs) and then exit before the GUI/tray/OLE machinery starts. Run it against a COPY -
; #SingleInstance Force on the real path would kill the running sender.
...
Menu, Tray, Add
Menu, Tray, Add, Upload PO Data, MenuUploadPo
Menu, Tray, Add, Receipting worklist`tCtrl+Space, MenuWorklist
...
BuildGui:
    ...
    Gui, Add, Text, x+90 yp+8, % "v" . VERSION
...
MenuUploadPo:
    ...
    Log("po-data: uploaded rows=" mRc1 " byd=" mBc1)
...
WorklistStart(wait := false) {
    ...
    if (hwnd)
        return hwnd
    path := A_ScriptDir "\" WL_SCRIPT
    if (!FileExist(path)) {
        Log("worklist: " WL_SCRIPT " not found beside the sender")
        if (wait)
            ShowToast("bad", "Worklist not found", WL_SCRIPT " is missing from the sender's folder")
        return 0
    }
    Run, % """" A_AhkPath """ """ path """", %A_ScriptDir%, UseErrorLevel
    if (ErrorLevel) {
        Log("worklist: could not launch " path)
        if (wait)
            ShowToast("bad", "Worklist would not start", "See sender.log")
        return 0
    }
    Log("worklist: launched " WL_SCRIPT)
    if (!wait)
        return 0
    ; Give the interpreter a moment to create the script window before we post to it - a
    ; PostMessage to hwnd 0 is silently lost, which would look exactly like "the button did
    ; nothing" (5 s is generous; a cold start measured well under 1 s).
    Loop, 50 {
        Sleep, 100
        hwnd := WorklistHwnd()
        if (hwnd)
    ...
    Log("worklist: launched but its window never appeared - not pinged")
    return 0
}
...
WorklistHwnd() {
    ...
    SetTitleMatchMode, 2
    hwnd := WinExist(WL_SCRIPT " - AutoHotkey ahk_class AutoHotkey")
...
ReadConfig() {
    global IsuzuUrl, BydUrl, ReconUrl, DefaultBrand, EnableEmailDrag, CfgFanOut
...
ReconBatchEnd() {
    ...
    if (res.status != 200) {
        Log("recon: flags failed http=" res.status " | " SubStr(res.text, 1, 200))
...
ShowToast(state, line1, line2 := "") {
    ...
    Gui, 1:Default      ; Toast:New hijacks the thread's default GUI - restore it
}
