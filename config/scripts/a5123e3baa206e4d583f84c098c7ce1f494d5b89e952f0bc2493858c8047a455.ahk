...
;  Despatch / Status+dot). Closes on: backtick again, Esc, the header X, any row
;  click, or clicking anywhere else (focus loss). Right-click = DockMenu. Over the
;  sender window or Outlook, backtick still means REBUILD (scoped #If wins there).
...
; The dock's three cells, popped as a floating strip AT THE MOUSE from any app - for
; when ERA (and its titlebar strip) is buried. Same labels, widths, teal; the dot is
; painted by the same StatusDot() the dock uses, so they can never disagree.
; AlwaysOnTop is fine HERE (unlike the dock, 1.8.0) because the strip is transient -
; it lives only until a cell click, Esc, Ctrl+Alt+J again, or focus loss
; (CurStripWatch), so it cannot strand itself over another app.
global CurStripHwnd := 0
global g_csMenuOn := false      ; StMenu is up - watcher must not hide the palette under it
global g_csGrace := 0           ; Show->activation lag counter, see CurStripWatch
...
Gui, CurStrip:New, +ToolWindow -Caption +AlwaysOnTop +Border +HwndCurStripHwnd
...
CurStripToggle() {
    global CurStripHwnd, csW, csH, g_csGrace
    ...
    CurStripClamp(x, y)
    g_csGrace := 0
    ; Shown ACTIVATED on purpose - focus loss is the close signal. (The dock showed NA
    ; and never took focus; the palette NEEDS focus or "click elsewhere to close"
    ; has nothing to watch.)
    Gui, CurStrip:Show, x%x% y%y% w%csW% h%csH%
    WinActivate, ahk_id %CurStripHwnd%
    SetTimer, CurStripWatch, 150
}
...
; "Click anywhere else and it goes." Polling WinActive is the reliable signal here;
; g_csMenuOn parks the watcher while StMenu is up (the menu takes activation and the
; strip would hide out from under it). The grace ticks cover Show->activation lag:
; Windows' foreground lock can delay our WinActivate a beat, and without the grace
; the strip would hide itself 150ms after appearing.
CurStripWatch:
    if (g_csMenuOn)
        return
    if (!DllCall("IsWindowVisible", "Ptr", CurStripHwnd)) {
    ...
    if (WinActive("ahk_id " CurStripHwnd)) {
        g_csGrace := 0
        return
    }
    g_csGrace += 1
