...
ExportOnly(eraQuery, csvName) {
    ...
    if !WaitAndSend("PC destination format", "ListBox1", "{Enter}", 10)
        return
    if !WaitAndSend("Select the destination file for the ERA data", "Button2", "{Enter}", 10)
        return
    if !WaitAndSend("Confirm Save As", "Button1", "{Left}{Enter}", 10)
        return
