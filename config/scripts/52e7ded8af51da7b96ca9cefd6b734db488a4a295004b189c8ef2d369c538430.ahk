...
; Ctrl+V in DMS: Reset -> wait for field to clear -> paste -> watcher fires Query
#IfWinActive DMS - Google Chrome
$^v::
    root := UIA.ElementFromHandle(WinExist("A"))
    if root {
        btn := root.FindFirstBy("ControlType=Button AND Name=Reset")
        if btn {
            ClickBtn(btn)
            Loop, 20 {
                fld := root.FindFirstBy("ControlType=Edit AND Name=Material code", 0x4, 2)
                try {
                    if (fld && fld.CurrentValue = "")
                        break
                }
                Sleep, 100
            }
...
WatchField:
    if !WinActive("DMS - Google Chrome")
        return
    ...
    if (val != lastVal && val != "") {
        ...
        if (val = clip && RegExMatch(val, "^\d{8}-\d{2}$")) {
            Sleep, 100
            root := UIA.ElementFromHandle(WinExist("A"))
            btn := root.FindFirstBy("ControlType=Button AND Name=Query")
            if btn
                ClickBtn(btn)
        }
    }
...
; Always-works click: Invoke pattern -> physical click at element centre
ClickBtn(el) {
    ; 1) Invoke pattern (never touches LegacyIAccessible - that's what threw 0x80070057)
    try {
        if el.GetCurrentPropertyValue(30031) {   ; IsInvokePatternAvailable
            el.GetCurrentPatternAs("Invoke").Invoke()
            return 1
        }
    }
    ; 2) Physical click at bounding-rect centre (works on anything visible)
    try {
