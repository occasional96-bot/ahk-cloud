...
;
;  Backtick / SC029 (1.31.0, was Ctrl+Alt+J in 1.29): the COMMAND PALETTE - a dark
;  panel at the mouse with the three old cells as rows (Receipting / BYD & Isuzu
;  Despatch / Status+dot). Opens WITHOUT stealing focus (1.32.0) and stays up while
;  you work; closes only on: backtick again, the header X, or any row click.
;  1.33.0: snaps OUTSIDE ERA's left edge (tops aligned), draggable by its header /
;  background and stays where dropped, hover glow on the row under the mouse.
;  Right-click = DockMenu. 1.39.1: backtick means the palette EVERYWHERE - the
;  scoped rebuild hotkey (sender window / Outlook) is gone, rebuild is a tray item.
;  1.30.0: the TITLEBAR strip is retired - the palette is the only launcher. The
;  Dock gui is still built and TrackWindow still ticks (trackedHwnd feeds toasts),
;  but an early return in TrackWindow keeps it hidden forever.
; ============================================================================
#NoEnv
...
; ---- dock globals ----------------------------------------------------------
global DOCK_VERSION := "1.42.0"  ; 1.42.0: refresh is single-run + transactional; stable/validated CSVs, SHA-256 compare, retried HTTP uploads, server read-back verification, partial/failed imports can no longer report green; redundant mid-run status/worklist pulls removed | 1.41.0: worklist 2.27.0 - "invoice already used" answers ERA No itself and STOPS (Kaine's circled-No screenshot); abort is final, the auto-retry no longer re-runs the loop | 1.40.1: the dot reads GREEN again - 1.40.0's dark trio overshot and 064E3B read black on the band; now 15803D/B45309/B91C1C | 1.40.0: header banner is TEAL permanently (each header static paints its own band through WM_CTLCOLORSTATIC - no overlapping bar, so nothing ghosts or eats a click); backtick raises a buried palette in three verified tiers (HWND_TOP, topmost flip, then a pin that lasts only while it is open) | 1.39.1: backtick ALWAYS opens the palette - the scoped SC029 rebuild (sender window/Outlook) hijacked it and popped the Invoice Sender; that hotkey is deleted, rebuild moved to the tray ("Rebuild window") | 1.39.0: row 2 renamed "BYD & Isuzu Invoice Sender"; Ctrl+Space and Ctrl+Alt+I hotkeys RETIRED (hints + menu accelerators scrubbed) - palette rows are the way in
                                 ; 1.38.1: Full refresh progress panel parks DEAD flush in ERA's bottom-right corner, matching the toast (12px margin gone)
                                 ; 1.38.0: palette down to 2 rows - Full refresh row gone; gear glyph in header opens the status drawer; worklist Refresh button = full pipeline; 5px gap
                                 ; 1.37.0: 10px breath between palette/worklist and ERA (DWM-measured); palette+drawer no longer topmost - windows cover them, backtick raises
                                 ; 1.36.0: header dot beside version; row 3 = Full refresh (left-click runs it), chevron/right-click = status drawer; DWM-rounded corners, drawer recessed + teal spine
                                 ; 1.35.0: Status dropdown is a custom panel, EXACT palette width, flush underneath (native menus size to content and cannot be widened); Status row click toggles it
...
global g_fullBusy := false       ; one refresh transaction at a time - repeated clicks are ignored
global g_fullRunId := ""         ; correlates every line written by one refresh
...
Menu, DockMenu, Add, Receipting worklist, DockMenuWorklist
Menu, DockMenu, Add, Invoice sender, DockMenuSender
Menu, DockMenu, Add
; One item since 2026-07-20 - the Kia + Hyundai CSV row moved to the status dot menu.
Menu, SndCellMenu, Add, BYD + Isuzu invoices (PDF window), DockMenuSender
...
; ---- Ctrl+Alt+J cursor strip (1.29.0) --------------------------------------
; The dock's three cells as a command palette AT THE MOUSE, from any app. The dot is
; painted by the same StatusDot() the dock used, so they can never disagree.
; 1.32.0: shown NA and NEVER takes focus. 1.37.0 (Kaine: "hidden if I bring another
; window over it... not bleed through"): NOT AlwaysOnTop anymore - it sits in the
; normal z-order, so any window dragged over it covers it like a normal app. It
; stays put; backtick raises it back (or hides it if it is already on top).
; Closes only deliberately: header X, backtick, or a row click.
; (Esc only works if you have clicked INTO the palette first - GuiEscape needs the
; window active, and by design it usually is not.)
global CurStripHwnd := 0
global csW := 280, csH := 100   ; header 8+16, two h28 rows at y32/64, 8px bottom pad (1.38.0: row 3 gone)
; 1.31.0: command-palette look (Kaine picked mockup 1) - dark vertical panel, header
; line, rows with the hotkey hints right-aligned. 1.31 removed the header dot; 1.36.0
; brought it BACK beside the version. 1.38.0 (Kaine: "remove the full refresh one and
; dropdown from that completely... come from that gear"): row 3 is GONE - the palette
; is Receipting + Despatch only, and the status drawer hangs off a gear glyph in the
; header (teal while open). Full refresh moved to the worklist's Refresh button and
; still lives inside the drawer.
; E0x08000000 = WS_EX_NOACTIVATE (1.32.1): clicks on the palette fire their g-labels
; WITHOUT ever activating the window - the app you were in keeps focus even while
; you click X or a row. Belt to the Show-NA braces: NA only stops the SHOW from
; activating; without this a later click still would.
Gui, CurStrip:New, +ToolWindow -Caption +Border +E0x08000000 +HwndCurStripHwnd
Gui, CurStrip:Color, 14161C
; Smooth rounded corners (Kaine 2026-07-24: "really smooth, not blocky"): DWM does the
; rounding (attr 33 DWMWA_WINDOW_CORNER_PREFERENCE = 2 DWMWCP_ROUND) - GPU-antialiased,
; same finish as Explorer's own menus. The old WinSet Region was a hard 1-bit pixel
; mask: jagged by construction, no AA possible. Set once here; survives Hide/Show.
DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", CurStripHwnd, "UInt", 33, "Int*", 2, "UInt", 4)
global csHoverRow := 0
global g_csPinned := 0          ; 1 while a tier-3 CsRaise has the palette pinned topmost (cleared on hide)
; ---- the teal header band (1.40.0) -----------------------------------------
; Kaine 2026-07-28: "make the top of the banner teal all the time" - teal used to
; appear only under the mouse. The band is NOT a coloured bar sitting behind the
; text: 1.33/1.34 already proved a full-width bar fights the statics for z-order
; and loses either the paint or the clicks (probe5). Instead every header static
; paints its OWN background teal from one shared brush in WM_CTLCOLORSTATIC, and
; the header is tiled edge to edge - the pads exist purely so the band has no
; gaps. Measured before shipping (cs_probe): all six band samples read 2FBFB5 and
; the panel below still reads 14161C.
; The ink map is keyed by NUMERIC hwnd. GuiControlGet hands back a hex STRING and
; lParam arrives as an integer, so a string key silently never matches and the
; band stays dark - that was the first probe's failure, not a paint problem.
global CS_BAND := 0x2FBFB5          ; the band itself
global CS_INK  := 0x08211F          ; default ink on the band (near-black teal)
global CsHdrInk := {}               ; numeric control hwnd -> ink color
global CsHdrBrush := DllCall("gdi32\CreateSolidBrush", "UInt", CsBGR(CS_BAND), "Ptr")
Gui, CurStrip:Font, s8 Norm, Segoe UI
Gui, CurStrip:Add, Text, x0 y0 w12 h24 vCsHdPadL,
; version rides the header (Kaine 2026-07-24: "add the version up at the very top,
; at the end of Quick Actions")
Gui, CurStrip:Add, Text, x12 y0 w163 h24 0x200 vCsHdTitle, ERA - QUICK ACTIONS   v%DOCK_VERSION%
; status dot rides the header beside the version (1.36.0). Same vCsDot the one-painter
; StatusDot() has always recolored - only its ADDRESS moved up here.
Gui, CurStrip:Add, Text, x175 y0 w16 h24 0x200 Center gCurStripDot vCsDot, % Chr(0x25CF)
Gui, CurStrip:Add, Text, x191 y0 w23 h24 vCsHdPadM,
; gear in the header (1.38.0): the status drawer's only front door now. 1.40.0: on
; a teal band a teal gear is invisible, so "drawer open" is now WHITE ink.
Gui, CurStrip:Font, s10 Norm, Segoe UI
Gui, CurStrip:Add, Text, x214 y0 w24 h24 0x200 Center gCurStripDot vCsGear, % Chr(0x2699)
Gui, CurStrip:Font, s8 Norm, Segoe UI
Gui, CurStrip:Add, Text, x238 y0 w6 h24 vCsHdPadR,
; X in the header's right corner - one more way out (Kaine 2026-07-24)
Gui, CurStrip:Add, Text, x244 y0 w28 h24 0x200 Center gCurStripX vCsX, % Chr(0x2715)
Gui, CurStrip:Add, Text, x272 y0 w8 h24 vCsHdPadE,
for csI, csV in ["CsHdPadL","CsHdTitle","CsDot","CsHdPadM","CsGear","CsHdPadR","CsX","CsHdPadE"]
    CsHdrInk[CsHwndOf(csV)] := CS_INK
OnMessage(0x138, "CsHdrColor")    ; WM_CTLCOLORSTATIC
; Rows deliberately NOT BackgroundTrans (1.34.2): the hover recolor re-sets their
; text, and a transparent static repainting over its own old glyphs ghosts. Opaque
; statics erase with the gui background brush first - same dark panel, clean swap.
Gui, CurStrip:Font, s10 Norm cWhite, Segoe UI
Gui, CurStrip:Add, Text, x12 y32 w168 h28 0x200 gCurStripWl vCsRow1, Receipting
; 0x80 = SS_NOPREFIX or the "&" renders as an underline mnemonic
Gui, CurStrip:Add, Text, x12 y64 w230 h28 0x200 0x80 gCurStripSnd vCsRow2, BYD & Isuzu Invoice Sender
; (1.39.0: the Ctrl+Space / Ctrl+Alt+I hint texts are gone WITH their hotkeys -
; Kaine: "remove those trigger keys entirely". Rows open only from here.)
; hover glow (rebuilt 1.34.2): teal TEXT + a 3px teal accent strip in the left
; gutter (x6-x9; rows start at x12, so it overlaps NOTHING - the 1.33/1.34 full-
; width bar fought the rows for z-order and lost either paint or input every time:
; on top it hid text and ate real clicks even with WS_EX_TRANSPARENT, probe5).
; Created last anyway; with no overlap z-order no longer matters. WM_MOUSEMOVE is
; not used - the worklist owns OnMessage(0x200) - so a 60ms timer runs while the
; palette is visible.
Gui, CurStrip:Add, Progress, x6 y32 w3 h28 -Theme E0x20 c2FBFB5 Background2FBFB5 vCsHovAc Hidden, 100

; ---- status dropdown panel (1.35.0) ----------------------------------------
; The dot's native StMenu could never match the palette's width (menus size to
; their longest row, full stop - Kaine 2026-07-24: "same width as the actual GUI
; itself"). So the palette's Status row gets its own child panel: same dark skin,
; EXACTLY csW wide, shown flush under the palette. Status row click = toggle -
; hide the panel, palette stays. Item click = fire + close both. The dock's own
; StMenu stays for the (retired) titlebar dot.
global CsStHwnd := 0
global csStH := 330
global g_csStOpen := 0           ; 1 while the Status drawer is open (pins the parent row lit)
Gui, CsSt:New, +ToolWindow -Caption +Border +E0x08000000 +HwndCsStHwnd
; Drawer is a CHILD of the Status row, not a second menu (Kaine 2026-07-24: "a way to
; tell that dropdown is not part of the main dropdown"). Sold three ways: (1) bg one
; shade darker than the palette's 14161C so it reads as a recessed well, (2) a teal
; spine down the left edge tying it back to Status, (3) rounded corners on both windows
; (WinSet Region after each Show) so a small gap opens between them - two cards, not one.
Gui, CsSt:Color, 0F1014
; DWM-rounded like the palette (see CurStrip build for the why)
DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", CsStHwnd, "UInt", 33, "Int*", 2, "UInt", 4)
; teal spine - x0..x3, rows start at x12 so it overlaps no text; z-order irrelevant.
; Inset 12px top+bottom (Kaine 2026-07-24: "see how it's cut off?") so it ends before
; the corner curves instead of getting sliced mid-arc.
csStSpH := csStH - 24
Gui, CsSt:Add, Progress, x0 y12 w3 h%csStSpH% -Theme c2FBFB5 Background2FBFB5, 100
Gui, CsSt:Font, s9 Norm cWhite, Segoe UI
Gui, CsSt:Add, Text, x12 y8  w256 h20 0x200 vCsStL1, starting...
Gui, CsSt:Add, Text, x12 y30 w256 h20 0x200 vCsStL2,
Gui, CsSt:Add, Text, x12 y52 w256 h20 0x200 vCsStL3,
Gui, CsSt:Add, Progress, x0 y78 w280 h1 -Theme c2FBFB5 Background2FBFB5, 100
Gui, CsSt:Font, s8 Norm c8A90A0, Segoe UI
Gui, CsSt:Add, Text, x12 y86 w256 h16 vCsStH1, RUN IT ALL
Gui, CsSt:Font, s9 Norm cWhite, Segoe UI
Gui, CsSt:Add, Text, x12 y104 w256 h24 0x200 gCsStFullRefresh, Full refresh
Gui, CsSt:Font, s8 Norm c8A90A0, Segoe UI
Gui, CsSt:Add, Text, x12 y134 w256 h16, INVOICES -> WORKLIST
Gui, CsSt:Font, s9 Norm cWhite, Segoe UI
Gui, CsSt:Add, Text, x12 y152 w256 h24 0x200 gCsStGetLatest, Get latest + import
Gui, CsSt:Font, s8 Norm c8A90A0, Segoe UI
Gui, CsSt:Add, Text, x12 y182 w256 h16, PO DATA -> SERVER
Gui, CsSt:Font, s9 Norm cWhite, Segoe UI
Gui, CsSt:Add, Text, x12 y200 w256 h24 0x200 gCsStUploadPo, Upload from a file...
Gui, CsSt:Add, Text, x12 y224 w256 h24 0x200 gCsStPoExport, Export from ERA + upload
Gui, CsSt:Font, s8 Norm c8A90A0, Segoe UI
Gui, CsSt:Add, Text, x12 y254 w256 h16, DIAGNOSTICS
Gui, CsSt:Font, s9 Norm cWhite, Segoe UI
Gui, CsSt:Add, Text, x12 y272 w256 h24 0x200 gCsStReCheck, Check status only
Gui, CsSt:Add, Progress, x0 y300 w280 h1 -Theme c2A2E38 Background2A2E38, 100
Gui, CsSt:Font, s8 Norm c8A90A0, Segoe UI
Gui, CsSt:Add, Text, x12 y306 w256 h16 vCsStAgo, never checked

; Drag (1.33.0, fixed 1.33.1): a left-click in the HEADER's dead space becomes an
; OS titlebar drag - move the palette, it stays where dropped. Row/X/dot clicks
; fall through to AHK's default handling, which is what fires their g-labels
; (see CurStripDrag for why they arrive at the parent at all).
OnMessage(0x201, "CurStripDrag")

; The Gui,*:New calls above made CURSTRIP this thread's default GUI - the sender's BuildGui uses bare
...
TrackWindow:
    ...
    ; 1.30.0 (Kaine 2026-07-24: "i only want the ctrl alt j one to show up, not the one
    ; on the era bar"): the titlebar strip is RETIRED. This timer still runs because it
    ; is what keeps trackedHwnd fresh - ToastAnchorRect parks toasts inside ERA with it,
    ; and adoption above still matters after an ERA restart. Everything below this
    ; return (titlebar fitting, positioning, z-order pinning) is dead while the early
    ; return stands; delete the return to resurrect the strip.
    if (DllCall("IsWindowVisible", "Ptr", DockHwnd))
        Gui, Dock:Hide
    return
    ...
    DockSetHeight(tbH)
    ; Right edge parks 8px left of the caption buttons (min/max/X). DWM says where they
    ; really are - the old hard-coded 142 was right only at 100% scale; at 125/150% the
    ; buttons are wider and the strip sat ON them (other PCs, 2026-07-24). Fallback keeps
    ; the exact old sum: (tbW - 134) - guiW - 8 = tbW - guiW - 142.
    btnL := CaptionButtonsLeft(trackedHwnd)
    gx := (btnL ? btnL : tbX + tbW - 134) - guiW - 8
    gy := tbY
    ; Show ONCE, move quietly after. Gui Show - NA or not - raises the window to the TOP
    ; of the z-order on every call, and this ran 4x a second: on any PC where Chrome (etc)
    ; covered ERA the strip jumped over it and DockPinZ yanked it straight back - a 4Hz
    ; raise/drop fight, visible as background flicker on slower machines (2026-07-24).
    ; Now Gui Show fires only on the hidden->visible edge; a moved/resized ERA repositions
    ; via SetWindowPos with NOZORDER|NOACTIVATE, which never raises anything.
    if (!DllCall("IsWindowVisible", "Ptr", DockHwnd)) {
        Gui, Dock:Show, x%gx% y%gy% w%guiW% h%guiH% NA
        dkX := gx, dkY := gy, dkW := guiW, dkH := guiH
    } else if (gx != dkX || gy != dkY || guiW != dkW || guiH != dkH) {
        DllCall("SetWindowPos", "Ptr", DockHwnd, "Ptr", 0, "Int", gx, "Int", gy
              , "Int", guiW, "Int", guiH, "UInt", 0x4 | 0x10)  ; SWP_NOZORDER|SWP_NOACTIVATE
        dkX := gx, dkY := gy, dkW := guiW, dkH := guiH
    }
    DockPinZ()
return
...
; Screen X of the LEFT edge of the caption buttons (min/max/close), or 0 when DWM cannot
; say (pre-Win10, custom-framed windows, empty rect). DWMWA_CAPTION_BUTTON_BOUNDS (5)
; returns the rect in WINDOW-relative coords - offset by the GetWindowRect origin.
CaptionButtonsLeft(hwnd) {
    VarSetCapacity(cb, 16, 0)
    if (DllCall("dwmapi\DwmGetWindowAttribute", "Ptr", hwnd, "UInt", 5, "Ptr", &cb, "UInt", 16) != 0)
        return 0
    l := NumGet(cb, 0, "Int"), r := NumGet(cb, 8, "Int")
    if (r <= l)                                   ; empty rect: borderless/custom frame
        return 0
    VarSetCapacity(wr, 16, 0)
    DllCall("GetWindowRect", "Ptr", hwnd, "Ptr", &wr)
    return NumGet(wr, 0, "Int") + l
}
...
StatusDot(state) {
    ...
    ; 1.29.0: the cursor strip's dot is the SAME dot - one painter, no drift.
    ; 1.40.0: that dot now sits ON the teal band, where 4ADE80/FBBF24 wash out to
    ; near-invisible, so it gets a trio of its own and paints through the band
    ; painter (GuiControl +c cannot reach a control we colour in WM_CTLCOLORSTATIC).
    ; 1.40.1: the first trio overshot dark - 064E3B read BLACK on the band (Kaine:
    ; "my dot's not green"). These sit in the middle: dark enough to hold contrast
    ; against 2FBFB5, saturated enough to still NAME the colour at 16px.
    CsInkPaint("CsDot", (state = "ok") ? 0x15803D : (state = "warn") ? 0xB45309 : (state = "bad") ? 0xB91C1C : 0x0B3B38)
}
...
; ---- SC029 (backtick): the command palette (1.31.0) ------------------------
; Global backtick pops the palette - everywhere, since 1.39.1. The scoped SC029
; rebuild (sender window / Outlook) used to outrank this and popped the Invoice
; Sender instead; that hotkey is gone, rebuild moved to the tray menu.
SC029::CurStripToggle()

CurStripToggle() {
    global CurStripHwnd, CsStHwnd, csW, csH
    ; 2026-08-12: a fleet copy reached here with csW/csH EMPTY and no palette built
    ; (auto-exec never finished on that machine) - Gui,Show then died with
    ; "Invalid option ... w h NA". Re-arm the sizes, and if the palette Gui was
    ; never created, say so instead of letting Show spawn a blank default window.
    if (csW + 0 < 1 || csH + 0 < 1)
        csW := 280, csH := 100
    if (!CurStripHwnd) {
        ToolTip, ERA palette not ready - restart the Receipting script
        SetTimer, __CsTipOff, -1800
        return
    }
    if (DllCall("IsWindowVisible", "Ptr", CurStripHwnd)) {
        ; 1.37.0: no longer topmost, so "visible" can mean BURIED under another window.
        ; Probe what the OS would hit at the palette's own centre: if it is not us,
        ; backtick means "bring it back", not "close it". Drawer rides along.
        ; 1.40.0: the raise is CsRaise, which verifies it worked and escalates until
        ; it does - the 1.37.0 WinSet,Top was fire-and-forget (Kaine 2026-07-28:
        ; "it's sitting behind the arrow window, not on top of it").
        if (!CsOnTop(CurStripHwnd)) {
            CsRaise(CurStripHwnd)
            if (DllCall("IsWindowVisible", "Ptr", CsStHwnd))
                CsRaise(CsStHwnd)
            return
        }
        CurStripHide()
    ...
    ; 1.33.0 (Kaine: "snap to the top outside... top left outside", then "on the side
    ; of it"): park OUTSIDE ERA's left edge, tops aligned. Every open re-snaps - a
    ; dragged palette goes back to the snap point next backtick. No ERA window at
    ; all: fall back to the mouse position (the pre-1.33 behavior).
    if (!CsSnapPos(x, y)) {
    ...
    ; 1.32.0: shown NA - whatever you were typing in KEEPS focus, and the palette
    ; PERSISTS until deliberately closed (X, backtick, or a row click).
    Gui, CurStrip:Show, x%x% y%y% w%csW% h%csH% NA
    ; 1.37.0: SW_SHOWNA shows at the window's OLD z-position - without topmost that
    ; can be under whatever covered it last time. Raise explicitly (no activation).
    CsRaise(CurStripHwnd)
    SetTimer, CsHoverTick, 60
}

; guard tooltip auto-hide (2026-08-12, see CurStripToggle)
__CsTipOff:
    ToolTip
return

; ---- raise-on-demand (1.40.0) ----------------------------------------------
; Kaine 2026-07-28: "when I press the trigger button... it comes to the top of the
; windows, not always on top". Three tiers, each MEASURED before moving to the
; next - a raise that is not verified is how 1.37.0 shipped a call that looked
; right and left the palette buried:
;   1. HWND_TOP + SWP_NOACTIVATE - the ordinary raise (this is all WinSet,Top did).
;      A probe proved it does work across processes, even over the foreground
;      window, so it stays the first move.
;   2. topmost, then straight back off - lands the window at the top of the
;      non-topmost band when tier 1 was refused. Still NOT always-on-top after.
;   3. leave it pinned topmost. Only reachable when the thing covering us is
;      ITSELF always-on-top, where nothing else can win. The pin is temporary:
;      CurStripHide clears it, so the palette is never left permanently on top.
; The tier is logged so the tier-3 case can be identified on a real desktop
; instead of guessed at.
CsRaise(h) {
    static SWP := 0x13                ; NOSIZE | NOMOVE | NOACTIVATE
    global g_csPinned
    DllCall("SetWindowPos", "Ptr", h, "Ptr", 0, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", SWP)
    if (CsOnTop(h))
        return "top"
    DllCall("SetWindowPos", "Ptr", h, "Ptr", -1, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", SWP)
    DllCall("SetWindowPos", "Ptr", h, "Ptr", -2, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", SWP)
    if (CsOnTop(h)) {
        Log("[palette] raise: HWND_TOP refused, topmost flip won")
        return "flip"
    }
    DllCall("SetWindowPos", "Ptr", h, "Ptr", -1, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", SWP)
    g_csPinned := 1
    Log("[palette] raise: covered by an always-on-top window - pinned until close")
    return "pinned"
}

; Does this window own the pixel at its own centre? The only honest test of "am I
; on top" - z-order APIs lie about windows that are merely higher in the list but
; still covered by a topmost sibling.
CsOnTop(h) {
    WinGetPos, bx, by, bw, bh, ahk_id %h%
    if (bw = "" || bw = 0)
        return false
    hAt := DllCall("WindowFromPoint", "Int64", ((bx+bw//2) & 0xFFFFFFFF) | (((by+bh//2) & 0xFFFFFFFF) << 32), "Ptr")
    hAt := hAt ? DllCall("GetAncestor", "Ptr", hAt, "UInt", 2, "Ptr") : 0    ; GA_ROOT
    return (hAt = h)
}

; Drop a tier-3 pin. Called on every hide so "topmost" can never outlive the
; palette being open - the whole point of 1.37.0 stays intact.
CsUnpin(h) {
    DllCall("SetWindowPos", "Ptr", h, "Ptr", -2, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", 0x13)
}

; Snap point: right edge of the palette a small BREATH off ERA's LEFT edge, tops
; aligned. 1.35 parked it dead flush; 1.37.0 opens a 10px gap (Kaine 2026-07-24:
; "slight gap... it looks too flush"). DWM extended frame bounds, same reason as
; ToastAnchorRect - GetWindowRect includes an invisible resize border and the gap
; would not LOOK consistent. Fallback when ERA is maximized / flush to the screen
; edge (no room on the left): park just INSIDE ERA's top-left instead. False = no
; usable ERA window.
CsSnapPos(ByRef x, ByRef y) {
    global trackedHwnd, targetTitle, csW
    hEra := (trackedHwnd && WinExist("ahk_id " trackedHwnd)) ? trackedHwnd : WinExist(targetTitle)
    if (!hEra)
        return false
    WinGet, mm, MinMax, ahk_id %hEra%
    if (mm = -1 || !DllCall("IsWindowVisible", "Ptr", hEra) || DockIsCloaked(hEra))
        return false
    VarSetCapacity(fr, 16, 0)
    if (DllCall("dwmapi\DwmGetWindowAttribute", "Ptr", hEra, "UInt", 9, "Ptr", &fr, "UInt", 16) = 0) {
        ex := NumGet(fr, 0, "Int"), ey := NumGet(fr, 4, "Int")
    } else {
        WinGetPos, ex, ey,,, ahk_id %hEra%
    }
    x := ex - csW - 5, y := ey       ; 1.38.0: 10px felt wide, Kaine asked for 5
    hMon := DllCall("MonitorFromWindow", "Ptr", hEra, "UInt", 2, "Ptr")
    VarSetCapacity(mi, 40, 0), NumPut(40, mi, 0, "UInt")
    if (hMon && DllCall("GetMonitorInfo", "Ptr", hMon, "Ptr", &mi)) {
        if (x < NumGet(mi, 20, "Int"))          ; rcWork.left - no room outside
            x := ex + 8, y := ey + 8
    }
    return true
}

; A window's VISIBLE top-left via DWM extended frame bounds - GetWindowRect pads
; the left with an invisible resize border, so gaps measured from it lie by ~7px.
; Shared by CsSnapPos's siblings (worklist park, 1.37.0).
DockDwmXY(h, ByRef ex, ByRef ey) {
    VarSetCapacity(fr, 16, 0)
    if (DllCall("dwmapi\DwmGetWindowAttribute", "Ptr", h, "UInt", 9, "Ptr", &fr, "UInt", 16) = 0) {
        ex := NumGet(fr, 0, "Int"), ey := NumGet(fr, 4, "Int")
    } else {
        WinGetPos, ex, ey,,, ahk_id %h%
    }
}

; Header-only drag (1.33.1). The BackgroundTrans rows/X are click-TRANSPARENT
; statics: their clicks arrive as the PARENT's WM_LBUTTONDOWN and AHK's own default
; handling is what fires their g-labels. 1.33.0 turned every one of those clicks
; into a zero-pixel HTCAPTION drag before AHK ever saw them - dead rows, dead X,
; dead dot (Kaine's screenshot 2026-07-24). So: drag ONLY from the header's dead
; space (y<32, left of the X); everything else falls through untouched.
CurStripDrag(wParam, lParam, msg, hwnd) {
    global CurStripHwnd
    if (hwnd != CurStripHwnd)
        return
    cx := lParam & 0xFFFF, cy := (lParam >> 16) & 0xFFFF
    ; 1.38.0: drag zone stops at x170 - the header dot (x175), gear (x214) and X
    ; (x244) all live right of it and their clicks must reach the g-labels, not
    ; become a zero-pixel drag (same trap as 1.33.0's dead rows).
    if (cy >= 32 || cx >= 170)
        return                                         ; row / dot / gear / X - not ours
    PostMessage, 0xA1, 2, 0,, ahk_id %CurStripHwnd%    ; WM_NCLBUTTONDOWN, HTCAPTION
    return 0
}

; Row under the mouse gets the glow: its text turns teal and the accent strip
; parks beside it. Bands mirror the control layout: y32/64, h28. Only repaints
; on row CHANGE, so the 60ms tick is normally a no-op.
CsHoverTick:
    CoordMode, Mouse, Screen
    MouseGetPos, hmx, hmy
    WinGetPos, hpx, hpy, hpw, hph, ahk_id %CurStripHwnd%
    csRow := 0
    if (hmx >= hpx && hmx < hpx+hpw && hmy >= hpy && hmy < hpy+hph) {
        csRy := hmy - hpy
        if (csRy >= 32 && csRy < 60)
            csRow := 1
        else if (csRy >= 64 && csRy < 92)
            csRow := 2
    }
    if (csRow = csHoverRow)
        return
    csHoverRow := csRow
    CsRowPaint(1, csRow = 1)
    CsRowPaint(2, csRow = 2)
    if (!csRow)
        GuiControl, CurStrip:Hide, CsHovAc
    else {
        csRy := 32 + (csRow-1)*32
        GuiControl, CurStrip:MoveDraw, CsHovAc, y%csRy%
        GuiControl, CurStrip:Show, CsHovAc
    }
return

; Recolor one row. Re-setting the text is what forces the static to repaint with
; the new color; the rows are opaque (gui-background erase), so no ghosting.
CsRowPaint(i, on) {
    static cap1 := "Receipting", cap2 := "BYD & Isuzu Invoice Sender"
    c := on ? "2FBFB5" : "FFFFFF"
    GuiControl, CurStrip:+c%c%, CsRow%i%
    GuiControl, CurStrip:, CsRow%i%, % cap%i%
}

; Gear color tracks the drawer: 1.38.0 had teal open / gray shut, but 1.40.0 put a
; teal band underneath it - so open is WHITE and shut is the band's dark ink.
CsGearPaint(open) {
    global CS_INK
    CsInkPaint("CsGear", open ? 0xFFFFFF : CS_INK)
}

; ---- the header band painter (1.40.0) --------------------------------------
; One brush, one handler, one ink map. Returning a brush here means AHK's own
; WM_CTLCOLORSTATIC handling never runs for these controls, so the ink is ours to
; set too - GuiControl +cXXXXXX no longer reaches them. Everything else in the
; script (drawer, worklist, sender, toast) falls through untouched by the hwnd
; guard, which is why this cannot disturb any existing window.
CsHdrColor(wParam, lParam, msg, hwnd) {
    global CsHdrInk, CsHdrBrush, CS_BAND, CurStripHwnd
    if (hwnd != CurStripHwnd || !CsHdrInk.HasKey(lParam))
        return
    DllCall("gdi32\SetTextColor", "Ptr", wParam, "UInt", CsBGR(CsHdrInk[lParam]))
    DllCall("gdi32\SetBkColor",   "Ptr", wParam, "UInt", CsBGR(CS_BAND))
    return CsHdrBrush
}

; Recolor one header static: swap its ink and invalidate. The repaint re-fires
; CsHdrColor, which fills the whole control with the band brush first - so unlike
; the old GuiControl trick there is no need to re-set the text and nothing ghosts.
CsInkPaint(ctl, ink) {
    global CsHdrInk
    h := CsHwndOf(ctl)
    if (!h)
...
CsHwndOf(ctl) {
    GuiControlGet, h, CurStrip:Hwnd, %ctl%
    return h + 0                      ; NUMERIC - see the ink-map note at the build
}

; GDI wants COLORREF (BGR); every color in this script is written RRGGBB.
CsBGR(rgb) {
    return ((rgb & 0xFF) << 16) | (rgb & 0xFF00) | ((rgb >> 16) & 0xFF)
}

; Keep the strip on the monitor the mouse is on, inside the WORK area (never under
; the taskbar). MONITORINFO.rcWork sits at byte offset 20.
CurStripClamp(ByRef x, ByRef y) {
    global csW, csH
    hMon := DllCall("MonitorFromPoint", "Int64", (x & 0xFFFFFFFF) | ((y & 0xFFFFFFFF) << 32), "UInt", 2, "Ptr")
    VarSetCapacity(mi, 40, 0), NumPut(40, mi, 0, "UInt")
    if (hMon && DllCall("GetMonitorInfo", "Ptr", hMon, "Ptr", &mi)) {
        wl := NumGet(mi, 20, "Int"), wt := NumGet(mi, 24, "Int")
        wr := NumGet(mi, 28, "Int"), wb := NumGet(mi, 32, "Int")
        x := (x + csW > wr) ? wr - csW : x,  x := (x < wl) ? wl : x
        y := (y + csH > wb) ? wb - csH : y,  y := (y < wt) ? wt : y
    }
}

CurStripHide() {
    global csHoverRow, g_csStOpen, g_csPinned, CurStripHwnd, CsStHwnd
    SetTimer, CsHoverTick, Off
    ; 1.40.0: a tier-3 raise pins the window topmost. It never survives a close.
    if (g_csPinned) {
        CsUnpin(CurStripHwnd)
        CsUnpin(CsStHwnd)
        g_csPinned := 0
    }
    csHoverRow := 0
    ...
    CsGearPaint(false)
    GuiControl, CurStrip:Hide, CsHovAc
    Gui, CsSt:Hide                  ; dropdown never outlives the palette
    Gui, CurStrip:Hide
}
...
; X = HIDE only, never ExitApp (Kaine 2026-07-24: "the x button is just meant to
; close the gui"). Exit lives on the right-click DockMenu and the tray, nowhere else.
; Backtick re-shows the same hidden Gui - nothing is destroyed here.
CurStripX:
    CurStripHide()
return

; Right-click on the cursor strip = the old titlebar strip's right-click (DockMenu).
; With the titlebar strip retired (1.30.0) this and the tray are Settings' two homes.
; 1.38.0: right-click on the header dot or gear = the drawer, same as left-click.
; Everywhere else keeps DockMenu.
CurStripGuiContextMenu:
    if (A_GuiControl = "CsGear" || A_GuiControl = "CsDot") {
        Gosub, CurStripDot
        return
    }
    Menu, DockMenu, Show
...
CurStripDot:
    ; Toggle (1.35.0, Kaine: "if I click back on status... it hides it again. Not
    ; exit"): dropdown open = hide IT, palette stays. Otherwise: same first-click
    ; check as the dock dot, refresh the lines, show flush under the palette at
    ; the palette's exact width. 1.38.0: fired by the header GEAR (or the header
    ; dot) - the drawer's only front doors now that row 3 is gone.
    if (DllCall("IsWindowVisible", "Ptr", CsStHwnd)) {
        Gui, CsSt:Hide
        g_csStOpen := 0
        CsGearPaint(false)
    ...
    if (g_stCheckedAt = 0)
        StatusRunCheck(true)
    CsStRefresh()
    WinGetPos, csMx, csMy, csMw, csMh, ahk_id %CurStripHwnd%
    ; 1px overlap onto the palette's bottom edge so the two rounded silhouettes read as
    ; a parent card with a child tucked under it, not two strangers with a gap.
    csMy := csMy + csMh - 1
    Gui, CsSt:Show, x%csMx% y%csMy% w%csMw% h%csStH% NA
    CsRaise(CsStHwnd)                   ; 1.37.0: not topmost - raise on show (1.40.0: verified raise)
    g_csStOpen := 1
    CsGearPaint(true)
return

; Fill the panel's live lines from the same globals StBuildMenu reads.
CsStRefresh() {
    global
    local ago, mins, rel, stStamp
    GuiControl, CsSt:, CsStL1, % StatusGlyph(g_stS1) "  " g_stT1
    GuiControl, CsSt:, CsStL2, % StatusGlyph(g_stS2) "  " g_stT2
    GuiControl, CsSt:, CsStL3, % StatusGlyph(g_stS3) "  " g_stT3
    if (g_stCheckedAt = 0)
        ago := "never checked"
    else {
        mins := (A_TickCount - g_stCheckedAt) // 60000
    ...
    GuiControl, CsSt:, CsStAgo, %ago%
}

; Panel item = fire the same routine the StMenu row fires, then close both.
CsStFullRefresh:
    CurStripHide()
    Gosub, DockFullRefresh
return
CsStGetLatest:
    CurStripHide()
    Gosub, DockGetLatest
...
DockImportRun(showLoad := true) {
    ...
    if (ReconUrl = "") {
        Log("status: import unavailable - ReconApiUrl is empty")
        return false
    }
    if (g_impBusy) {
        Log("status: import refused - another import is already running")
        return false
    }
    g_impBusy := true
    StatusDot("busy")
    ...
    impRes := HttpPostPumpedRetriable(ReconUrl "/import-dealer-invoices", "{}", "application/json", 120, 3)
    ok := false
    if (impRes.status = 200) {
        ...
        WorklistPing(2)
        ok := true
    } else if (impRes.status = 207) {
        ; 207 is explicitly partial. It used to paint the lane green and let Full refresh
        ; claim success even though one feed could have failed.
        Log("status: import PARTIAL http=207 | " SubStr(impRes.text, 1, 500))
        ShowToast("bad", "Import only partly completed", "HTTP 207 - see the log")
    } else {
...
DockSender:
    ...
    ; you another click. SndCellMenu is left defined but unused.
    Gosub, ShowMainWindow
return
...
FullRefreshRun() {
    global g_fullBusy, g_fullRunId, g_jobState, g_jobLine
    if (g_fullBusy) {
        Log("full-refresh: ignored repeated click - a refresh is already running")
        ShowToast("load", "Refresh already running", "Please let the current refresh finish")
        return false
    }
    g_fullBusy := true
    g_fullRunId := A_Now "-" DllCall("GetCurrentProcessId") "-" A_TickCount
    g_jobLine := ""
    ProgShow()
    Log("full-refresh[" g_fullRunId "]: started")
    try {
    ...
    if (!frOk) {
        ...
        Log("full-refresh[" g_fullRunId "]: stopped - the ERA export or PO upload failed")
        Sleep, 2500
        ProgHide()
    ...
    if (g_jobState = "nolaunch") {
        ...
        while (g_jobState = "running" && A_TickCount - frT0 < 60000)
            Sleep, 100
        if (g_jobState = "running") {
            ; Never import an unknown mixture. The next refresh will compare the verified
            ; server copies and safely finish anything this worker may have uploaded.
            g_jobState := "timeout"
            g_jobLine := "no reply in 60s"
            Log("full-refresh[" g_fullRunId "]: invoice child never reported in 60s - import stopped")
            ProgSeg("B", 3, "err")
            ProgLaneText("B", "no reply in 60s - import stopped")
        }
    }

    if (g_jobState != "done") {
        ProgLaneText("C", "not run - invoice files were not fully verified")
        ProgSub("Stopped safely. PO is in; invoice import was not run.")
        Log("full-refresh[" g_fullRunId "]: stopped before import | invoices: " g_jobLine)
        Sleep, 1800
        ProgHide()
        ShowToast("bad", "Full refresh incomplete", "Invoice files failed verification - no import run")
        StatusRunCheck(false)
        return false
    }
    ...
    if (!DockImportRun(true)) {
        ProgSeg("C", 2, "err"), ProgLaneText("C", "import failed - see the log")
        ProgSub("Uploads are safe, but the final import failed.")
        Log("full-refresh[" g_fullRunId "]: stopped - import failed")
        Sleep, 1800
        ProgHide()
        ShowToast("bad", "Full refresh incomplete", "Uploads succeeded; final import failed")
        StatusRunCheck(false)
        return false
    }
    ...
    StatusRunCheck(true)
    Log("full-refresh[" g_fullRunId "]: finished | invoices: " (g_jobLine = "" ? "n/a" : g_jobLine))
    ...
    return true
        ...
        Log("full-refresh[" g_fullRunId "]: UNEXPECTED ERROR - " e.message " at line " e.line)
        ProgHide()
        ShowToast("bad", "Full refresh stopped", "Unexpected error - see the log")
        return false
    } finally {
        g_fullBusy := false
        g_fullRunId := ""
...
ProgShow() {
    ...
    if (ph = "")
        ph := 176
    ; Bottom-right INSIDE the tracked ERA window, exactly like the toast - and like the
    ; toast, DEAD flush into the corner (Kaine 2026-07-24: "should be flush in the corner,
    ; lined with ERA from side to bottom" - the old 12px margin left a visible L of ERA).
    if (ToastAnchorRect(ax, ay, aw, ah)) {
        px := ax + aw - pw
        py := ay + ah - ph
        if (px < ax)                      ; window narrower than the panel: never spill out
            px := ax
        if (py < ay)
            py := ay
    } else {
...
StdCsvSync(f, ByRef upN, ByRef keptN, ByRef failN) {
    ...
    srvWhy := "", locWhy := ""
    srvOk := (srvRes.status = 200 && CsvValidate(srvRes.text, fn, srvWhy))
    locOk := ReadStableCsv(f, glTxt, locWhy, 5000)
    if (locOk)
        locOk := CsvValidate(glTxt, fn, locWhy)
    if (!srvOk && srvRes.status != 200)
        srvWhy := "HTTP " srvRes.status (srvRes.text = "" ? "" : " - " SubStr(srvRes.text, 1, 100))
    if (!locOk && FileExist(f))
        Log("latest: " fn " - local copy rejected: " locWhy)
    if (!srvOk && srvRes.status = 200)
        Log("latest: " fn " - server copy rejected: " srvWhy)
    if (!locOk && !srvOk) {
        failN++
        Log("latest: " fn " - no usable copy | local: " locWhy " | server: " srvWhy)
        return false
    }

    locMax := locOk ? CsvMaxDate(glTxt) : "", srvMax := srvOk ? CsvMaxDate(srvRes.text) : ""
    locRows := locOk ? CsvRowCount(glTxt) : 0, srvRows := srvOk ? CsvRowCount(srvRes.text) : 0
    locFp := locOk ? CsvFingerprint(glTxt) : "", srvFp := srvOk ? CsvFingerprint(srvRes.text) : ""

    ; Exact content identity is the fastest route and avoids an unnecessary upload.
    if (locOk && srvOk && locFp = srvFp) {
        keptN++
        Log("latest: " fn " - already identical (sha256 " SubStr(locFp, 1, 12) ")")
        StdCacheWinner(fn, glTxt, f)
        return true
    }

    ; Keep a demonstrably newer server file. On a same-day tie, row count still protects a
    ; larger server export; equal-row but changed content is uploaded instead of being
    ; silently mistaken for identical as it was before.
    if (!locOk || (srvOk && (locMax < srvMax || (locMax = srvMax && locRows < srvRows)))) {
        keptN++
        Log("latest: " fn " - verified server copy kept (" CsvDateHuman(srvMax) ", " srvRows " rows, sha256 " SubStr(srvFp, 1, 12) ")")
        StdCacheWinner(fn, srvRes.text)
        return true
    }

    ; Local route: this machine's copy is newer - upload it.
    ShowToast("load", "Uploading " fn "...", locRows " validated rows - verifying after upload")
    upRes := CurlUploadFile(stdUrl, f, 3)
    if (upRes.status < 200 || upRes.status >= 300) {
        failN++
        Log("latest: " fn " upload FAILED http=" upRes.status " curl=" upRes.exit " | " SubStr(upRes.text, 1, 240))
        return false
    }

    verifiedTxt := ""
    if (!VerifyServerCsv(fn, locFp, verifiedTxt)) {
        failN++
        Log("latest: " fn " upload response was HTTP " upRes.status " but post-upload verification FAILED")
        return false
    }
    upN++
    Log("latest: " fn " uploaded and verified - " CsvDateHuman(locMax) ", " locRows " rows, sha256 " SubStr(locFp, 1, 12))
    StdCacheWinner(fn, glTxt, f)
    return true
}

; Waits for OneDrive/ERA to finish writing, then reads one coherent snapshot.
ReadStableCsv(path, ByRef txt, ByRef why, timeoutMs := 5000) {
    txt := "", why := ""
    if (!FileExist(path)) {
        why := "file not found"
        return false
    }
    if (!WaitFileStable(path, timeoutMs, 2)) {
        why := "file kept changing for " timeoutMs "ms"
        return false
    }
    FileRead, txt, %path%
    if (ErrorLevel) {
        why := "file could not be read"
        return false
    }
    return true
}

WaitFileStable(path, timeoutMs := 10000, samples := 2) {
    started := A_TickCount, lastSize := -1, lastTime := "", stable := 0
    Loop {
        FileGetSize, size, %path%
        sizeErr := ErrorLevel
        FileGetTime, stamp, %path%, M
        timeErr := ErrorLevel
        if (!sizeErr && !timeErr && size > 0 && size = lastSize && stamp = lastTime)
            stable++
        else
            stable := 0
        if (stable >= samples)
        ...
        if (A_TickCount - started >= timeoutMs)
            return false
        lastSize := size, lastTime := stamp
        Sleep, 250
    }
}

CsvValidate(txt, fn, ByRef why) {
    why := ""
    if (txt = "") {
        why := "empty file"
        return false
    ...
    eol := InStr(txt, "`n")
    header := eol ? SubStr(txt, 1, eol - 1) : txt
    if (!InStr(header, "Inv. No.") || !InStr(header, "Inv. Date")
        || !InStr(header, "Dealer Order No") || !InStr(header, "Total (Ex GST)")) {
        why := "unexpected CSV header"
        return false
    }
    rows := CsvRowCount(txt)
    if (rows < 1) {
        why := "no invoice rows"
        return false
    }
    if (CsvMaxDate(txt) = "") {
        why := "no usable invoice date"
        return false
    }
    return true
}

; Canonical line endings make hashes stable across WinHTTP/curl decoding differences.
CsvFingerprint(txt) {
    txt := StrReplace(txt, "`r", "")
    if (SubStr(txt, 1, 1) = Chr(0xFEFF))
        txt := SubStr(txt, 2)
    return Sha256Text(RTrim(txt, "`n"))
}

Sha256Text(txt) {
    hAlg := 0, hHash := 0, objLen := 0, cbOut := 0
    if (DllCall("bcrypt\BCryptOpenAlgorithmProvider", "Ptr*", hAlg, "WStr", "SHA256", "Ptr", 0, "UInt", 0, "UInt") != 0)
        return "fallback-" StrLen(txt) "-" CsvRowCount(txt) "-" CsvMaxDate(txt)
    if (DllCall("bcrypt\BCryptGetProperty", "Ptr", hAlg, "WStr", "ObjectLength", "UInt*", objLen, "UInt", 4, "UInt*", cbOut, "UInt", 0, "UInt") != 0) {
        DllCall("bcrypt\BCryptCloseAlgorithmProvider", "Ptr", hAlg, "UInt", 0)
        return "fallback-" StrLen(txt) "-" CsvRowCount(txt) "-" CsvMaxDate(txt)
    }
    VarSetCapacity(obj, objLen, 0)
    VarSetCapacity(digest, 32, 0)
    bytes := StrPut(txt, "UTF-8") - 1
    VarSetCapacity(data, bytes + 1, 0)
    StrPut(txt, &data, bytes + 1, "UTF-8")
    ok := (DllCall("bcrypt\BCryptCreateHash", "Ptr", hAlg, "Ptr*", hHash, "Ptr", &obj, "UInt", objLen, "Ptr", 0, "UInt", 0, "UInt", 0, "UInt") = 0)
       && (DllCall("bcrypt\BCryptHashData", "Ptr", hHash, "Ptr", &data, "UInt", bytes, "UInt", 0, "UInt") = 0)
       && (DllCall("bcrypt\BCryptFinishHash", "Ptr", hHash, "Ptr", &digest, "UInt", 32, "UInt", 0, "UInt") = 0)
    if (hHash)
...
CurlUploadFile(url, path, maxAttempts := 3) {
    out := {status: 0, text: "", exit: -1}
    q := Chr(34), pid := DllCall("GetCurrentProcessId")
    Loop, %maxAttempts% {
        n := A_Index
        token := pid "-" A_TickCount "-" n
        hdr := A_Temp "\era-upload-" token ".headers"
        body := A_Temp "\era-upload-" token ".body"
        FileDelete, %hdr%
        FileDelete, %body%
        htxt := "", outText := ""
        cmd := "curl.exe --silent --show-error --connect-timeout 10 --max-time 90 -D " q hdr q " -o " q body q " -F " q "file=@" path q " " q url q
        RunWait, %cmd%, , Hide UseErrorLevel
        out.exit := ErrorLevel
        FileRead, htxt, %hdr%
        FileRead, outText, %body%
        out.text := outText
        out.status := HttpLastStatus(htxt)
        FileDelete, %hdr%
        FileDelete, %body%
        if (out.exit = 0 && out.status >= 200 && out.status < 300)
            return out
        if (n >= maxAttempts || !HttpTransient(out.status, out.exit))
            return out
        Sleep, % (n = 1 ? 350 : 900)
    }
    return out
...
HttpLastStatus(headers) {
    status := 0, pos := 1
    while (pos := RegExMatch(headers, "i)HTTP/[^\s]+\s+(\d{3})", m, pos)) {
        status := m1 + 0
        pos += StrLen(m)
    }
    return status
...
HttpTransient(status, exitCode) {
    return (exitCode != 0 || status = 0 || status = 408 || status = 425
         || status = 429 || (status >= 500 && status <= 599))
}

VerifyServerCsv(fn, wantedFp, ByRef verifiedTxt) {
    global CsvBase
    verifiedTxt := ""
    Loop, 3 {
        r := HttpGetJson(CsvBase "/file/" fn, 8000, 1)
        why := ""
        if (r.status = 200 && CsvValidate(r.text, fn, why)
            && CsvFingerprint(r.text) = wantedFp) {
            verifiedTxt := r.text
            return true
        }
        if (A_Index < 3)
            Sleep, % (A_Index = 1 ? 350 : 900)
    }
    return false
}

StdCacheWinner(fn, txt, sourcePath := "") {
    global g_dataDir
    cache := g_dataDir "\inv-cache-" (InStr(fn, "ki") ? "KI" : "HY") ".csv"
    master := "C:\Temp\" fn
    FileCreateDir, C:\Temp
    if (sourcePath != "") {
        FileCopy, %sourcePath%, %cache%, 1
        if (sourcePath != master)
            FileCopy, %sourcePath%, %master%, 1
    } else {
        AtomicTextWrite(cache, txt)
        AtomicTextWrite(master, txt)
    }
}

AtomicTextWrite(path, txt) {
...
JobLaunch() {
    global g_jobState
    g_jobState := "running"
    jobPid := DllCall("GetCurrentProcessId")
    if (A_IsCompiled) {
        exe := A_Temp "\ERA_Dock_job_" jobPid ".exe"
        FileCopy, %A_ScriptFullPath%, %exe%, 1
        if (ErrorLevel) {
            Log("invoice job: could not prepare worker copy - running inline")
            g_jobState := "nolaunch"
            return false
        }
        ...
        job := A_Temp "\ERA_Dock_job_" jobPid ".ahk"
...
; ---- GUI is built by the BuildGui label (called at startup + tray "Rebuild window") ----
...
Menu, Tray, Add, Rebuild window, RebuildWindow
Menu, Tray, Add
...
Gosub, BuildGui          ; build the window once at startup (stays hidden until the palette row opens it)
return

; ============================================================
;  BuildGui:  destroy the current window (if any) and build a
;  fresh one. Safe to call anytime. Used at startup and by the
;  tray "Rebuild window" item for a clean rebuild.
; ============================================================
BuildGui:
    ...
    Gui, Show, Hide w484 h470, Invoice Sender   ; built hidden; the palette row shows it
...
MenuUploadPo:
    ...
    if (poCsv = "")
        return
    if (!WaitFileStable(poCsv, 10000, 2)) {
        SndLog("po-data: upload refused - file did not become stable: " poCsv)
        ShowToast("bad", "PO upload failed", "The CSV was still changing")
        return
    }
    ...
    poRes := HttpPostRetriable(ReconUrl "/po-data", poText, "text/csv", 3)
    if (poRes.status != 200) {
...
; Tray "Rebuild window": tears down + rebuilds the sender window (was the scoped
; backtick hotkey until 1.39.1 - backtick now always means the command palette).
RebuildWindow:
...
; (1.39.0: the Ctrl+Alt+I global hotkey is RETIRED - Kaine: "remove those trigger
; keys entirely". The sender opens from the palette row or the menus only.)

; 1.39.1: the scoped SC029 rebuild hotkey is GONE. With it, backtick over the
; sender window or Outlook popped the Invoice Sender instead of the palette
; (#If hotkeys outrank global ones). Backtick now means the palette everywhere;
; a clean rebuild lives in the tray menu ("Rebuild window").
...
global VERSION    := "2.27.0"    ; 2.27.0: "invoice already used" answers ERA No itself and stops - no MsgBox, no auto-retry (the old No never set g_rcpUserAbort, so the retry chain re-ran the loop)
...
Menu, Tray, NoStandard
Menu, Tray, Add, Show worklist, WlTrayShow
...
Menu, Tray, Default, Show worklist
...
; (1.39.0: the Ctrl+Space hotkey is RETIRED - Kaine: "remove those trigger keys
; entirely". The worklist opens from the palette's Receipting row or the menus.)
...
SbRefreshBtn:
    ; 1.38.0 (Kaine: "change the refresh function on the receiving work list to the
    ; full refresh function"): the button now fires the WHOLE pipeline - ERA PO export
    ; + invoice CSVs + import - not just a list reload. The list still catches up
    ; because the pipeline's import pings the worklist when it lands.
    Gosub, DockFullRefresh
return
...
SbReposition() {
    ...
    SetTitleMatchMode, 2
    if (hEra := WinExist("ERA Port")) {
        ; 1.37.0: DWM frame bounds like CsSnapPos - the old WinGetPos included ERA's
        ; ~7px invisible resize border, which ATE the +8 and the worklist LOOKED flush
        ; (Kaine 2026-07-24: "make the receiving work list do the same"). Same 10px
        ; breath as the palette now, measured from the visible edge.
        DockDwmXY(hEra, ex, ey)
        x := ex - (W + 5)            ; 1.38.0: 5px, matching the palette
        if (x < 0)
            x := 0
        y := ey
    }
...
SidebarBuild(visible := true) {
    ...
    SetTitleMatchMode, 2
    if (hEra := WinExist("ERA Port")) {
        DockDwmXY(hEra, ex, ey)      ; visible edge + 5px breath (see SbReposition)
        x := ex - (W + 5)
        if (x < 0)
            x := 0
        y := ey
...
PoUploadRun(fromSet, presetCsv := "", silent := false) {
    ...
    if (poCsv = "")
        return false
    if (!WaitFileStable(poCsv, 10000, 2)) {
        Log("po-data: upload refused - file did not become stable: " poCsv)
        if (fromSet)
            MsgBox, 0x30, PO upload failed, The selected CSV is still being written or synchronised. Wait a moment and try again.
        else
            ShowToast("bad", "PO upload failed", "The CSV was still changing")
        return false
    }
    ...
    ; Full refresh no longer spends two status requests describing a snapshot nobody has
    ; to approve. The server cliff gate still protects it, and a single post-upload status
    ; request below verifies the replacement. This is both faster and more conclusive.
    if (silent) {
        Log("po-data: fresh ERA export auto-accepted by Full refresh (" poName ", sha256 " SubStr(CsvFingerprint(poText), 1, 12) ")")
    } else {
    ...
    if (poRes.status = 409) {
        ...
        poRes := HttpPostRetriable(ReconUrl "/po-data?force=1", poText, "text/csv", 3)
    }
    ...
    if (mRc1 = "") {
        Log("po-data: HTTP 200 carried no rowCount - refusing to claim success | " SubStr(poRes.text, 1, 300))
        ShowToast("bad", "PO upload not verified", "The service response was incomplete")
        return false
    }
    if (!PoVerifyUpload(mRc1 + 0)) {
        Log("po-data: post-upload status did not confirm " mRc1 " rows")
        ShowToast("bad", "PO upload not verified", "Server status did not match the upload")
        return false
    }
    FileCopy, %poCsv%, %A_ScriptDir%\po-cache.csv, 1   ; local copy feeds the PO-only rows
    parts := ""
    ...
    ; Full refresh reloads once after the final import. Avoiding this mid-pipeline worklist
    ; fetch makes the common path faster and prevents it painting a half-refreshed state.
    if (!silent)
        SidebarRefresh(false)
    return true
}

PoVerifyUpload(expectedRows) {
    global ReconUrl
    Loop, 3 {
        r := HttpGetJson(ReconUrl "/po-data/status", 8000, 1)
        RegExMatch(r.text, """rowCount""\s*:\s*(\d+)", vRc)
        if (r.status = 200 && vRc1 != "" && (vRc1 + 0) = expectedRows)
            return true
        if (A_Index < 3)
            Sleep, % (A_Index = 1 ? 350 : 900)
    }
...
; Vendor ..."). Port of Speed_Receipting's Invoice_used_Dialouge_Box.
; v2.27.0 (Kaine 2026-07-28, F088045 screenshot with No circled: "build it so it
; answers No and stops"): the run answers ERA's dialog No ITSELF and aborts - the
; old flow answered ERA Yes, then popped a MsgBox asking Kaine, and a No there
; never set g_rcpUserAbort, so the v2.9.0 auto-retry re-launched the run and the
; same dialog came straight back ("I don't want it re-running the loop"). An
; already-used invoice is a fact a re-run cannot change, so the abort is FINAL:
; g_rcpUserAbort goes up and the retry chain stays down.
; CfgContinueInvUsed still opts a machine into the old Yes-and-continue for
; legitimate re-use flows (split shipments) - that path is unchanged.
RcpInvoiceUsedCheck(plan) {
    global CfgContinueInvUsed, g_rcpDry, g_rcpUserAbort
    if (g_rcpDry)
    ...
    if (CfgContinueInvUsed) {
        ControlClick, Button1, ahk_id %used%, , Left, 1, NA   ; Yes - carry on
        Log("receipt: " plan.id " supplier inv# already used - continuing (setting is ON)")
        return true
    }
    ; Button2 = No on ERA's Yes/No dialog (Button1 = Yes, same dialog the old code
    ; clicked). Verified closed before we abort: a wrong control id would leave the
    ; dialog up and ERA wedged behind it, so that outcome is logged loudly instead
    ; of assumed away.
    ControlClick, Button2, ahk_id %used%, , Left, 1, NA
    WinWaitClose, ahk_id %used%, , 1
    if (ErrorLevel) {
        ControlSend, , {Esc}, ahk_id %used%      ; belt: Esc = No on a Yes/No dialog
        WinWaitClose, ahk_id %used%, , 1
        if (ErrorLevel)
            Log("receipt: " plan.id " inv-used dialog did NOT close - check ERA")
    }
    g_rcpUserAbort := true       ; final - a re-run cannot change "already used"
    Log("receipt: " plan.id " aborted - supplier inv# already used, answered ERA No (no retry)")
    ShowToast("bad", "Already receipted", plan.inv " was receipted before for this vendor - answered No, run stopped")
    return false
}
...
; Retry only operations whose server semantics are safe to repeat (snapshot replacement
; and import from the same verified feeds). Generic parse/send calls keep their original
; behavior and are not broadened by this helper.
HttpPostRetriable(url, body, ctype := "application/json", maxAttempts := 3) {
    out := {status: 0, text: ""}
    Loop, %maxAttempts% {
        out := HttpPostJson(url, body, false, ctype, 15000, 1)
        if (!HttpTransient(out.status, 0) || A_Index >= maxAttempts)
            return out
        Sleep, % (A_Index = 1 ? 350 : 900)
    }
    return out
...
HttpPostPumpedRetriable(url, body, ctype := "application/json", timeoutS := 60, maxAttempts := 3) {
    out := {status: 0, text: ""}
    Loop, %maxAttempts% {
        out := HttpPostJsonPumped(url, body, ctype, timeoutS)
        if (!HttpTransient(out.status, 0) || A_Index >= maxAttempts)
            return out
        Sleep, % (A_Index = 1 ? 350 : 900)
    }
...
PoExportRun(silent := false, doImport := true) {
    ...
    if (ok && doImport)
        ok := DockImportRun(true)    ; propagate the import result instead of claiming upload-only success
    busy := false
    ; Full refresh performs one final status check after its verified import. Avoid three
    ; redundant network pulls here while the invoice lane is trying to finish quickly.
    if (!silent)
        StatusRunCheck(false)
    return ok
}
...
EraExportPo() {
    ...
    if (!WaitFileStable(found, 10000, 2))
        return EraFail("The fresh CSV appeared but ERA/OneDrive kept writing it for more than 10 seconds: " found)
